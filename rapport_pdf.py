...
def generate_rapport(data):
    ...
    for _bidx, bloc in enumerate(mesures_techniques):
        ...
        if bloc_type == 'tableau':
            ...
            def _mod_label(txt):
                # Remplacer les espaces par des espaces insécables pour empêcher le retour à la ligne
                escaped = _xml_esc_m(txt).replace(' ', '&nbsp;')
                return Paragraph(f'<font color="#64748B" size="8"><b>{escaped}</b></font>', styles['Normal'])
            ...
            mes_table = Table(mes_data, colWidths=[_avail_w * 0.35, _avail_w * 0.65])
            base_style = [
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                ('LEFTPADDING', (0, 0), (-1, -1), 6),
                ('RIGHTPADDING', (0, 0), (-1, -1), 6),

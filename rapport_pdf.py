...
def generate_rapport(data):
    ...
    for bloc in mesures_techniques:
        ...
        # ─── Type TABLEAU : 3 colonnes (Mesure / Valeur / Unité) ───
        if bloc_type == 'tableau':
            mes_data = [[
                Paragraph('<font color="white"><b>Mesure</b></font>', styles['Normal']),
                Paragraph('<font color="white"><b>Valeur</b></font>', styles['Normal']),
                Paragraph('<font color="white"><b>Unité</b></font>', styles['Normal']),
            ]]
            for lg in lignes:
                val = (lg.get('valeur') or '').strip()
                display_val = val if val else '—'
                mes_data.append([
                    Paragraph(_xml_esc_m(lg.get('libelle','')), styles['Normal']),
                    Paragraph(_xml_esc_m(display_val), styles['Normal']),
                    Paragraph(_xml_esc_m(lg.get('unite','')), styles['Normal']),
                ])
            mes_table = Table(mes_data, colWidths=[9*cm, 5*cm, 3*cm])

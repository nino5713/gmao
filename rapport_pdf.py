...
def _render_modules_mesures(mesures_techniques, styles):
    ...
    Chaque module est titré et contient un tableau libellé/valeur/unité.
    Retourne une liste d'éléments Flowable.
    v218.207 : ignore les slugs Heroicons (ex: "chart-bar") car Helvetica ne rend pas les emojis.
    v218.208 : si une icône PNG existe pour le slug dans icons/modules/, l'affiche à gauche du titre."""
    story = []
    if not mesures_techniques:
        return story
    for bloc in mesures_techniques:
        nom = bloc.get("nom") or "MODULE"
        # v218.208 : si l'icône a un PNG correspondant, on l'affiche dans le bandeau de titre
        icon_slug = (bloc.get("icon") or "").strip()
        icon_path = _module_icon_path(icon_slug) if icon_slug else None
        title_text = nom

        # Helper local pour récupérer le titre approprié (avec ou sans icône)
        def _mk_title():
            return _section_title_with_icon(title_text, icon_path, styles) if icon_path else _section_title(title_text, styles)
        btype = (bloc.get("type") or "tableau").lower()
        lignes = bloc.get("lignes") or []

        # Construction du contenu selon le type de bloc
        if btype in ("tableau", "mesures_batteries", "checklist", "preconisations"):
            if not lignes:
                continue
            # Header
            tbl_data = [[
            ...
            tbl = Table(tbl_data, colWidths=[9*cm, 5*cm, 3*cm])
            tbl.setStyle(TableStyle([
            ...
            story.append(KeepTogether([_mk_title(), tbl]))
            story.append(Spacer(1, 8))

        elif btype == "texte":
            # Texte libre : on prend le premier libellé/valeur
            txt = ""
            if lignes:
                txt = (lignes[0].get("valeur") or "").strip()
            if txt:
                p = Paragraph(f'<font size="10">{txt}</font>', styles['Normal'])
                story.append(KeepTogether([_mk_title(), p]))
                story.append(Spacer(1, 8))
            ...
            if png:
                try:
                    img = Image(io.BytesIO(png), width=15*cm, kind='proportional')
                    story.append(KeepTogether([_mk_title(), img]))
                    story.append(Spacer(1, 8))
                except Exception:
                    pass

        else:
            # Type non géré : afficher juste le titre + liste
            if lignes:
                tbl_data = []
                for lg in lignes:
                    lib = str(lg.get("libelle") or "")
                ...
                tbl = Table(tbl_data, colWidths=[10*cm, 7*cm])
                tbl.setStyle(TableStyle([
                    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                    ('GRID', (0, 0), (-1, -1), 0.4, BORDER),
                    ('LEFTPADDING', (0, 0), (-1, -1), 6),

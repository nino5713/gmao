...
def generate_planning_client_pdf(data):
    ...
    if intro_data:
        # En paysage : largeur utile ~24.7 cm
        intro_table = Table(intro_data, colWidths=[5*cm, 19.5*cm])
    ...
    if len(rows_table) == 1:
        ...
        # Largeurs en paysage A4 (≈ 27.7 cm utiles = 24.7 cm avec marges 1.3+1.3) :
        # N° 1cm | Équipement 5cm | Technique 3cm | Localisation 4cm | Type 4cm | Occurrence 2.7cm | Date 5cm
        main_table = Table(
            rows_table,
            colWidths=[1*cm, 5*cm, 3*cm, 4*cm, 4*cm, 2.7*cm, 5*cm],
            repeatRows=1
        )

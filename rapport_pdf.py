    ...
    if btype in ("tableau", "checklist"):
        ...
        if not pieces:
            ...
            header_row = [
                ...
                Paragraph('<b><font color="white" size="9">Réf.</font></b>', styles['Normal']),
                Paragraph('<b><font color="white" size="9">N° série</font></b>', styles['Normal']),
            ...
            for pc in pieces:
                ...
                tbl_data.append([
                    Paragraph(f'<font size="9"><b>{pc.get("type_piece", "") or "—"}</b></font>', styles['Normal']),
                    Paragraph(f'<font size="9">{pc.get("reference", "") or "—"}</font>', styles['Normal']),
                    Paragraph(f'<font size="9">{pc.get("numero_serie", "") or "—"}</font>', styles['Normal']),
                    ...
                    Paragraph(f'<font size="9" color="{lvl_col}">{pc.get("preconisation", "") or ""}</font>', styles['Normal']),
                ])
                r_idx = len(tbl_data) - 1
                # Fond coloré pour cellule Statut
                row_styles.append((r_idx, 4, colors.HexColor(statut_bg)))
                # Fond coloré pour cellule Préconisation
                row_styles.append((r_idx, 5, lvl_bg))

            # Largeurs des colonnes (au prorata de width_cm)
            ratio = width_cm / 17.0
            col_widths = [
                2.8 * cm * ratio,  # Type
                2.5 * cm * ratio,  # Réf
                2.2 * cm * ratio,  # N° série
                1.8 * cm * ratio,  # Installation
                2.2 * cm * ratio,  # Statut
                5.5 * cm * ratio,  # Préconisation
            ]

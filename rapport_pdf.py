...
def _section_title(text, styles, width=None):
...
def generate_rapport(data):
    ...
    def _close_pair_into(real_story_target, sub_l, sub_r):
        ...
        _half_w = 8.4*cm
        # On insère directement les listes de flowables dans les cellules.
        # Les modules savent eux-mêmes adapter leur largeur via la variable _avail_w.
        ...
        _pair_table = Table(_pair_data, colWidths=[_half_w, _half_w], style=TableStyle([
            ('VALIGN', (0,0), (-1,-1), 'TOP'),
    ...
    for _bidx, bloc in enumerate(mesures_techniques):
        ...
        if bloc_type == 'tableau':
            ...
            mes_table = Table(mes_data, colWidths=[_avail_w * 0.47, _avail_w * 0.53])
            base_style = [
                ('BACKGROUND', (0, 0), (-1, 0), ACCENT),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
            ...
            story.append(KeepTogether([
                _section_title(bloc_nom.upper(), styles, _avail_w),
                mes_table,
            ]))
            story.append(Spacer(1, _sect_gap))
        ...
        # ─── Type GRAPHIQUE : image PNG générée par matplotlib (pré-calculée côté serveur) ───
            ...
            if png_bytes:
                ...
                story.append(_section_title(bloc_nom.upper(), styles, _avail_w))
                story.append(img)
            story.append(Spacer(1, _sect_gap))
        ...
        # ─── Type MESURES BATTERIES : grille 6 colonnes par chaîne, valeurs colorées ───
            ...
            if not chains:
                ...
                story.append(Paragraph(config_html, styles['Normal']))
                story.append(Spacer(1, 4))

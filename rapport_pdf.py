...
def _build_module_flowables(bloc, width_cm, styles, mk_title):
    ...
    if btype in ("tableau", "checklist"):
    ...
    elif btype == "preconisations":
        # v218.218 : rendu spécifique du module Préconisations.
        # Tableau des pièces de l'équipement (Type / Réf / N° série / Installation / Statut / Préconisation),
        # avec couleur de fond selon le niveau (critique / warning / info / ok).
        pieces = bloc.get("pieces") or []
        if not pieces:
            warn = Paragraph(
                '<font size="9" color="#78350F"><i>⚠ Aucune pièce enregistrée pour cet équipement.</i></font>',
                styles['Normal']
            )
            ...
            # Mêmes couleurs que le frontend
            STATUT_DISPLAY = {
                'OK':            ('OK',            '#065F46', '#ECFDF5'),
                'A_SURVEILLER':  ('À surveiller',  '#92400E', '#FEF3C7'),
                'A_REMPLACER':   ('À remplacer',   '#991B1B', '#FEE2E2'),
            }
            LEVEL_BG  = {
                'critique': colors.HexColor('#FEE2E2'),
            ...
            # En-tête : bleu SOCOM comme tous les autres modules (v218.222)
            # v218.221 : 4 colonnes uniquement (Type / Installation / Statut / Préconisation)
            #           — Réf. et N° série retirés pour gagner de l'espace.
            HEADER_BG = ACCENT
            header_row = [
                Paragraph('<b><font color="white" size="9">Type</font></b>', styles['Normal']),
                Paragraph('<b><font color="white" size="9">Installation</font></b>', styles['Normal']),
                Paragraph('<b><font color="white" size="9">Statut</font></b>', styles['Normal']),
                Paragraph('<b><font color="white" size="9">Préconisation</font></b>', styles['Normal']),
            ]
            tbl_data = [header_row]
            # Pour appliquer les couleurs ligne par ligne sur les colonnes Statut + Préconisation
            row_styles = []
            # Style centré pour la cellule Installation
            center_style = ParagraphStyle(
                'PrecoCenter', parent=styles['Normal'], alignment=1
            )
            # v218.221 : on collecte aussi le texte brut pour mesurer la largeur min
            raw_rows = []  # liste de (type, install, statut_label, preco)
            for pc in pieces:
                statut_raw = (pc.get('statut') or 'OK').upper()
                statut_label, statut_col, statut_bg = STATUT_DISPLAY.get(
                    statut_raw, STATUT_DISPLAY['OK']
                )
                level = (pc.get('preconisation_level') or 'ok').lower()
                lvl_bg = LEVEL_BG.get(level, LEVEL_BG['ok'])
                lvl_col = LEVEL_COL.get(level, LEVEL_COL['ok'])
                # Date installation : on extrait l'année si format YYYY-MM-DD, sinon brut
                date_inst = pc.get('date_installation') or ''
                if isinstance(date_inst, str) and len(date_inst) >= 4 and date_inst[:4].isdigit():
                    date_disp = date_inst[:4]
                else:
                    date_disp = date_inst or '—'
                type_disp = pc.get("type_piece", "") or "—"
                preco_disp = pc.get("preconisation", "") or ""
                tbl_data.append([
                    Paragraph(f'<font size="9"><b>{type_disp}</b></font>', styles['Normal']),
                    Paragraph(f'<font size="9">{date_disp}</font>', center_style),
                    Paragraph(f'<font size="9" color="{statut_col}"><b>{statut_label}</b></font>', styles['Normal']),
                    Paragraph(f'<font size="9" color="{lvl_col}">{preco_disp}</font>', styles['Normal']),
                ])
                raw_rows.append((type_disp, date_disp, statut_label, preco_disp))
                r_idx = len(tbl_data) - 1
                # Fond coloré pour cellule Statut (colonne 2)
                row_styles.append((r_idx, 2, colors.HexColor(statut_bg)))
                # Fond coloré pour cellule Préconisation (colonne 3)
                row_styles.append((r_idx, 3, lvl_bg))

            # v218.221 : calcul automatique des largeurs de colonnes selon le contenu.
            # On mesure la largeur du texte le plus long dans chaque colonne via stringWidth,
            # puis on répartit en gardant un minimum + de la place pour la préconisation.
            from reportlab.pdfbase.pdfmetrics import stringWidth
            def _max_width(idx, headers, font_size=9):
                # En-têtes (gras), donc Helvetica-Bold
                widths = [stringWidth(headers[idx], 'Helvetica-Bold', font_size)]
                for row in raw_rows:
                    txt = row[idx] or ''
                    # Type et Statut sont en gras
                    font = 'Helvetica-Bold' if idx in (0, 2) else 'Helvetica'
                    widths.append(stringWidth(txt, font, font_size))
                return max(widths) if widths else 0
            headers_txt = ['Type', 'Installation', 'Statut', 'Préconisation']
            # +12pt pour padding interne (5+5 + marge confort)
            pad = 14
            min_pts = [_max_width(i, headers_txt) + pad for i in range(4)]
            # Total disponible en points (1 cm ≈ 28.35 pt)
            total_pts = width_cm * 28.35
            sum_min = sum(min_pts)
            if sum_min <= total_pts:
                # Tout rentre naturellement : on donne l'espace restant à la colonne Préconisation
                extra = total_pts - sum_min
                col_pts = min_pts[:]
                col_pts[3] += extra  # toute la marge va à Préconisation
            else:
                # Ça déborde : on réduit proportionnellement la colonne Préconisation,
                # puis Type et Statut si encore trop large. Installation et en-têtes restent au min.
                col_pts = min_pts[:]
                overflow = sum_min - total_pts
                # Étape 1 : réduire Préconisation (jusqu'à un plancher de 6cm)
                preco_floor = 6 * 28.35
                reduce_preco = min(overflow, max(0, col_pts[3] - preco_floor))
                col_pts[3] -= reduce_preco
                overflow -= reduce_preco
                # Étape 2 : s'il reste du débordement, réduire Type et Statut proportionnellement
                if overflow > 0:
                    reducible = col_pts[0] + col_pts[2]
                    if reducible > 0:
                        ratio_red = overflow / reducible
                        col_pts[0] *= max(0.5, 1 - ratio_red)
                        col_pts[2] *= max(0.5, 1 - ratio_red)
            col_widths = col_pts  # en points (ReportLab accepte les points)
            ...
            # Appliquer les couleurs de fond sur les cellules statut + préconisation
        ...
        if not chains:
            ...
            for chain in chains:
                ...
                # v218.223 : flowables locaux de la chaîne (regroupés dans un KeepTogether
                # pour empêcher le split entre pages — si pas assez de place, la chaîne
                # entière saute à la page suivante).
                chain_flows = []
                # Titre de la chaîne
                chain_title_html = (
                ...
                chain_flows.append(Paragraph(chain_title_html, styles['Normal']))
                chain_flows.append(Spacer(1, 3))
                ...
                chain_flows.append(bat_tbl)
                chain_flows.append(Spacer(1, 6))
                # v218.223 : KeepTogether sur toute la chaîne — saute à la page suivante
                # si pas assez de place pour titre + tableau.
                flow.append(KeepTogether(chain_flows))
...
def _render_modules_mesures(mesures_techniques, styles):
    ...
    def _is_heavy(bloc):
        ...
        if btype == "mesures_batteries":
            ...
            return total > 18
        if btype == "preconisations":
            # v218.218 : preconisations utilise un tableau 6 colonnes — peu compatible avec demi-largeur.
            # Considéré lourd dès qu'il y a au moins une pièce.
            pieces = bloc.get("pieces") or []
            return len(pieces) > 0
        lignes = bloc.get("lignes") or []

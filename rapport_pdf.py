...
def generate_rapport(data):
    ...
    # ═════════ GAMME DE MAINTENANCE (BP uniquement) ═════════
    gamme = data.get('gamme_maintenance') or []
    if data.get('is_maintenance') and gamme:
        def _xml_esc(s):
            return (str(s) if s is not None else "").replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
        gamme_rows = [['Sous-équipement', 'Localisation', 'Date de réalisation', 'CR']]
        for g in gamme:
            nom = g.get('nom','') or '—'
        ...
        gamme_table = Table(gamme_rows, colWidths=[5.5*cm, 4*cm, 4*cm, 3.5*cm])
        gamme_table.setStyle(TableStyle([
        ...
        story.append(_section_title("GAMME DE MAINTENANCE", styles))
        story.append(gamme_table)
        story.append(Spacer(1, 6))

    ...
    for _bidx, bloc in enumerate(mesures_techniques):
        ...
        if bloc_type == 'equipement':
            ...
            try:
                ...
                _eq_fopts = {}
            # Champs activés (par défaut, tous)
            ...
            # Défaut 35% = match du legacy [3*cm / 8.5*cm].
            ...
            _eq_field_specs = {
                ...
                'notes':             ('Notes',             data.get('eq_notes') or ''),
            }
            _eq_pairs = []
            _notes_value = ''
            for _f in _eq_active:
                if _f not in _eq_field_specs:
                    continue
                ...
                if _f == 'notes':
                    if _val and str(_val).strip():
                        _notes_value = _val
                    continue
                # Pour designation, on affiche toujours (même si '—'), pour les autres seulement si rempli
                if _f == 'designation' or (_val and str(_val).strip() not in ('', '—')):
                    _eq_pairs.append((_lbl, _val if _val else '—'))

            # Si rien à afficher, on saute
            if not _eq_pairs and not _notes_value:
                continue
            ...
            # v218.89 : Calcul des largeurs : 4 cellules (label | val | label | val)
            # _eq_label_ratio = fraction du label dans une demi-ligne. Une demi-ligne = _avail_w / 2.
            # On force la dernière colonne à absorber le reste pour éviter les erreurs d'arrondi.
            _half_w = _avail_w / 2.0
            _eq_lbl_w = _half_w * _eq_label_ratio
            _eq_val_w = _half_w - _eq_lbl_w
            _eq_lbl_w2 = _eq_lbl_w
            _eq_val_w2 = _avail_w - _eq_lbl_w - _eq_val_w - _eq_lbl_w2  # absorbe l'erreur d'arrondi
            _eq_table = Table(_eq_rows, colWidths=[_eq_lbl_w, _eq_val_w, _eq_lbl_w2, _eq_val_w2])
            _eq_style_cmds = [
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                ('LEFTPADDING', (0, 0), (-1, -1), 6),
                ('RIGHTPADDING', (0, 0), (-1, -1), 6),
            ...
            _eq_table.setStyle(TableStyle(_eq_style_cmds))
            _eq_title = bloc_nom.upper() if bloc_nom else 'ÉQUIPEMENT CONCERNÉ'
            # v218.91 : le tableau equipement est centré par défaut (hAlign CENTER de ReportLab),
            # donc on retire le hAlign='LEFT' du titre pour qu'il s'aligne avec le tableau.
        ...
        # ─── Type IMAGE_CHECKLIST : image à gauche + checklist à droite ───
            ...
            if _ic_image_rel:
                ...
                if not os.path.exists(_ic_img_abs):
                    # Fallback : essayer chemin absolu direct
                ...
                if _ic_img_abs and os.path.exists(_ic_img_abs):
                    try:
                        ...
                        with PILImageLib_ic.open(_ic_img_abs) as _pim:
                            _iw, _ih = _pim.size
                        # Largeur cible : largeur de la cellule gauche moins padding (8pt total)
                        _ic_target_w = _ic_left_w - 8
                        # Hauteur max : 12cm pour rester raisonnable
                        _ic_max_h = 12 * cm
                        _ic_target_h = _ic_target_w * (_ih / _iw) if _iw > 0 else _ic_max_h
                        if _ic_target_h > _ic_max_h:
                            _ic_target_h = _ic_max_h
                            _ic_target_w = _ic_target_h * (_iw / _ih) if _ih > 0 else _ic_target_w
                        _ic_image_cell = RLImage_ic(_ic_img_abs, width=_ic_target_w, height=_ic_target_h)
            ...
            if _ic_image_cell is None:
                _ic_image_cell = Paragraph(
                    '<i><font color="#94a3b8" size="9">Aucune image configurée</font></i>',
                    styles["Normal"]
                )

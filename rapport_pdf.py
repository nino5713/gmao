    ...
    for b in blocks or []:
        try:
            ...
            if t == 'spacer':
                ...
                # v218.106 : filtrer uniquement les opérations réalisées (done=True)
                has_ops = any(
                    any((op.get('done') if isinstance(op, dict) else False) for op in (g.get('operations') or []))
                ...
                for g in gamme_ops_list:
                    ...
                    # v218.106 : ne garder que les opérations effectivement réalisées
                    ops_done = [op for op in ops if isinstance(op, dict) and op.get('done')]
                    if not ops_done: continue
                    # v218.107 : on n'affiche plus le nom de la gamme avant les opérations
                    for op in ops_done:
...
def generate_rapport(data):
    ...
    if is_maintenance_layout:
        ...
        # v218.106 : ne montrer la section que s'il y a au moins une opération réalisée
        gamme_ops_list_main = data.get('gamme_operations') or []
        has_gamme_ops_main = any(
            any((op.get('done') if isinstance(op, dict) else False) for op in (g.get('operations') or []))
    ...
    gamme_ops_list = data.get('gamme_operations') or []
    # v218.106 : ne considérer que les opérations réalisées
    has_gamme_ops = is_maint and any(
        any((op.get('done') if isinstance(op, dict) else False) for op in (g.get('operations') or []))
        for g in gamme_ops_list
    )

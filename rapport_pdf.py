...
def generate_rapport(data):
    ...
    if cellules:
        ...
        cell_rows = [['Désignation', 'Marque', 'Type']]
        for c in cellules:
            cell_rows.append([
                Paragraph(_xml_esc_c(c.get('designation','') or '—'), styles['Normal']),
                Paragraph(_xml_esc_c(c.get('marque','') or ''), styles['Normal']),
                Paragraph(_xml_esc_c(c.get('type','') or ''), styles['Normal']),
            ])
        cell_table = Table(cell_rows, colWidths=[7*cm, 5*cm, 5*cm])

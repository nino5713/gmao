...
def generate_rapport(data):
    ...
    if securite_items:
        ...
        def _make_check_cell(label, checked):
            ...
            color = '#1E3A8A' if checked else '#64748B'
        ...
        def _build_sec_card(item):
            ...
            conforme = bool(item.get('conforme'))
            # Inverse pour Absent / Non conforme
            absent = not present
            non_conforme = not conforme
            # Photo
            photo_cell = ''
            ...
            # Libellé en haut sur fond bleu clair
            title = Paragraph(f'<font size="9"><b>{_xml_esc_s(item.get("libelle",""))}</b></font>', h_normal)
            # Tableau interne : 1ère ligne = titre fusionné ; 2ème ligne = photo (col 1) + 4 cases en colonne (col 2)
            checks_table = Table([

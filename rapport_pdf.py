...
def generate_rapport(data):
    ...
    for _bidx, bloc in enumerate(mesures_techniques):
        ...
        # Pour 'graphique', 'preconisations', 'mesures_batteries', 'image' et 'equipement', les lignes ne sont pas affichées une à une
        if not lignes and bloc_type not in ('graphique', 'preconisations', 'mesures_batteries', 'image', 'equipement'):
            continue
        ...
        if bloc_type == 'equipement':
            ...
            # Ratio de largeur (utilisé pour la disposition 2-cols : label | val)
            # Pour ce module, on garde le layout 4-cells (label | val | label | val) si plusieurs champs simples
            # Mais on permet aussi le ratio si l'utilisateur le règle
            _eq_label_ratio = 0.20  # défaut : labels prennent 20% (3cm sur 17cm avec 4 cellules)
            try:
                _lr = _eq_fopts.get('label_ratio')
                if _lr is not None:
                    _lr_int = int(_lr)
                    if 10 <= _lr_int <= 60:  # max 60% pour rester lisible
            ...
            # Calcul des largeurs : 4 cellules (label/val/label/val)
            # Avec ratio configurable : labels prennent _eq_label_ratio chacun, vals le reste
            _eq_lbl_w = _avail_w * _eq_label_ratio
            _eq_val_w = _avail_w * (0.5 - _eq_label_ratio)  # 0.5 par "moitié"
            _eq_table = Table(_eq_rows, colWidths=[_eq_lbl_w, _eq_val_w, _eq_lbl_w, _eq_val_w])
            _eq_style_cmds = [
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                ('LEFTPADDING', (0, 0), (-1, -1), 6),
                ('RIGHTPADDING', (0, 0), (-1, -1), 6),
            ...
            story.append(KeepTogether([
                _section_title(_eq_title, styles, _avail_w, icon=bloc.get("icon")),
                _eq_table,
            ]))
            story.append(Spacer(1, _sect_gap))

...
def _build_module_flowables(bloc, width_cm, styles, mk_title):
    ...
    # Titre : recalculer à la bonne largeur si demi
    ...
    if btype in ("tableau", "mesures_batteries", "checklist", "preconisations"):
        ...
        img_name = bloc.get("equipement_image_nom") or ""
        if img_path and os.path.exists(img_path):
            try:
                ...
                if img_name:
                    ...
                    cap = Paragraph(
                        f'<font size="8" color="#64748B"><i>{img_name}</i></font>',
                        styles['Normal']
                    )
                    flow.append(cap)
            except Exception:
...
def _render_modules_mesures(mesures_techniques, styles):
    ...
    while i < n:
        ...
        if bl_largeur == "demi" and i + 1 < n:
            ...
            if (bloc2.get("largeur") or "pleine").lower() == "demi":
                ...
                if flows1 and flows2:
                    # Empaqueter chaque colonne dans un mini-doc
                    col1 = KeepTogether(flows1)
                    col2 = KeepTogether(flows2)
                    pair = Table([[col1, col2]], colWidths=[half_w*cm, half_w*cm])
        ...
        # Cas 2 : module pleine largeur OU module demi isolé

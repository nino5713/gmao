        ...
        for g in gamme_ops_list:
            ...
            for op in ops:
                ...
                if done:
                    ...
                    suffix = ''
                    if date_real or tech_nom:
                        info_parts = []
                        if date_real: info_parts.append('le ' + date_real)
                        if tech_nom: info_parts.append('par ' + tech_nom)
                        suffix = f'  <font color="#10B981" size="8"><i>(fait {" ".join(info_parts)})</i></font>'
                    check_rows.append([
                        Paragraph(f'{box}&nbsp;&nbsp;{_xml_esc_d(desc)}{suffix}', h_normal)
                    ])
                else:
                    check_rows.append([

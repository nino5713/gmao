...
def generate_rapport(data):
    ...
    def _xml_esc_m(s):
        return (str(s) if s is not None else "").replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
    for bloc in mesures_techniques:
        bloc_nom = bloc.get('nom', '') or ''
        bloc_type = (bloc.get('type') or 'tableau').lower()
        lignes = bloc.get('lignes') or []
        # Pour 'graphique' et 'preconisations', les lignes ne sont pas affichées une à une
        if not lignes and bloc_type not in ('graphique', 'preconisations'):
            continue
        ...
        if bloc_type == 'tableau':
            ...
            mes_data = [[
                Paragraph('<font color="white"><b>Élément</b></font>', styles['Normal']),
                Paragraph('<font color="white"><b>Valeur</b></font>', styles['Normal']),
            ]]
            # Style commands pour la coloration des cellules (binary)
            row_styles = []
            for idx, lg in enumerate(lignes):
                row_idx = idx + 1  # +1 pour le header
                ...
                libelle_html = f'<b>{_xml_esc_m(lg.get("libelle",""))}</b>'

                if ftype == 'numeric':
                    # Affichage : valeur + unité
                    unite = lg.get('unite','') or ''
                    if val_raw:
                        display = f"{_xml_esc_m(val_raw)} <font color='#64748b'>{_xml_esc_m(unite)}</font>" if unite else _xml_esc_m(val_raw)
                    else:
                        display = '—'
                    mes_data.append([
                        Paragraph(libelle_html, styles['Normal']),
                        Paragraph(display, styles['Normal']),
                    ])

                elif ftype == 'binary':
                    # Choix binaire : afficher le label sélectionné, coloré
                    ...
                    # Choix multiple : afficher le choix
                    display = _xml_esc_m(val_raw) if val_raw else '—'
                    mes_data.append([
                        Paragraph(libelle_html, styles['Normal']),
                        Paragraph(f'<b>{display}</b>', styles['Normal']),
                    ])

                elif ftype == 'text_long':
                    # Texte long : affichage multi-lignes
                    ...
                    # Deux champs côte à côte stockés séparés par |||
            ...
            mes_table = Table(mes_data, colWidths=[8*cm, 9*cm])
            base_style = [
                ('BACKGROUND', (0, 0), (-1, 0), ACCENT),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
            ...
            # Appliquer les styles spécifiques (binary) après les ROWBACKGROUNDS pour qu'ils dominent
            mes_table.setStyle(TableStyle(base_style + row_styles))
            story.append(KeepTogether([
                _section_title(bloc_nom.upper(), styles),
                mes_table,
            ]))
            story.append(Spacer(1, _sect_gap))
        ...
        # ─── Type TEXTE : zone de texte libre (paragraphe encadré) ───
            ...
            txt_table = Table([[txt_para]], colWidths=[17*cm])
        ...
        # ─── Type CHECKLIST : liste de cases à cocher ───
            ...
            for lg in lignes:
                ...
                if done:
                    box = '<font name="Helvetica-Bold" size="13" color="#10B981">&#10003;</font>'
                    check_rows_m.append([
                        Paragraph(f'{box}&nbsp;&nbsp;{_xml_esc_m(lg.get("libelle",""))}', styles['Normal'])
                    ])
                else:
                    check_rows_m.append([
                        Paragraph(f'<font size="13">&nbsp;&nbsp;&nbsp;</font>&nbsp;&nbsp;{_xml_esc_m(lg.get("libelle",""))}', styles['Normal'])
                    ])
            check_table_m = Table(check_rows_m, colWidths=[17*cm])
        ...
        # ─── Type GRAPHIQUE : image PNG générée par matplotlib (pré-calculée côté serveur) ───
            ...
            if png_bytes:
                ...
                img = RLImage(_io_pdf.BytesIO(png_bytes), width=17*cm, height=8.5*cm, kind='proportional')
                story.append(_section_title(bloc_nom.upper(), styles))
                story.append(img)
            story.append(Spacer(1, _sect_gap))
        ...
        # ─── Type PRÉCONISATIONS : tableau des pièces de l'équipement avec préconisation calculée ───
            ...
            if not pieces:
                ...
                empty_box = Table([[empty_para]], colWidths=[17*cm])
                ...
                def _fmt_date_p(s):
                    ...
                    try:
                        ...
                        return str(s)
                # Header
                hdr_style = ParagraphStyle('precoHdr', fontSize=8, textColor=colors.white, fontName='Helvetica-Bold', leading=10)
                ...
                preco_table = Table(preco_data, colWidths=[2.2*cm, 2.4*cm, 2.4*cm, 2*cm, 2*cm, 2.2*cm, 3.8*cm])
                base_style_p = [
                    ('BACKGROUND', (0, 0), (-1, 0), ACCENT),
                    ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                    ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),

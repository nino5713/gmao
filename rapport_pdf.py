...
def _build_field_map(data):
    ...
    return {
        ...
        "projet": data.get("projet_nom") or data.get("numero_projet") or "",
...
def _render_cover_blocks_dynamic(cover_blocks, data, styles):
    ...
    for b in cover_blocks:
        ...
        if t == "spacer":
            ...
            path = data.get("equipment_image_path") or ""
            if path and os.path.exists(path):
                try:
...
def _render_modules_mesures(mesures_techniques, styles):
    ...
    for bloc in mesures_techniques:
        ...
        icon = bloc.get("icon") or ""
        title_text = (icon + " " + nom).strip() if icon else nom
        ...
        if btype in ("tableau", "mesures_batteries", "checklist", "preconisations"):
            ...
            story.append(KeepTogether([_section_title(title_text, styles), tbl]))
            ...
            if txt:
                ...
                story.append(KeepTogether([_section_title(title_text, styles), p]))
                story.append(Spacer(1, 8))
            ...
            if png:
                try:
                    ...
                    story.append(KeepTogether([_section_title(title_text, styles), img]))
                    story.append(Spacer(1, 8))
                except Exception:
                    pass

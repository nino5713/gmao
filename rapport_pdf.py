...
def generate_planning_client_pdf(data):
    ...
    # ─── Bloc d'introduction ──────────────────────
    intro_data = []
    if projet.get('client_nom'):
        intro_data.append(['Client', projet['client_nom']])
    if projet.get('nom'):
        intro_data.append(['Site / Projet', projet['nom']])
    if projet.get('numero_projet'):
        intro_data.append(['N° de projet', projet['numero_projet']])
    if projet.get('adresse'):
        intro_data.append(['Adresse', projet['adresse']])
    intro_data.append(['Période', f"Année {annee}"])
    intro_data.append(["Nombre d'interventions", str(len([l for l in lignes if l.get('date')]))])

    if intro_data:
        intro_table = Table(intro_data, colWidths=[4.5*cm, 13.5*cm])
    ...
    # ─── Tableau principal ──────────────────────────
    story.append(Paragraph("Calendrier des interventions", style_h2))
    story.append(Spacer(1, 0.2*cm))

    rows_table = [['N°', 'Équipement', "Type d'intervention", 'Date prévue', 'Horaire']]
    ...
    for idx, l in enumerate(lignes_valides, 1):
        ...
        if l.get('occ_total', 1) > 1:
            gamme += f" ({l.get('occ_idx', 0) + 1}/{l['occ_total']})"
        perio = l.get('periodicite', '')
        if perio:
            gamme_para = Paragraph(
                f"<b>{_xml_escape_plan(gamme)}</b><br/><font size='8' color='#64748B'>{_xml_escape_plan(perio)}</font>",
                style_normal
            )
        else:
            gamme_para = Paragraph(f"<b>{_xml_escape_plan(gamme)}</b>", style_normal)

        rows_table.append([
            str(idx),
            Paragraph(f"<b>{_xml_escape_plan(l.get('equipement', '—'))}</b>", style_normal),
            gamme_para,
            date_fmt,
            l.get('heure', '08:00'),
        ])
    ...
    if len(rows_table) == 1:
        ...
        main_table = Table(
            rows_table,
            colWidths=[1.0*cm, 5.5*cm, 5.0*cm, 4.8*cm, 1.8*cm],
            repeatRows=1
        )
    ...
    notes = (
        "<b>Note :</b> Les dates indiquées sont prévisionnelles et peuvent faire l'objet d'ajustements "
        "en concertation avec le client. En cas de jour férié ou de weekend, l'intervention est "
        "automatiquement reportée au jour ouvré suivant. "
        "Pour toute modification ou question, veuillez contacter votre interlocuteur SOCOM."
    )
    ...
    # ─── Bloc signature ───────────────────────────────────────────
    sig_data = [
        [Paragraph('<b>Pour SOCOM S.A.</b>', style_normal),
         Paragraph('<b>Bon pour accord — Client</b>', style_normal)],
        ['', ''], ['', ''], ['', ''],
        [Paragraph('<font size="8" color="#64748B">Nom, fonction, signature</font>', style_muted),
         Paragraph('<font size="8" color="#64748B">Date, nom, signature</font>', style_muted)],
    ]
    sig_table = Table(sig_data, colWidths=[9*cm, 9*cm], rowHeights=[0.7*cm, 0.6*cm, 0.6*cm, 0.6*cm, 0.6*cm])
    sig_table.setStyle(TableStyle([
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),

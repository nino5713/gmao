...
def get_occupations():
    ...
    sql = """SELECT o.*, u.nom AS technicien_nom, ot.nom AS type_nom, ot.couleur AS type_couleur,
             ...
             WHERE 1=1"""
    params = []
    # v218.48 : un tech voit ses propres occupations + celles où il est accompagnant
    # On utilise json_each pour parser le champ accompagnants_ids (JSON array)
    def _accompagnant_filter(uid):
        # Le champ accompagnants_ids stocke un JSON array (ex: '[2, 5, 8]') ou chaîne vide
        # SQLite json_each renverra des erreurs si NULL/empty -> on protège avec un IIF/CASE
        return ("(o.technicien_id = ? OR ("
                "o.accompagnants_ids IS NOT NULL AND o.accompagnants_ids != '' "

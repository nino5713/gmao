...
def _migrate_cover_pages(db):
    ...
    try:
        ...
        if not existing:
            db.execute("INSERT INTO cover_pages (societe_id, blocks_json) VALUES (1, ?)",
                       (json.dumps(_COVER_DEFAULT_BLOCKS),))
            logger.info("[cover-pages] SOCOM (id=1) initialisée avec layout par défaut")
        db.commit()
    except Exception as e:
...
@require_auth
def get_cover_page():
    """Retourne la configuration de la page de garde de la société active.
    Tous les utilisateurs peuvent consulter (utile pour preview), seuls
    admin et superadmin peuvent modifier (cf PUT)."""
    sid = current_societe_id()
    db = get_db()
    row = one(db.execute("SELECT blocks_json FROM cover_pages WHERE societe_id=?", (sid,)))
    if not row:
...
@require_role("admin")
def set_cover_page():
    """Met à jour la configuration de la page de garde de la société active.
    Body: { blocks: [...] }
    Réservé aux admins et superadmins."""
    d = request.json or {}
    blocks = d.get("blocks")
    if not isinstance(blocks, list):
        return jsonify({"error": "blocks doit être une liste"}), 400
    # Validation basique : chaque bloc a un 'type' valide
    valid_types = {"title", "subtitle", "image", "info_box", "text", "spacer", "separator"}
    cleaned = []
    for b in blocks:
        if not isinstance(b, dict): continue
        t = b.get("type")
        if t not in valid_types: continue
        cleaned.append(b)
    sid = current_societe_id()
    db = get_db()
    blocks_json = json.dumps(cleaned)
    # Upsert (INSERT OR REPLACE WHERE societe_id)
    existing = one(db.execute("SELECT id FROM cover_pages WHERE societe_id=?", (sid,)))
    if existing:
        db.execute("UPDATE cover_pages SET blocks_json=?, updated_at=datetime('now') WHERE societe_id=?",
                   (blocks_json, sid))
    else:
        db.execute("INSERT INTO cover_pages (societe_id, blocks_json) VALUES (?, ?)",
                   (sid, blocks_json))
    db.commit()
    log_action(request.user, "UPDATE", "cover-page", sid, f"sid={sid}", f"blocks={len(cleaned)}")
    return jsonify({"ok": True, "blocks": cleaned})
...
def get_interventions():
    ...
    sql="""SELECT i.*,e.designation AS equip_nom,e.type_technique,
           ...
           LEFT JOIN equipement_tableaux et ON i.tableau_id=et.id WHERE i.societe_id=?"""
    params=[sid]
    if u["role"]=="technicien":
...
def create_intervention():
    ...
    try:
        ...
        db.execute("""INSERT INTO interventions
            (numero,equipement_id,tableau_id,technicien_id,equipe_id,type,statut,date_prevue,heure_prevue,description,societe_id)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (num,d["equipement_id"],to_int(d.get("tableau_id")),main_tech,to_int(d.get("equipe_id")),
             type_iv,d.get("statut",default_statut),d.get("date_prevue") or None,heure_prevue_v,d.get("description",""),sid))
...
def generate_pdf(iid):
    ...
    try:
        ...
        try:
            ...
            _cover_row = one(db.execute("SELECT blocks_json FROM cover_pages WHERE societe_id=?", (_iv_sid,)))
            if _cover_row and _cover_row.get("blocks_json"):
...
def create_planning():
    ...
    sid = current_societe_id()
    db = get_db()
    db.execute("INSERT INTO plannings (nom,equipe_id,heures_par_jour,heure_debut,societe_id) VALUES (?,?,?,?,?)",
               (d["nom"],d.get("equipe_id"),d.get("heures_par_jour",8.0),d.get("heure_debut","08:00"),sid))
    db.commit()
    return jsonify({"id":db.execute("SELECT last_insert_rowid()").fetchone()[0]}),201

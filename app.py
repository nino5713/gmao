...
def list_equipement_champs():
    ...
    if technique_id:
        try:
            ...
            where.append("(technique_id IS NULL OR technique_id=?)")
            params.append(tid)
        except: pass
    sql = "SELECT c.*, t.nom AS technique_nom FROM equipement_champs c LEFT JOIN techniques t ON c.technique_id=t.id WHERE " + " AND ".join(where) + " ORDER BY c.technique_id NULLS FIRST, c.ordre, c.label"
    # SQLite ne supporte pas NULLS FIRST → contournement via CASE
    sql = sql.replace("c.technique_id NULLS FIRST", "(c.technique_id IS NULL) DESC, c.technique_id")
...
def get_equipement_valeurs_custom(eid):
    ...
    db = get_db()
    sid = current_societe_id()
    eq = one(db.execute("SELECT id, technique_id, societe_id FROM equipements WHERE id=?", (eid,)))

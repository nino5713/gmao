                ...
                if r.get("type") == "MAINTENANCE":
                    ...
                    if eq_id_bp:
                        ...
                        for gid in gids:
                            ...
                            ops = rows(db.execute(
                                "SELECT description FROM gamme_operations WHERE gamme_id=? ORDER BY ordre, id", (gid,)
                            ))
                            if ops:
...
def generate_pdf(iid):
    ...
    try:
        pdf = generate_rapport({
            ...
            "gamme_operations": (lambda eq_id_, type_iv: (
                (lambda gids: [
                    {
                        "gamme_nom": ((one(db.execute("SELECT nom FROM gammes WHERE id=?", (gid,))) or {}).get("nom","") or ""),
                        "operations": [op["description"] for op in rows(db.execute(
                            "SELECT description FROM gamme_operations WHERE gamme_id=? ORDER BY ordre, id", (gid,)
                        ))]
                    }
                    for gid in gids
            ...
            ))(i.get("equipement_id"), i.get("type")),

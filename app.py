...
def list_equipement_champs():
    ...
    v218.119 : SQL simplifié — on ordonne par (technique_id IS NULL) DESC pour mettre
    les globaux en premier, puis par ordre/label."""
    db = get_db()
    sid = current_societe_id()
    ...
    try:
        ...
        logger.info(f"[equipement-champs GET] {len(champs)} champ(s) trouvé(s) (sid={sid})")
    except Exception as e:

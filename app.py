...
def _bon_sous_type_label(intervention_id):
    ...
    """
    db = get_db()
    # Récupérer les gamme_id distinctes utilisées dans les CR de cette intervention
    # Plusieurs sources possibles : cr_gammes (table de liaison), cr_operations, etc.
    # On utilise comptes_rendus.gammes_ids qui est un CSV ou table de liaison.
    # Solution générique : remonter via interventions.gammes_ids (champ JSON/CSV) si présent,
    # sinon via les gammes liées aux opérations cochées dans le CR.
    iv = one(db.execute("SELECT gammes_ids FROM interventions WHERE id=?", (intervention_id,)))
    gamme_ids = []
    if iv and iv.get("gammes_ids"):
        try:
            raw = iv["gammes_ids"]
            if isinstance(raw, str):
                if raw.startswith("["):
                    gamme_ids = json.loads(raw)
                else:
                    gamme_ids = [int(x.strip()) for x in raw.split(",") if x.strip()]
        except Exception:
            gamme_ids = []
    if not gamme_ids:
        return None
    # Récupérer les sous-types distincts
    placeholders = ",".join(["?"] * len(gamme_ids))
    sous_types = rows(db.execute(
        f"""SELECT DISTINCT g.sous_type_id, st.nom
            FROM gammes g LEFT JOIN maintenance_sous_types st ON g.sous_type_id = st.id

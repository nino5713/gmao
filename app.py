...
def get_mesure_blocs():
    ...
    sql += " ORDER BY t.nom, st.ordre, mb.ordre, mb.nom"
...
def create_mesure_bloc():
    """Crée un nouveau bloc de mesures.
    Body : { technique_id, sous_type_id, nom, ordre, lignes: [{libelle, unite}, ...] }"""
    d = request.json or {}
    ...
    ordre = int(d.get("ordre") or 0)
    db = get_db()
    db.execute("INSERT INTO mesure_blocs (technique_id, sous_type_id, nom, ordre) VALUES (?, ?, ?, ?)",
               (technique_id, sous_type_id, nom, ordre))
    bid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    for i, lg in enumerate(d.get("lignes") or []):
        lib = (lg.get("libelle") or "").strip()
        if not lib: continue
        un = (lg.get("unite") or "").strip()
        db.execute("INSERT INTO mesure_lignes (bloc_id, libelle, unite, ordre) VALUES (?, ?, ?, ?)",
                   (bid, lib, un, i))
    db.commit()
...
def debug_intervention_mesures(iid):
    ...
    # 1. Vérif technique_id
    if not iv["technique_id"]:
        diag["etapes"].append("❌ L'équipement n'a PAS de technique_id renseigné (champ legacy: " + str(iv["type_technique"]) + ")")
        diag["solution"] = "Va dans la fiche de l'équipement et associe-le à une technique."
        return jsonify(diag)
    diag["etapes"].append("✓ technique_id = " + str(iv["technique_id"]))
    # Nom de la technique
    t = one(db.execute("SELECT nom FROM techniques WHERE id=?", (iv["technique_id"],)))
    ...
    """, (iv["technique_id"], st_row["id"])))
    diag["blocs_trouves"] = [b["nom"] for b in blocs]
    if not blocs:
        diag["etapes"].append("❌ Aucun bloc configuré pour technique_id=" + str(iv["technique_id"]) + " × sous_type_id=" + str(st_row["id"]))
...
def get_intervention_mesures(iid):
    ...
    db = get_db()
    iv = one(db.execute("""
        SELECT i.id, i.type, e.technique_id
        FROM interventions i
        JOIN equipements e ON i.equipement_id = e.id
        WHERE i.id = ?
    """, (iid,)))
    ...
    if not iv.get("technique_id") or not sous_type_id:
        return jsonify({"blocs": [], "valeurs": {},
                        "technique_id": iv.get("technique_id"),
                        "sous_type_id": sous_type_id,
                        "sous_type_label": sous_type_label})
    blocs = rows(db.execute("""
        SELECT mb.* FROM mesure_blocs mb
        WHERE mb.technique_id = ? AND mb.sous_type_id = ?
        ORDER BY mb.ordre, mb.nom
    """, (iv["technique_id"], sous_type_id)))
    for b in blocs:
        b["lignes"] = rows(db.execute(
            "SELECT * FROM mesure_lignes WHERE bloc_id=? ORDER BY ordre, id",
            (b["id"],)
...
def _intervention_mesures_data(intervention_id):
    ...
    db = get_db()
    iv = one(db.execute("""
        SELECT i.id, e.technique_id
        FROM interventions i
        JOIN equipements e ON i.equipement_id = e.id
        WHERE i.id = ?
    """, (intervention_id,)))
    if not iv or not iv.get("technique_id"):
        return []

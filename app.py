...
def generate_rapport_complet(projet_id):
    try:
        ...
        import base64, io, json as _json, calendar, re as _re
        ...
        ax.text(8,9.0,"TABLEAU DE BORD",ha="center",va="center",fontsize=10,color="white",fontweight="bold")
        kpi2=[("ELEC","284 kWh","-8%","#3b82f6","#eff6ff"),("EAU","42 m3","+2%","#06b6d4","#ecfeff"),("GAZ","18.9 MWh","-12%","#f97316","#fff7ed"),("DJU","318 J","-5%","#8b5cf6","#f5f3ff")]
        for i2,(t2,v2,d2,c2,bg2) in enumerate(kpi2):
            kx2=0.6+i2*3.8
            ax.add_patch(FancyBboxPatch((kx2,6.8),3.4,1.5,boxstyle="round,pad=0.12",facecolor=bg2,edgecolor=c2,lw=1.5))
        ...
        mn=np.arange(12); d25=np.array([1250,1180,980,820,650,520,490,510,680,920,1100,1280])
        def ny(v): return 1.3+(v-400)/(1350-400)*4.8
        xs2=0.7+mn*0.75; y25=ny(d25)
        ax.fill_between(xs2,1.3,y25,alpha=0.08,color="#0d1b2a"); ax.plot(xs2,y25,"-",color="#0d1b2a",lw=2)
        ...
        for sz2,pc2 in zip([42,28,18,12],["#3b82f6","#00b4d8","#f97316","#22c55e"]):
            ax.add_patch(Wedge((12.6,2.3),0.9,0,sz2*3.6,facecolor=pc2,edgecolor="white",lw=2,alpha=0.85))
        ...
        try:
            ...
            m2=_re.search(r'src="data:image/png;base64,([^"]+)"',html_c)
            if m2:
                limg=ImageReader(io.BytesIO(base64.b64decode(m2.group(1))))
                lw2=175; lh2=lw2*148/650
                cv.drawImage(limg,36,PAGE_H-lh2-24,width=lw2,height=lh2)
        except: pass
        proj_nom = proj["nom"] or ""
        proj_loc = (proj.get("ville","")+" - "+proj.get("code_postal","")).strip(" -") if proj.get("ville") else ""
        ...
        def mk_table(data_r, cw=None):
            t=Table(data_r,colWidths=cw,repeatRows=1)
            t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),BLUE),("TEXTCOLOR",(0,0),(-1,0),colors.white),
                ...
                ("ALIGN",(0,0),(-1,-1),"CENTER"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("PADDING",(0,0),(-1,-1),4)]))
            return t
        ...
        for eq in equips:
            ...
            cols_d=[]
            for iv in ivs:
                dp=iv.get("date_prevue","")
                if not dp: continue
                yr=str(dp)[:4]
                if yr not in [str(annee),str(annee-1)]: continue
                ...
                for cr in crs_iv:
                    ...
                    cr["numero_bt"]=iv["numero"]; cr["date_planif"]=dp
                    cols_d.append(cr)
            cols_d.sort(key=lambda x:str(x.get("date_planif","")))
            eq["cols"]=cols_d[:12]
            if not eq["compteurs"] or not eq["cols"]: continue
            story_p.append(Paragraph("Equipement : "+str(eq["designation"]),title_s))
            ...
            cw_base=[2*cm,3*cm,2.5*cm,1.5*cm]
            cw_bt=[2.2*cm]*len(cols)
            # Relevés
            story_p.append(Paragraph("Releves",section_s))
            hdrs=["Type","Compteur","Localisation","Unite"]+[str(c3.get("numero_bt",""))+" "+str(c3.get("date_planif",""))[:10] for c3 in cols]
            rrows=[hdrs]
            for cpt in cpts:
                row=[(cpt.get("type_nom") or "—"),cpt["nom"],(cpt.get("localisation") or "—"),(cpt.get("unite") or "—")]
                for c3 in cols:
                    r3=next((r for r in c3.get("releves",[]) if r["compteur_id"]==cpt["id"]),None)
                    row.append(str(round(r3["valeur"],1)) if r3 and r3.get("valeur") is not None else "—")
                rrows.append(row)
            story_p.append(mk_table(rrows,cw_base+cw_bt))
            story_p.append(Spacer(1,6))
            # Conso Brutes
            if len(cols)>1:
                story_p.append(Paragraph("Consommations Brutes",section_s))
                hdrs2=["Type","Compteur","Localisation","Unite"]+[str(cols[i3].get("numero_bt",""))+" "+str(cols[i3].get("date_planif",""))[:10] for i3 in range(1,len(cols))]
                cbrows=[hdrs2]
                for cpt in cpts:
                    row=[cpt.get("type_nom","—"),cpt["nom"],cpt.get("localisation","—"),cpt.get("unite","—")]
                    for i3 in range(1,len(cols)):
                        rp3=next((r for r in cols[i3-1].get("releves",[]) if r["compteur_id"]==cpt["id"]),None)
                        rc3=next((r for r in cols[i3].get("releves",[]) if r["compteur_id"]==cpt["id"]),None)
                        if rp3 and rc3 and rp3.get("valeur") is not None and rc3.get("valeur") is not None:
                            row.append(str(round(rc3["valeur"]-rp3["valeur"],2)))
                        else: row.append("—")
                    cbrows.append(row)
                story_p.append(mk_table(cbrows,cw_base+[2.2*cm]*(len(cols)-1)))
                story_p.append(Spacer(1,6))
            # Conso Corrigées
            if len(cols)>1:
                story_p.append(Paragraph("Consommations Corrigees",section_s))
                hdrs3=["Type","Compteur","Localisation","Unite"]+[str(cols[i3-1].get("numero_bt",""))+" "+str(cols[i3-1].get("date_planif",""))[:10] for i3 in range(1,len(cols))]
                ccrows=[hdrs3]
                for cpt in cpts:
                    row=[cpt.get("type_nom","—"),cpt["nom"],cpt.get("localisation","—"),cpt.get("unite","—")]
                    for i3 in range(1,len(cols)):
                        rp3=next((r for r in cols[i3-1].get("releves",[]) if r["compteur_id"]==cpt["id"]),None)
                        rc3=next((r for r in cols[i3].get("releves",[]) if r["compteur_id"]==cpt["id"]),None)
                        dp3s=str(cols[i3-1].get("date_planif","")); dc3s=str(cols[i3].get("date_planif",""))
                        if rp3 and rc3 and rp3.get("valeur") is not None and rc3.get("valeur") is not None and dp3s and dc3s:
                            try:
                                dp3=datetime.strptime(dp3s[:10],"%Y-%m-%d"); dc3=datetime.strptime(dc3s[:10],"%Y-%m-%d")
                                nj=abs((dc3-dp3).days); njm=calendar.monthrange(dp3.year,dp3.month)[1]
                                row.append(str(round((rc3["valeur"]-rp3["valeur"])/nj*njm,2)) if nj>0 else "—")
                            except: row.append("—")
                        else: row.append("—")
                    ccrows.append(row)
                story_p.append(mk_table(ccrows,cw_base+[2.2*cm]*(len(cols)-1)))
            story_p.append(PageBreak())
        # Graphiques
        if graphs_b64:
            story_p.append(Paragraph("Graphiques",title_s))
            story_p.append(HRFlowable(width="100%",thickness=1,color=ACCENT,spaceAfter=6))
            for g2 in graphs_b64:
                if g2.get("titre"): story_p.append(Paragraph(str(g2["titre"]),section_s))
                try:
                    img_d=base64.b64decode(g2["image_b64"].split(",")[-1])
                    story_p.append(RLImage(io.BytesIO(img_d),width=24*cm,height=11*cm))
        ...
        if analyses:
            ...
            for a2 in analyses:
                if a2.get("texte"):
                    story_p.append(Paragraph(str(a2["type_analyse"]),section_s))
                    story_p.append(Paragraph(str(a2["texte"]),ParagraphStyle("N2",parent=styles2["Normal"],fontSize=9,spaceAfter=4)))
        ...
        if params:
            ...
            for ch3,items3 in groupby(params,key=lambda x:x["chapitre"]):
                ...
                if len(pr)>1:
                    tp=Table(pr,colWidths=[10*cm,10*cm],repeatRows=1)
                    tp.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),BLUE),("TEXTCOLOR",(0,0),(-1,0),colors.white),
        ...
        def on_pg(c4,doc):
            c4.saveState()
            c4.setFillColor(BLUE); c4.rect(0,PAGE_H-26,PAGE_W,26,fill=1,stroke=0)
            c4.setFillColor(colors.white); c4.setFont("Helvetica-Bold",9)
            c4.drawString(16,PAGE_H-17,"SUIVI ENERGETIQUE - "+proj_nom)
            c4.setFont("Helvetica",8); c4.drawRightString(PAGE_W-16,PAGE_H-17,"Annee "+str(annee))
            c4.setFillColor(colors.HexColor("#f8fafc")); c4.rect(0,0,PAGE_W,18,fill=1,stroke=0)
            c4.setFillColor(ACCENT); c4.rect(0,0,4,18,fill=1,stroke=0)
            c4.setFillColor(colors.HexColor("#64748b")); c4.setFont("Helvetica",7)
            c4.drawString(12,5,"SOCOM")
            c4.drawRightString(PAGE_W-12,5,"Page "+str(doc.page))
            c4.restoreState()
        ...
        if story_p:
            ...
            cv2=rl_canvas.Canvas(buf,pagesize=landscape(A4))
            cv2.setFont("Helvetica",12); cv2.drawString(100,PAGE_H/2,"Aucune donnee disponible")
            cv2.save()

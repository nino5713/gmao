...
def planning_me():
    ...
    # ═══════════════════════════════════════════════════════════════════
    intervention_ids = list({it.get("intervention_id") for it in items if it.get("intervention_id")})
    if intervention_ids:
        ...
        # Grouper les CRs par intervention
        crs_by_iv = {}
        for cr in all_crs:
            crs_by_iv.setdefault(cr["intervention_id"], []).append(cr)
        # Pour chaque CR, charger horaires (min/max) + intervenants
        for cr in all_crs:
            try:
                horaires = one(db.execute(
                    """SELECT MIN(NULLIF(heure_debut,'')) AS h_min, MAX(NULLIF(heure_fin,'')) AS h_max
            ...
            try:
                ...
                cr["technicien_ids"] = []
        # Retirer les items des interventions qui ont des CRs, et construire les pseudo-items CR
        ivs_with_crs = set(crs_by_iv.keys())
        # Indexer les items originaux par intervention_id pour récupérer equip_nom, projet_nom, etc.
        iv_meta = {}
        for it in items:
        ...
        # Supprimer les items dont l'intervention a au moins un CR
        items = [it for it in items if not (it.get("intervention_id") in ivs_with_crs)]
        # Ajouter les pseudo-items CR (seulement ceux où le technicien cible est intervenant)
        for iid, crs in crs_by_iv.items():
            meta = iv_meta.get(iid, {})
            for cr in crs:
                # Le tech voit le CR uniquement s'il y est intervenant
                ...
                items.append({
                    ...
                    "intervention_id": iid,
                    "creneau_id": None,
                    "date": (cr.get("date_intervention") or "")[:10],
                    "heure_debut": cr.get("heure_debut") or "",
                    "heure_fin": cr.get("heure_fin") or "",
                    "notes": "",
                    "numero": meta.get("numero") or "",      # numéro du bon parent (BC/BP)
                    "type": meta.get("type") or "",           # MAINTENANCE ou DEPANNAGE

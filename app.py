...
Déploiement :
  ...
  cp app.py index.html /var/www/gmao/
  cd /var/www/gmao
  python3 -m venv venv
  venv/bin/pip install flask flask-cors
  # Configurer systemd (voir README)
"""
...
def calc_fin_de_vie(date_install, duree_ans):
    try:
        ...
        return (d + timedelta(days=duree_ans*365)).strftime("%Y-%m-%d")
    except: return None
...
def criticite_equipement(equip_id, db):
    pieces_ = rows(db.execute("SELECT statut FROM pieces WHERE equipement_id=?", (equip_id,)))
    if any(p["statut"] == "A_REMPLACER" for p in pieces_): return "CRITIQUE"
    if any(p["statut"] == "A_SURVEILLER" for p in pieces_): return "ATTENTION"
    return "OK"

def gen_numero(db, prefix, table, col):
...
def check_auth():
    ...
    try:
        db = get_db()
        r  = one(db.execute("SELECT id,nom,email,role FROM utilisateurs WHERE id=? AND password=? AND actif=1", (uid, pw)))
        return r
    except: return None
    ...
    try:
        ...
        r  = one(db.execute("""
            SELECT i.numero, i.type, i.statut, e.designation,
                   p.nom AS projet, u.email AS manager_email, u.nom AS manager_nom
        ...
        if not r or not r["manager_email"]: return
        send_mail(r["manager_email"],
            f"[GMAO] {action} — {r['numero']} — {r['designation']}",
            f"Bonjour {r['manager_nom']},\n\n{action}\n\nIntervention : {r['numero']}\nType : {r['type']}\nStatut : {r['statut']}\nÉquipement : {r['designation']}\nProjet : {r['projet']}\n\nCordialement,\nSOCOM GMAO")
    except Exception as e:
        print(f"[NOTIFY] {e}")
...
def login():
    ...
    if not email or not pwd: return jsonify({"error":"Identifiants manquants"}),400
    try:
        db = get_db()
        r  = one(db.execute(
...
def get_clients():
    db = get_db()
    return jsonify(rows(db.execute("SELECT * FROM clients ORDER BY societe")))
...
def get_projets():
    db = get_db()
    return jsonify(rows(db.execute("""
        SELECT p.*, c.societe AS client_nom,
...
def get_projet(pid):
    ...
    # Calcul criticité pour chaque équipement
...
def create_projet():
    ...
    # Vérifier manager
    if d.get("manager_id"):
        m = one(db.execute("SELECT role FROM utilisateurs WHERE id=?", (d["manager_id"],)))
        if not m or m["role"] not in ("manager","admin"):
            return jsonify({"error":"Le manager doit avoir le rôle manager ou admin"}),400
    num = d.get("numero_projet") or gen_numero(db,"PRJ","projets","numero_projet")
    try:
        db.execute("""INSERT INTO projets (numero_projet,nom,client_id,manager_id,description,date_debut,date_fin,statut)
            VALUES (?,?,?,?,?,?,?,?)""",
...
def get_equipements():
    ...
    sql = """
        ...
        WHERE 1=1"""
    params = []
    if pid: sql += " AND e.projet_id=?"; params.append(pid)
...
def get_pieces():
    db  = get_db()
    eid = request.args.get("equipement_id")
    sql = """
        SELECT p.*, e.designation AS equip_nom, e.projet_id,
               pj.nom AS projet_nom
        FROM pieces p
        JOIN equipements e ON p.equipement_id=e.id
        JOIN projets pj ON e.projet_id=pj.id
        WHERE 1=1"""
    params=[]
    if eid: sql += " AND p.equipement_id=?"; params.append(eid)
    # Alertes seulement
...
def create_piece():
    ...
    if d.get("date_installation") and d.get("duree_vie_estimee"):
        fdv = calc_fin_de_vie(d["date_installation"], int(d["duree_vie_estimee"]))
...
def update_piece(pid):
    d = request.json or {}
    db = get_db()
    # Recalcul fin de vie si nécessaire
...
def create_equipe():
    ...
    for mid in d.get("membres",[]):
        try: db.execute("INSERT OR IGNORE INTO equipe_membres (equipe_id,technicien_id) VALUES (?,?)", (eid,mid))
        except: pass
    db.commit()
...
def get_interventions():
    ...
    user   = request.user
    sql    = """
        SELECT i.*, e.designation AS equip_nom, e.projet_id,
        ...
        WHERE 1=1"""
    params = []
    # Technicien : voir seulement les siennes
    if user["role"] == "technicien" or request.args.get("technicien") == "me":
...
def get_intervention(iid):
    ...
    if cr:
        cr["intervenants"] = rows(db.execute("""
            SELECT ci.*, u.nom FROM cr_intervenants ci
            JOIN utilisateurs u ON ci.utilisateur_id=u.id
...
def update_intervention(iid):
    ...
    for f in ["technicien_id","type","statut","date_prevue","date_realisation","description","rapport"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f] or None if f in ["technicien_id"] else d[f])
...
def create_or_update_cr(iid):
    d  = request.json or {}
    db = get_db()
    existing = one(db.execute("SELECT id FROM comptes_rendus WHERE intervention_id=?", (iid,)))
    ...
    if existing:
        ...
        db.execute("""INSERT INTO comptes_rendus
            (intervention_id,date_intervention,observations,actions_realisees,mesures,recommandations,conclusion)
            VALUES (?,?,?,?,?,?,?)""",
    ...
    if "intervenants" in d:
        ...
        for iv in d["intervenants"]:
            ...
            uid = iv.get("utilisateur_id") or None
            nom = iv.get("nom","")
            db.execute("""INSERT INTO cr_intervenants
                (cr_id,utilisateur_id,date,heure_debut,heure_fin,total_heures)
                VALUES (?,?,?,?,?,?)""",
                (cr_id, uid, iv.get("date",today()),
                 iv.get("heure_debut",""), iv.get("heure_fin",""), round(h,2)))
            total += h
        db.execute("UPDATE comptes_rendus SET total_heures=? WHERE id=?", (round(total,2), cr_id))

    # Photos (stockées en JSON base64)
    if "photos" in d and d["photos"]:
        import json as _json
        db.execute("UPDATE comptes_rendus SET photos=? WHERE id=?",
                   (_json.dumps(d["photos"]), cr_id))
...
def dashboard():
    ...
    kpi = one(db.execute("""
        SELECT
          COUNT(*) AS total,
          SUM(CASE WHEN statut='A_PLANIFIER' THEN 1 ELSE 0 END) AS a_planifier,
          ...
          SUM(CASE WHEN type='DEPANNAGE' THEN 1 ELSE 0 END) AS depannage
        FROM interventions WHERE statut NOT IN ('TERMINEE','ANNULEE')"""))

    # Interventions du jour
    today_interv = rows(db.execute("""
        SELECT i.numero,i.type,i.statut,e.designation,p.nom AS projet
        FROM interventions i
        JOIN equipements e ON i.equipement_id=e.id
        JOIN projets p ON e.projet_id=p.id
        WHERE DATE(i.date_prevue)=? AND i.statut NOT IN ('TERMINEE','ANNULEE')
        ORDER BY i.date_prevue""", (today(),)))

    # Retards
    retards = rows(db.execute("""
        SELECT i.numero,i.type,i.date_prevue,e.designation,p.nom AS projet
        FROM interventions i
        JOIN equipements e ON i.equipement_id=e.id
        JOIN projets p ON e.projet_id=p.id
        WHERE i.date_prevue < ? AND i.statut NOT IN ('TERMINEE','ANNULEE')
        ORDER BY i.date_prevue""", (today(),)))
    ...
    # Stats délai
    dep = rows(db.execute("""
        SELECT JULIANDAY(date_realisation)-JULIANDAY(date_creation) AS delta
        FROM interventions
        WHERE type='DEPANNAGE' AND statut='TERMINEE'
          AND date_realisation IS NOT NULL"""))
    mnt = rows(db.execute("""
        SELECT JULIANDAY(date_realisation)-JULIANDAY(date_prevue) AS delta
        FROM interventions
        WHERE type='MAINTENANCE' AND statut='TERMINEE'
          AND date_realisation IS NOT NULL AND date_prevue IS NOT NULL"""))
    ...
    return jsonify({
        ...
        "stats": {
            "moy_jours_depannage":   moy_dep,
            "nb_depannages":         len(dep),
            "moy_jours_maintenance": moy_mnt,
            "nb_maintenances":       len(mnt)
        }
    })
...
def get_smtp():
    db = get_db()
    r  = one(db.execute("SELECT host,port,username,use_tls,sender_email,sender_name,enabled FROM smtp_config WHERE id=1"))
    return jsonify(r or {})
...
def save_smtp():
    ...
    db.execute("""UPDATE smtp_config SET
        host=?,port=?,username=?,password=?,use_tls=?,sender_email=?,sender_name=?,enabled=?
        WHERE id=1""",
        (d.get("host",""),int(d.get("port",587)),d.get("username",""),
         d.get("password",""),1 if d.get("use_tls",True) else 0,
...
def test_smtp():
    ...
    ok = send_mail(to,"[GMAO] Test SMTP","Test de configuration SMTP — OK")
    return jsonify({"ok":ok,"error":None if ok else "Échec envoi"})

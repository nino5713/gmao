...
def _find_next_available_date(db, start_date_str, type_iv, sid, planning_id=None,
    ...
    while iters < max_lookahead_days:
        ...
        if planning_id and nb_simultane >= 1:
            ...
            # On ne compte pas le total des chevauchements, mais le maximum de bons SIMULTANÉMENT en cours.
            # Algorithme : créer des "events" (+1 au début, -1 à la fin), trier, scanner.
            events = []  # (time_min, delta)
            ...
            for b in day_bons:
                ...
                be_min = bs_min + bd
                # Ne considérer que les bons dans la fenêtre [new_start, new_end]
                # (sinon ils ne peuvent pas saturer le nouveau bon)
                # En fait on prend TOUS pour avoir un comptage complet du pic
                if be_min <= new_start_min or bs_min >= new_end_min:
                    continue  # bon hors fenêtre → ignoré
                events.append((bs_min, +1))
                events.append((be_min, -1))
            # Trier : à temps égal, traiter les -1 avant les +1
            # (un bon qui finit à 09:00 libère, puis un autre qui commence à 09:00)
            ...
            # peak inclut le nouveau bon ; si peak > nb_simultane → conflit

...
def declencher_astreinte():
    ...
    Body :
      - equipement_id (requis)
      - description (requis)
      - demandeur_nom, demandeur_tel, demandeur_email
      - criticite (BLOQUANT, CRITIQUE, MINEUR — default CRITIQUE)
      - heure_appel, heure_arrivee, heure_fin (HH:MM)
      - kilometrage (float)
      - specialite_id (id de astreinte_specialites)
      - technicien_astreinte (texte libre, parfois nom hors utilisateurs)
      - technicien_id (utilisateur GMAO si correspondance)
    """
    d = request.json or {}
    if not d.get("equipement_id"):
        return jsonify({"error": "equipement_id requis"}), 400
    ...
    try:
        ...
        cur = db.execute("""
            ...
            VALUES (?, ?, ?, 'ASTREINTE', 'EN_COURS', ?, ?, ?)
        """, (num, d["equipement_id"], tech_id, date_prevue, heure_prevue, d["description"]))
        iid = cur.lastrowid
        # Si technicien GMAO, l'enregistrer comme intervenant
        if tech_id:
            try:
                db.execute("INSERT OR IGNORE INTO intervention_techniciens (intervention_id, utilisateur_id) VALUES (?, ?)",
                           (iid, tech_id))
            except Exception: pass
        # 2) Créer l'astreinte_declenchement lié
        db.execute("""
            INSERT INTO astreinte_declenchements
              (intervention_id, demandeur_nom, demandeur_tel, demandeur_email,
               criticite, heure_appel, heure_arrivee, heure_fin, kilometrage,
               specialite_id, technicien_astreinte, declenche_par)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            iid,
            ...
            (d.get("heure_arrivee") or "").strip(),
            (d.get("heure_fin") or "").strip(),
            float(d.get("kilometrage") or 0),
            to_int(d.get("specialite_id")),
            (d.get("technicien_astreinte") or "").strip(),
            request.user.get("id"),
        ))
        db.commit()
        log_action(request.user, "CREATE", "astreinte_declenchement", iid,
                   f"BA {num} - {d.get('demandeur_nom','')[:40]}")
        return jsonify({"ok": True, "intervention_id": iid, "numero": num})
    except Exception as e:
        db.rollback()

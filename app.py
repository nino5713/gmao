...
def init_db():
    ...
    -- ASTREINTES BIS : tableau éditable (1 ligne par jour, 1 col par spécialité)
    -- Réutilise astreinte_specialites + techniciens_astreinte
    CREATE TABLE IF NOT EXISTS astreinte_bis_planning (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        date TEXT NOT NULL,
        specialite_id INTEGER NOT NULL REFERENCES astreinte_specialites(id) ON DELETE CASCADE,
        technicien TEXT DEFAULT '',
        created_at TEXT DEFAULT (datetime('now')),
        updated_at TEXT DEFAULT (datetime('now')),
        UNIQUE(date, specialite_id)
    );
    CREATE INDEX IF NOT EXISTS idx_astreinte_bis_planning_date ON astreinte_bis_planning(date);
    ...
    # Cleanup : suppression des tables astreinte_bis v1 (groupes/rotations) — remplacé par tableau éditable
    try:
        db = get_db()
        ...
        # Cleanup : entrées polluées par d'anciens imports (dates dans la colonne technicien)
        # On utilise LIKE avec patterns variés pour attraper tous les formats datetime
        for _patt in ('% 00:00:00', '%-__-__ %', '%-__-__'):
            try:
                db.execute("DELETE FROM astreinte_planning WHERE technicien LIKE ?", (_patt,))
            except Exception: pass
            try:
                db.execute("DELETE FROM astreinte_bis_planning WHERE technicien LIKE ?", (_patt,))
            except Exception: pass
        # Filtre supplémentaire en Python pour les cas atypiques (regex date YYYY-MM-DD)
        try:
            import re as _re
            for tbl in ('astreinte_planning', 'astreinte_bis_planning'):
                pollues = rows(db.execute(f"SELECT id, technicien FROM {tbl} WHERE technicien != ''"))
                for r in pollues:
                    v = (r["technicien"] or "").strip()
                    if _re.match(r'^\d{4}-\d{1,2}-\d{1,2}', v):
                        db.execute(f"DELETE FROM {tbl} WHERE id=?", (r["id"],))
        except Exception: pass
        db.commit()
...
# ══════════════════════════════════════════════════════════════════════
# ASTREINTES BIS — Tableau éditable (réutilise specialites + techs v1)
# ══════════════════════════════════════════════════════════════════════
@app.route("/api/astreintes_bis/planning", methods=["GET"])
@require_auth
def ab_get_planning():
    """Récupère le planning sur une plage de dates.
    Query params: date_debut, date_fin (incluses, format YYYY-MM-DD)"""
    db = get_db()
    ...
    return jsonify(rows(db.execute("""
        SELECT id, date, specialite_id, technicien
        FROM astreinte_bis_planning
        WHERE date >= ? AND date <= ?
        ORDER BY date, specialite_id
    """, (dd, df))))

@app.route("/api/astreintes_bis/planning", methods=["POST"])
@require_role("admin","manager")
def ab_set_planning():
    """Définit (insert ou update) une cellule du planning.
    Body: {date, specialite_id, technicien}"""
    d = request.json or {}
    date = d.get("date")
    sid = int(d.get("specialite_id") or 0)
    tech = (d.get("technicien") or "").strip()
    if not date or not sid:
        return jsonify({"error":"date et specialite_id requis"}), 400
    db = get_db()
    # UPSERT
    existing = one(db.execute(
        "SELECT id FROM astreinte_bis_planning WHERE date=? AND specialite_id=?",
        (date, sid)
    ))
    if existing:
        if tech:
            db.execute("UPDATE astreinte_bis_planning SET technicien=?, updated_at=datetime('now') WHERE id=?",
                       (tech, existing["id"]))
        else:
            # Si tech vide, on supprime la ligne (cellule vide)
            db.execute("DELETE FROM astreinte_bis_planning WHERE id=?", (existing["id"],))
    else:
        if tech:
            db.execute("INSERT INTO astreinte_bis_planning (date, specialite_id, technicien) VALUES (?,?,?)",
                       (date, sid, tech))
    db.commit()
    return jsonify({"ok": True})

@app.route("/api/astreintes_bis/planning/today", methods=["GET"])
@require_auth
def ab_planning_today():
    """Retourne l'astreinte en cours pour aujourd'hui."""
    from datetime import date as dt_date
    today = dt_date.today().strftime("%Y-%m-%d")
    db = get_db()
    return jsonify(rows(db.execute("""
        SELECT p.date, p.technicien, s.id AS specialite_id, s.nom AS specialite_nom
        FROM astreinte_bis_planning p
        JOIN astreinte_specialites s ON p.specialite_id = s.id
        WHERE p.date = ?

...
def _find_next_available_date(db, start_date_str, type_iv, sid, planning_id=None,
    ...
    # v218.137 : force_planif court-circuit
    if force_planif:
        return (start_date_str, False)
    try:
    ...
    while iters < max_lookahead_days:
        ...
        # Vérifier weekend (5=samedi, 6=dimanche) si demandé
...
def find_next_slot():
    ...
    if eq_id and gamme_id:
        ...
        if eg and eg.get("force_planif"):
            force_planif = True
    # v218.137 : si la date demandée tombe sur samedi/dimanche, c'est qu'elle est délibérée
    # → ne pas décaler le weekend (mais on décale toujours pour les fériés)
    skip_weekend = True
    try:

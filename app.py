...
def _bloc_equipement_data(equipement_id, intervention_id, db):
    ...
    custom_fields = {}
    try:
        # Récupérer la société de l'équipement
        eq_sid = one(db.execute("SELECT societe_id FROM equipements WHERE id=?", (equipement_id,)))
        sid_eq = eq_sid.get("societe_id") if eq_sid else current_societe_id()
        # Charger les définitions de champs pour la société (et de la technique de l'équipement)
        tech_id_row = one(db.execute("SELECT technique_id FROM equipements WHERE id=?", (equipement_id,)))
        tech_id = tech_id_row.get("technique_id") if tech_id_row else None

...
ALLOWED_UPDATE_COLS = {
    ...
    "interventions":         {"equipement_id","technicien_id","equipe_id","type","statut",
                              "date_prevue","date_realisation","description","rapport","heure_prevue","numero_offre"},
...
def create_intervention():
    ...
    db.execute("INSERT INTO interventions (numero,equipement_id,technicien_id,equipe_id,type,statut,date_prevue,description,numero_offre) VALUES (?,?,?,?,?,?,?,?,?)",
               (num,d["equipement_id"],to_int(d.get("technicien_id")),to_int(d.get("equipe_id")),
                type_iv,d.get("statut","PLANIFIEE"),d.get("date_prevue") or None,d.get("description",""),numero_offre))
...
def update_intervention(iid):
    d=request.json or {}; db=get_db(); sets,params=[],[]
    # Si le type est changé en OFFRE ou si numero_offre est fourni, valider
    if "numero_offre" in d or d.get("type") == "OFFRE":
    ...
    for f in ["technicien_id","equipe_id","type","statut","date_prevue","heure_prevue","date_realisation","description","rapport","numero_offre"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
...
def pointage_export(annee, mois):
    ...
    ws = wb.active
    ws.title = "Détail"
    headers = ["Technicien", "Date", "Début", "Fin", "Total h", "N° Bon", "Type", "Équipement", "Projet"]
    ...
    for idx, l in enumerate(lignes, 2):
        ...
        for ci, v in enumerate(row_data, 1):
            ...
            c.border = border
    # Largeurs
    widths = [22, 12, 8, 8, 10, 14, 14, 30, 25]

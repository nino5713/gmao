...
def pointage_export(annee, mois):
    ...
    # v218.166 : ajouter les jours fériés du mois pour chaque technicien actif
    # 8h par férié, catégorie "Férié"
    feries = rows(db.execute(
        "SELECT date, libelle FROM jours_feries WHERE societe_id=? AND date BETWEEN ? AND ? ORDER BY date",
        [sid, date_debut, date_fin]
    ))

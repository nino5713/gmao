...
def init_db():
    ...
    # v206 : type de module (tableau / texte / checklist)
...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist"):
        return jsonify({"error": "type invalide (tableau, texte ou checklist)"}), 400
    ordre = int(d.get("ordre") or 0)
    db = get_db()
    db.execute("INSERT INTO mesure_blocs (technique_id, sous_type_id, nom, type, ordre) VALUES (?, ?, ?, ?, ?)",
               (technique_id, sous_type_id, nom, type_module, ordre))
    bid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
...
def get_intervention_mesures(iid):
    ...
    blocs = rows(db.execute("""
        SELECT mb.id, mb.nom, mb.ordre, mb.type FROM mesure_blocs mb
...
def _intervention_mesures_data(intervention_id):
    ...
    if not st:
        return []
    blocs = rows(db.execute("""
        SELECT id, nom, type FROM mesure_blocs
        WHERE technique_id=? AND sous_type_id=?
        ORDER BY ordre, nom
    """, (technique_id, st["id"])))
    if not blocs:

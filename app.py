            ...
            if uid:
                ...
                if u and hp(f"{uid}:{u['email']}:{u['password']}") == parts[1]:
                    ...
                    try:
                        ...
                        u["societe_role"] = (r and r["role"]) or u.get("role")
                    except Exception:
                        u["societe_id"] = 1
                        u["societe_role"] = u.get("role")
                    return u
...
def login():
    ...
    societes_dispo = rows(db.execute("""
        SELECT us.societe_id, us.role, s.nom, s.code, s.logo_path, s.couleur_primaire
    ...
    logger.info(f"[LOGIN] succes uid={u['id']} role={societe_role} societe={target_sid} depuis {request.remote_addr}")
    ...
    return jsonify({
        ...
        "user": {
            ...
            "nom": u["nom"],
            "email": u["email"],
            "role": societe_role,  # rôle dans la société active (peut différer entre sociétés)
...
def switch_societe():
    ...
    db = get_db()
    access = one(db.execute("""
        SELECT us.role, s.nom, s.code, s.logo_path, s.couleur_primaire
        FROM utilisateur_societes us
        JOIN societes s ON s.id = us.societe_id
    ...
    return jsonify({
        ...
        "user": {
            ...
            "role": access["role"], "societe_id": target_sid,
        },
        "societe": {
            "id": target_sid,
            "nom": access["nom"],
...
def create_societe():
    ...
    db.execute("""
        INSERT INTO societes (nom, code, couleur_primaire, adresse, telephone, email, site_web, config_json, actif)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)
    """, (
        nom, code,
    ...
    logger.info(f"[SUPERADMIN] Société créée : id={new_id} nom='{nom}' code='{code}'")
...
def create_utilisateur():
    ...
    try:
        ...
        db.commit()
        new_id = db.execute("SELECT last_insert_rowid()").fetchone()[0]
        log_action(request.user, "CREATE", "utilisateur", new_id, d["nom"], f"role={role_demande}")
        return jsonify({"id":new_id}),201
    except Exception as e: return jsonify({"error":str(e)}),400

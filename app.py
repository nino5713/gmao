...
def get_equipements():
    db   = get_db()
    user = request.user
    sql  = """SELECT e.*,p.nom AS projet_nom,p.numero_projet,g.nom AS gamme_nom
              FROM equipements e JOIN projets p ON e.projet_id=p.id
              LEFT JOIN gammes g ON e.gamme_id=g.id WHERE 1=1"""
    params = []
...
def get_equipement(eid):
    db = get_db()
    e  = one(db.execute("SELECT e.*,p.nom AS projet_nom,p.numero_projet,g.nom AS gamme_nom FROM equipements e JOIN projets p ON e.projet_id=p.id LEFT JOIN gammes g ON e.gamme_id=g.id WHERE e.id=?",(eid,)))
...
def update_equipement(eid):
    ...
    db = get_db()
    sets,params=[],[]
    for f in ["designation","type_technique","localisation","marque","modele","puissance","numero_serie","in_out","date_mise_en_service","statut","technique","notes"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
    if not sets: return jsonify({"error":"Rien"}),400
    params.append(eid)

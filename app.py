...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations", "mesures_batteries", "image", "equipement"):
        return jsonify({"error": "type invalide"}), 400
    ordre = int(d.get("ordre") or 0)
...
def _bloc_equipement_data(equipement_id, intervention_id, db):
    ...
    Retourne un dict avec toutes les valeurs résolues (designation, marque_modele, etc.)."""
...
def generate_pdf(iid):
    ...
    try:
        ...
        try:
            ...
            _cover_row = one(db.execute("""SELECT blocks_json, header_blocks_json, footer_blocks_json
                                           FROM cover_pages WHERE societe_id=?""", (_iv_sid,)))
            if _cover_row:

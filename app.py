...
def create_occupation():
    ...
    db.execute("""INSERT INTO occupations
        (technicien_id, type_id, date, heure_debut, heure_fin, total_heures, notes, numero_projet)
        VALUES (?,?,?,?,?,?,?,?)""",
        (technicien_id, type_id, date, heure_debut, heure_fin,
         total_heures, d.get("notes",""), numero_projet))
    db.commit()
    return jsonify({"id": db.execute("SELECT last_insert_rowid()").fetchone()[0]}), 201
...
def update_occupation(oid):
    ...
    if final_type_id:
        ...
        if t and (t.get("nom") or "").strip().lower() == "offre":
            is_offre = True
    # Valider / normaliser numero_projet
    if is_offre:
        ...
        # Type non-Offre : on vide le numero_projet
        d["numero_projet"] = ""
    sets, params = [], []
    for f in ["type_id", "date", "heure_debut", "heure_fin", "notes", "numero_projet"]:
        if f in d:
            sets.append(f"{f}=?")
...
def pointage_export(annee, mois):
    ...
    occupations = rows(db.execute("""
        SELECT o.date, o.heure_debut, o.heure_fin, o.total_heures,
               COALESCE(u.nom, 'Inconnu') AS nom_tech,
               o.notes, o.numero_projet AS num_projet_occ, ot.nom AS type_occupation
        FROM occupations o
        LEFT JOIN utilisateurs u ON o.technicien_id = u.id
        LEFT JOIN occupation_types ot ON o.type_id = ot.id
        WHERE o.date BETWEEN ? AND ?
...
def planning_me():
    ...
    occs = rows(db.execute("""
        SELECT o.id AS occupation_id, o.date, o.heure_debut, o.heure_fin, o.notes,
               ...
               o.total_heures, o.numero_projet
        FROM occupations o
        LEFT JOIN occupation_types ot ON o.type_id = ot.id

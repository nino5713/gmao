...
ALLOWED_UPDATE_COLS = {
    ...
    "utilisateurs":          {"nom","email","role","actif","password","token_version","matricule","manager_id","techniques"},
...
def create_utilisateur():
    ...
    try:
        db.execute("INSERT INTO utilisateurs (nom,email,password,role,matricule,manager_id,techniques) VALUES (?,?,?,?,?,?,?)",
                   (d["nom"],d["email"].strip().lower(),hash_password(d["password"]),d.get("role","technicien"),matricule,manager_id,techniques_csv))
        db.commit()
        ...
        if raw is None or raw == "":
            ...
            try:
                ...
                try:
                    ...
                    d["accompagnants_ids"] = ""
    sets, params = [], []
    for f in ["type_id", "date", "heure_debut", "heure_fin", "notes", "numero_projet", "nom_chantier", "accompagnants_ids"]:
        if f in d:
            sets.append(f"{f}=?")

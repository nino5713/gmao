...
def get_dashboard_heures():
    ...
    # BC (dépannage) : somme total_heures des intervenants CR des bons DEPANNAGE
    row_bc = one(db.execute("""
    ...
    # BP (maintenance) : somme total_heures des intervenants CR des bons MAINTENANCE
    row_bp = one(db.execute("""
        SELECT COALESCE(SUM(ci.total_heures),0) AS h
        FROM cr_intervenants ci
        JOIN comptes_rendus cr ON ci.cr_id=cr.id
        JOIN interventions i ON cr.intervention_id=i.id
    ...
    # Occupations : somme par type
    occs = rows(db.execute("""
        SELECT ot.nom AS type_nom,
               ot.couleur AS type_couleur,
               COALESCE(SUM(
                 CASE
                   WHEN o.heure_debut IS NOT NULL AND o.heure_fin IS NOT NULL
                        AND o.heure_debut != '' AND o.heure_fin != ''
                   THEN (
                     CAST(substr(o.heure_fin,1,2) AS REAL) * 60 + CAST(substr(o.heure_fin,4,2) AS REAL)
                     - CAST(substr(o.heure_debut,1,2) AS REAL) * 60 - CAST(substr(o.heure_debut,4,2) AS REAL)
                   ) / 60.0
                   ELSE 0
                 END
               ),0) AS h
        FROM occupations o
        JOIN occupation_types ot ON o.type_occupation_id=ot.id
    ...
    # Gestion du passage minuit (résultat négatif → +24h)
    for o in occs:
        h = float(o.get("h") or 0)
        if h < 0: h += 24
        o["h"] = round(h, 2)

    ...
    try:
        ...
        iv = one(db.execute(
            """SELECT i.id, i.numero, i.type, i.date_prevue, i.heure_prevue, i.technicien_id,
                      e.designation AS equip_nom, p.nom AS projet_nom
        ...
        if not iv: return
        # Collecter les techs concernés : technicien_id + intervention_techniciens + extras
        user_ids = set()
        ...
        # Ajouter les techs des créneaux (même s'ils ne sont pas dans intervention_techniciens)
        try:
        ...
        # Ajouter les extras (utile pour suppression de créneau où le tech n'est plus lié)
        if extra_user_ids:
            for uid in extra_user_ids:
                if uid: user_ids.add(uid)
        if not user_ids: return
        # Construire le texte
        type_label = "Maintenance" if iv.get("type") == "MAINTENANCE" else "Dépannage"
        titles = {
            "assigned": f"Nouveau bon {type_label}",
            "modified": f"{type_label} modifié",
            "removed": f"Retiré d'un {type_label.lower()}",
        }
        date_str = ""
        if iv.get("date_prevue"):
            try:
                d_part = iv["date_prevue"][:10]
                parts = d_part.split("-")
                if len(parts) == 3:
                    date_str = f"{parts[2]}/{parts[1]}/{parts[0]}"
                    if iv.get("heure_prevue"):
                        date_str += f" à {iv['heure_prevue']}"
            except Exception: pass
        body_parts = [iv.get("numero") or ""]
        if iv.get("equip_nom"): body_parts.append(iv["equip_nom"])
        if iv.get("projet_nom"): body_parts.append(iv["projet_nom"])
        if date_str: body_parts.append(date_str)
        if details: body_parts.append(details)
        payload = {
            "title": titles.get(event_type, "Intervention mise à jour"),
...
def _notify_occupation(occ_id, event_type, details="", extra_user_ids=None, explicit_user_ids=None):
    ...
    try:
        ...
        if not explicit_user_ids:
            occ = one(db.execute(
                """SELECT o.id, o.technicien_id, o.date, o.heure_debut, o.heure_fin, o.accompagnants_ids,
        ...
        if not user_ids: return
        # Construire le titre
        type_nom = (occ or {}).get("type_nom") or "Occupation"
        titles = {
            "assigned": f"Nouvelle {type_nom.lower()}",
            "modified": f"{type_nom} modifiée",
            "removed": f"{type_nom} supprimée",
        }
        # Construire le body
        date_str = ""
        if (occ or {}).get("date"):
            try:
                ...
                if len(parts) == 3:
                    ...
                    if occ.get("heure_debut") and occ.get("heure_fin"):
                        date_str += f" {occ['heure_debut']}→{occ['heure_fin']}"
                    elif occ.get("heure_debut"):
                        date_str += f" à {occ['heure_debut']}"
            except Exception: pass
        body_parts = []
        if date_str: body_parts.append(date_str)
        if details: body_parts.append(details)
        payload = {
            "title": titles.get(event_type, "Occupation mise à jour"),
            "body": " · ".join([p for p in body_parts if p]) or type_nom,
            "icon": "/icons/socom-icon-192.png",
            "url": "/mobile",
            "tag": f"occ-{occ_id}"
        }
...
def delete_occupation(oid):
    ...
    # Notification (ligne déjà supprimée → passer explicit_user_ids)
    try:
        ot_nom = (ot_info or {}).get("nom", "Occupation")
        # On envoie un payload simple, sans lire en base
        if notify_users:
            send_push_to_users(list(notify_users), {
                "title": f"{ot_nom} supprimée",
                "body": f"Le {cur.get('date','')}" + (f" ({cur.get('heure_debut','')}→{cur.get('heure_fin','')})" if cur.get('heure_debut') else ""),
                "icon": "/icons/socom-icon-192.png",
                "url": "/mobile",
                "tag": f"occ-{oid}"
            })

...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations", "mesures_batteries", "image", "equipement", "image_checklist", "mesures_string"):
        return jsonify({"error": "type invalide"}), 400
    ordre = int(d.get("ordre") or 0)
...
def _bloc_equipement_data(equipement_id, intervention_id, db):
    ...
    try:
        sous_equipements_list = [dict(r) for r in rows(db.execute(
            "SELECT designation, marque, type, nombre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id",
            (equipement_id,)
        ))]
    except Exception:
        sous_equipements_list = []

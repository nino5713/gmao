    ...
    try:
        ...
        if tech_astreinte_nom:
            user_match = one(db.execute(
                "SELECT id FROM utilisateurs WHERE role='technicien' AND nom=? AND actif=1 LIMIT 1",
                (tech_astreinte_nom,)
            ))
            if user_match:
                tech_id = user_match["id"]
            else:
                # Tentative en LIKE (insensible à la casse, espaces)
                user_match = one(db.execute(
                    "SELECT id FROM utilisateurs WHERE role='technicien' AND LOWER(nom) LIKE ? AND actif=1 LIMIT 1",
...
def update_intervention(iid):
    ...
    if d.get("statut") == "EN_COURS" and request.user.get("role") == "technicien":
        ...
        if iv_curr and iv_curr.get("type") == "ASTREINTE" and iv_curr.get("statut") == "PLANIFIEE":
            ...
            logger.info(f"[ASTREINTE] BA iid={iid} pris en charge par tech_id={request.user['id']} ({request.user.get('nom','')})")

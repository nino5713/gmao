    ...
    try:
        ...
        if r["statut"]=="TERMINEE":
            try:
                ...
                tech_nom = db.execute("SELECT nom FROM utilisateurs WHERE id=?", (r.get("technicien_id"),)).fetchone()
                tech_nom = tech_nom[0] if tech_nom else "--"
                all_ivs = []
                for cr in crs:
...
def get_dashboard():
    ...
    sql_today = f"""SELECT i.*,e.designation AS equip_nom,e.type_technique,
                    ...
                    AND date(i.date_prevue)=?{role_filter}
                    ORDER BY i.date_prevue"""
    today_ivs = rows(db.execute(sql_today, [td] + role_params))
    ...
    sql_retards = f"""SELECT i.*,e.designation AS equip_nom,e.type_technique,
                    ...
                    LEFT JOIN utilisateurs u2 ON i.technicien_id=u2.id
                    WHERE i.statut NOT IN ('TERMINEE','ANNULEE')
                    AND i.date_prevue IS NOT NULL AND date(i.date_prevue) < ?{role_filter}
                    ORDER BY i.date_prevue"""
    retards = rows(db.execute(sql_retards, [td] + role_params))
...
def generate_pdf(iid):
    ...
    def mf(field): return "\n".join([cr.get(field,"") for cr in crs if cr.get(field,"")])
    # Construire liste intervenants depuis technicien + CR
    all_ivs = []
    if i.get("technicien_nom"):
        for cr in crs:
            all_ivs.append({

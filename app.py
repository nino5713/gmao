...
def list_equipe_techniciens():
    ...
    # v218.28 : on prend les techniciens actifs uniquement
    where = "WHERE u.role='technicien' AND COALESCE(u.actif,1)=1"
    params = []
    if role == "technicien":
        where += " AND u.id=?"
    ...
    techs = rows(db.execute(f"""
        ...
        ORDER BY u.nom
    """, params))
    logger.info(f"[EQUIPE] user={user.get('id')} role={role} → {len(techs)} techniciens trouvés")
...
def get_technicien_stats(tech_id):
    ...
    try:
        occupations_par_type = rows(db.execute("""
            ...
            FROM occupations o
            JOIN occupation_types ot ON o.type_id=ot.id
            WHERE o.technicien_id=? AND o.date>=? AND o.date<=?
            GROUP BY ot.id, ot.nom, ot.couleur
    ...
    # v218.40 : Score QSE — sommes des % obtenus aux causeries de l'année
    # Règle : 80% = 80pts. Total = somme des % de chaque causerie répondue.
...
def list_causeries():
    ...
    if role == "admin":
        ...
        if annee_str:
            try:
                annee = int(annee_str)
                where = """ WHERE (c.planif_annee=? OR (c.planif_annee=0 AND substr(c.date_publication,1,4)=?))"""
...
def get_mon_profil():
    ...
    # Heures BC/BP totales
    heures = one(db.execute("""
        SELECT
          COALESCE(SUM(CASE WHEN i.type='MAINTENANCE' THEN ci.heures_total ELSE 0 END), 0) AS h_bc,
          COALESCE(SUM(CASE WHEN i.type IN ('DEPANNAGE','ASTREINTE') THEN ci.heures_total ELSE 0 END), 0) AS h_bp,
          COALESCE(SUM(ci.heures_total), 0) AS h_total
        FROM cr_intervenants ci
        JOIN comptes_rendus cr ON ci.cr_id=cr.id
        JOIN interventions i ON cr.intervention_id=i.id
        WHERE ci.utilisateur_id=?
          AND cr.date_debut >= ? AND cr.date_debut <= ?
    """, (tech_id, date_debut, date_fin))) or {}
    ...
    """, (tech_id, tech_id, date_debut, date_fin))) or {}
    # Congés
    h_conges = (one(db.execute("""
        SELECT COALESCE(SUM(jours), 0) AS jours
        FROM demandes_conges
        WHERE utilisateur_id=? AND statut='APPROUVEE'
          AND date_debut >= ? AND date_debut <= ?
    """, (tech_id, date_debut, date_fin))) or {}).get("jours", 0)
    # Astreintes (BA déclenchées où le tech est intervenant)
    nb_astreintes = (one(db.execute("""
        SELECT COUNT(DISTINCT i.id) AS n
        FROM interventions i
        JOIN astreinte_declenchements ad ON ad.intervention_id=i.id
        WHERE i.technicien_id=? AND i.date_prevue >= ? AND i.date_prevue <= ?
    """, (tech_id, date_debut, date_fin))) or {}).get("n", 0)
    # Score QSE
    qse_data = rows(db.execute("""
        SELECT c.id, c.titre, c.planif_mois, c.date_publication,
               r.score, r.total_questions, r.completed_at
    ...
    try:
        cur = db.execute("""
            INSERT INTO causeries (titre, frequence, date_publication, date_cloture,
                                   ...
                                   planif_rang, planif_jour, planif_mois, planif_annee)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            titre,
...
def repondre_causerie(cid):
    ...
    Body : {reponses: [{question_id, choix_index}]}
    """
    d = request.json or {}
    ...
    try:
        cur = db.execute("""
            INSERT INTO causeries_reponses (causerie_id, utilisateur_id, score, total_questions)
            VALUES (?, ?, ?, ?)
        """, (cid, user_id, score, len(questions)))
        ...
        db.commit()
        log_action(request.user, "ANSWER", "causerie", cid, f"score={score}/{len(questions)}")
        # v218.39 : envoi mail récap individuel à l'utilisateur (avec ses réponses, couleurs rouge/vert)
        try:
            _envoyer_mail_recap_individuel(cid, user_id, score, questions, details)
        except Exception as e:
...
def _envoyer_mail_recap_individuel(cid, user_id, score, questions, details):
...
<div style="background:#1E3A8A;color:#fff;padding:24px 28px">
  ...
  <div style="font-size:13px;margin-top:6px;opacity:0.9">Bonjour <b>{html_escape(user['nom'] or '')}</b>, voici le récapitulatif de vos réponses au questionnaire.</div>
    ...
    # Score
    html_parts.append(f"""
<div style="padding:24px 28px;text-align:center;background:#F8FAFC;border-bottom:1px solid #E2E8F0">
  <div style="font-size:13px;color:#64748B;margin-bottom:6px">Votre score</div>
  <div style="font-size:42px;font-weight:700;color:{score_color}">{score} / {total_q}</div>
  <div style="font-size:18px;color:{score_color};font-weight:600">{pct}%</div>
</div>""")
    # Liste des questions
...
def _check_and_send_causerie_recap(cid):
    ...
    pct_w = 6
    date_w = 19
    lines.append(f"{'Nom':<{name_w}} | {'Rôle':<{role_w}} | {'Score':>{score_w}} | {'%':>{pct_w}} | {'Date complétion':<{date_w}}")
    lines.append("-" * (name_w + role_w + score_w + pct_w + date_w + 12))
    for u in users:
        ...
        lines.append(f"{nom[:name_w]:<{name_w}} | {role:<{role_w}} | {score:>4} / {total:<3} | {pct:>3}% | {date_str:<{date_w}}")
    lines.append("")
    lines.append("=" * 50)
...
def get_causerie_resultats(cid):
    """Tableau des résultats : tous les techniciens + leur statut/score."""
    db = get_db()
    c = one(db.execute("SELECT * FROM causeries WHERE id=?", (cid,)))
    if not c:
        return jsonify({"error": "Introuvable"}), 404
    nb_q = one(db.execute("SELECT COUNT(*) AS n FROM causeries_questions WHERE causerie_id=?", (cid,))).get("n", 0)
    # Tous les utilisateurs actifs (technicien + manager) qui DOIVENT répondre
    users = rows(db.execute("""
        SELECT u.id, u.nom, u.email, u.role,
          r.score, r.total_questions, r.completed_at
        FROM utilisateurs u
...
def create_intervention():
    ...
    try:
        ...
        db.execute("""INSERT INTO interventions
            (numero,equipement_id,tableau_id,technicien_id,equipe_id,type,statut,date_prevue,heure_prevue,description)
            VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (num,d["equipement_id"],to_int(d.get("tableau_id")),main_tech,to_int(d.get("equipe_id")),
             type_iv,d.get("statut",default_statut),d.get("date_prevue") or None,heure_prevue_v,d.get("description","")))
...
def update_intervention(iid):
    ...
    for f in ["technicien_id","equipement_id","equipe_id","type","statut","date_prevue","heure_prevue","date_realisation","description","rapport","tableau_id"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations", "mesures_batteries", "image"):
        return jsonify({"error": "type invalide"}), 400
    ordre = int(d.get("ordre") or 0)
...
def _bon_sous_type_label(intervention_id):
    """Retourne le sous-type d'une intervention en agrégeant ses gammes.
    Règles :
    ...
    v217.18 : on remonte les gammes via equipement_gammes (équipement → gammes liées),
    PAS via intervention_operations qui ne contient que les cochages d'opérations.
    Avant ce fix, tant qu'aucune op n'était cochée, on renvoyait None et donc blocs:[].
    """
    db = get_db()
    iv = one(db.execute("SELECT equipement_id FROM interventions WHERE id=?", (intervention_id,)))
    if not iv:
        return None
    eq_id = iv["equipement_id"]
    # Toutes les gammes liées à cet équipement (table de liaison) + fallback colonne legacy gamme_id
    gids_rows = rows(db.execute("SELECT gamme_id FROM equipement_gammes WHERE equipement_id=?", (eq_id,)))
    gids = [g["gamme_id"] for g in gids_rows]
    eq_row = one(db.execute("SELECT gamme_id FROM equipements WHERE id=?", (eq_id,)))
    if eq_row and eq_row.get("gamme_id") and eq_row["gamme_id"] not in gids:

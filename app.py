...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations"):
    ...
    if isinstance(bloc_fopts, dict):
        ...
        bloc_fopts = ""
    db = get_db()
    db.execute("INSERT INTO mesure_blocs (technique_id, sous_type_id, nom, type, ordre, field_options) VALUES (?, ?, ?, ?, ?, ?)",
               (technique_id, sous_type_id, nom, type_module, ordre, bloc_fopts))
    bid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
...
def get_intervention_mesures(iid):
    ...
    blocs = rows(db.execute("""
        SELECT mb.id, mb.nom, mb.ordre, mb.type, mb.field_options FROM mesure_blocs mb
...
def _intervention_mesures_data(intervention_id):
    ...
    if not st:
        return []
    blocs = rows(db.execute("""
        SELECT id, nom, type, field_options FROM mesure_blocs
        WHERE technique_id=? AND sous_type_id=?
        ORDER BY ordre, nom
    """, (technique_id, st["id"])))
    if not blocs:

...
def init_db():
    ...
    # projets.deplacement_km + nb_deplacements : distance A/R et nombre de déplacements
    # (repris en info sur les CRs de dépannage pour ce projet)
...
def get_interventions():
    ...
    if u["role"]=="technicien":
        ...
        params_mgr = [u["id"], u["id"], u["id"], u["id"], u["id"]]
        if tech_list:
            placeholders = ",".join(["?"] * len(tech_list))
...
def generate_pdf(iid):
    ...
    def _calc_heures(hd, hf):
        ...
        try:
            ...
            diff = m_fin - m_debut
            if diff < 0: return 0.0  # horaire invalide
    ...
    # Déplacement projet (pour dépannages uniquement - affichage en info)
    depl_km = 0.0; depl_nb = 0
    try:
        depl_km = float(i.get("deplacement_km") or 0)
        depl_nb = int(i.get("nb_deplacements") or 0)
    except Exception: pass
    # Fallback : lire sur la table projets si pas dans l'intervention
    if (depl_km <= 0 or depl_nb <= 0) and i.get("equipement_id"):
        try:
            proj = one(db.execute("""SELECT p.deplacement_km, p.nb_deplacements
                FROM projets p JOIN equipements e ON e.projet_id=p.id WHERE e.id=?""",
                (i["equipement_id"],)))
            if proj:
                depl_km = float(proj.get("deplacement_km") or 0)
                depl_nb = int(proj.get("nb_deplacements") or 0)
        except Exception: pass

...
def get_equipements():
    ...
    if user["role"] == "manager":
        ...
        sql += """ AND e.projet_id IN (SELECT DISTINCT e2.projet_id FROM interventions i
                   JOIN equipements e2 ON i.equipement_id=e2.id WHERE i.technicien_id=?)"""
        params.append(user["id"])
    equips = rows(db.execute(sql+" ORDER BY p.nom,e.designation",params))
    for e in equips: e["criticite"] = criticite(e["id"],db)
...
def get_pieces():
    ...
    return jsonify(rows(db.execute(sql+" ORDER BY p.date_fin_de_vie",params)))
...
def create_piece():
    ...
    db = get_db()
    db.execute("INSERT INTO pieces (equipement_id,type_piece,date_installation,duree_vie_estimee,date_fin_de_vie,statut,commentaire) VALUES (?,?,?,?,?,?,?)",
               (d["equipement_id"],d["type_piece"],d.get("date_installation") or None,d.get("duree_vie_estimee") or None,dv,st,d.get("commentaire","")))
    db.commit()
    return jsonify({"id":db.execute("SELECT last_insert_rowid()").fetchone()[0]}),201
...
def get_interventions():
    ...
    sql  = """SELECT i.*,e.designation AS equip_nom,e.type_technique,e.localisation,e.projet_id,
                     p.nom AS projet_nom,p.numero_projet,u.nom AS technicien_nom
              FROM interventions i
              JOIN equipements e ON i.equipement_id=e.id
              JOIN projets p ON e.projet_id=p.id
              LEFT JOIN utilisateurs u ON i.technicien_id=u.id WHERE 1=1"""
    params = []
...
def create_intervention():
    ...
    db.execute("INSERT INTO interventions (numero,equipement_id,technicien_id,type,statut,date_prevue,description) VALUES (?,?,?,?,?,?,?)",
               (num,d["equipement_id"],tech,d["type"],d.get("statut","A_PLANIFIER"),d.get("date_prevue") or None,d.get("description","")))
    db.commit()
...
def update_intervention(iid):
    ...
    for f in ["technicien_id","type","statut","date_prevue","date_realisation","description","rapport"]:
        if f in d:
            v = to_int(d[f]) if f=="technicien_id" else d[f]
...
def save_cr(iid):
    ...
    # Si CR existant, seuls admin/manager peuvent modifier
    if ex and user["role"] == "technicien":
...
def export_excel():
    try:
        import openpyxl
        db = get_db(); wb = openpyxl.Workbook(); first = True
        for name,sql in [("Interventions","SELECT * FROM interventions"),
                         ("Equipements","SELECT * FROM equipements"),
                         ("Pieces","SELECT * FROM pieces"),
                         ("Projets","SELECT * FROM projets"),
                         ("Clients","SELECT * FROM clients"),
                         ("CR","SELECT id,intervention_id,date_intervention,observations,actions_realisees,mesures,recommandations,conclusion,total_heures FROM comptes_rendus")]:
            try:

...
def declencher_astreinte():
    ...
    v218.19 : equipement_id facultatif (le technicien le complétera lors de la prise en compte).
    Body :
      - description (requis)
      ...
      - technicien_astreinte (texte libre)
      - technicien_id (utilisateur GMAO — REQUIS pour notifier le technicien)
    """
    d = request.json or {}
    ...
    if not tech_id:
        return jsonify({"error": "Technicien GMAO requis (il recevra le bon)"}), 400
    db = get_db()
    try:
        ...
        iid = cur.lastrowid
        # Enregistrer le tech comme intervenant
        try:

...
def get_classement_qse():
    ...
    db = get_db()
    annee = int(request.args.get("annee") or datetime.now().year)
    # Récupère toutes les causeries actives de l'année
    ...
    # Tous les utilisateurs concernés (techniciens + managers uniquement, pas les admins)
    users = rows(db.execute("""
        SELECT id, nom, email, role
        FROM utilisateurs
        WHERE actif=1 AND role IN ('technicien','manager')
        ORDER BY nom
    """))
    if not causerie_ids:
...
def get_occupations():
    ...
    sql = """SELECT o.*, u.nom AS technicien_nom, ot.nom AS type_nom, ot.couleur AS type_couleur,
             ...
             WHERE 1=1"""
    params = []
    # v218.48/51 : un tech voit ses propres occupations + celles où il est accompagnant
    # On utilise json_each pour parser le champ accompagnants_ids (JSON array)
    # Protection v218.51 : LIKE '[%' filtre les anciennes données mal formées (ex: '8' au lieu de '[8]')
    def _accompagnant_filter(uid):
        return ("(o.technicien_id = ? OR ("
                "o.accompagnants_ids IS NOT NULL "
                "AND o.accompagnants_ids LIKE '[%' "
                "AND EXISTS (SELECT 1 FROM json_each(o.accompagnants_ids) WHERE CAST(value AS INTEGER) = ?)"
                "))"), [uid, uid]
    if u["role"] == "technicien":
        clause, p = _accompagnant_filter(u["id"])
        sql += " AND " + clause
        params.extend(p)
    elif request.args.get("technicien_id"):
        clause, p = _accompagnant_filter(to_int(request.args["technicien_id"]))
        sql += " AND " + clause
        params.extend(p)
...
def planning_me():
    ...
    # v218.49 : inclut aussi les occupations où le tech est accompagnant (pas seulement le créateur principal)
    # v218.51 : protection contre les anciennes données corrompues (accompagnants_ids non-JSON)
    #          → on filtre avec LIKE '[%' avant d'appeler json_each (évite "malformed JSON")
    occs = rows(db.execute("""

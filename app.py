...
def require_auth(fn):
    ...
    def wrapper(*a, **k):
        ...
        if not u or hp(f"{uid}:{u['email']}:{u['password_hash']}")!=parts[1]:
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS utilisateurs (
        ...
        password_hash TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'technicien'
    ...
    db.commit()
    try:
        db.execute("INSERT OR IGNORE INTO utilisateurs (nom,email,password_hash,role) VALUES (?,?,?,?)",
...
def login():
    ...
    if not u or u["password_hash"]!=hp(d.get("password","")): return jsonify({"error":"Email ou mot de passe incorrect"}),401
    token=f"{u['id']}:{hp(str(u['id'])+':'+u['email']+':'+u['password_hash'])}"
...
def create_utilisateur():
    ...
    db=get_db()
    try:
        db.execute("INSERT INTO utilisateurs (nom,email,password_hash,role) VALUES (?,?,?,?)",
...
def update_utilisateur(uid):
    ...
    if "password" in d and d["password"]: sets.append("password_hash=?"); params.append(hp(d["password"]))
...
def create_compte_rendu(iid):
    d=request.json or {}; db=get_db()
    db.execute("INSERT INTO comptes_rendus (intervention_id,date_intervention,observations,actions_realisees,mesures,recommandations,conclusion) VALUES (?,?,?,?,?,?,?)",
               (iid,d.get("date_intervention") or None,d.get("observations",""),d.get("actions_realisees",""),d.get("mesures",""),d.get("recommandations",""),d.get("conclusion","")))
    ...
    db.commit(); return jsonify({"id":cr_id}),201

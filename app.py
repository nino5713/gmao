...
def generate_projet_planning_pdf(pid):
    ...
    d = request.json or {}
    db = get_db()
    proj = one(db.execute("""SELECT p.id, p.nom, p.numero_projet, p.adresse,
    ...
    try:
        ...
        fname = f"Planning_{proj.get('numero_projet','').replace('/','-') or 'projet'}_{data['annee']}.pdf"
...
def _find_next_available_date(db, start_date_str, type_iv, sid, planning_id=None,
    ...
    """Trouve la première date >= start_date_str qui :
       - n'est ni samedi, ni dimanche
       - n'est pas un jour férié (table jours_feries pour la société)
       - n'atteint pas la capacité nb_simultane du planning (si planning_id fourni)
    Ne s'applique QU'aux interventions de type MAINTENANCE.
    Retourne (date_str, was_shifted: bool). Si pas de date trouvée → retourne (None, False).
    """
    ...
    try:
        ...
        return (start_date_str, False)
    original = cur
    # Pré-charger les fériés sur la fenêtre
    ...
    while iters < max_lookahead_days:
        ...
        # Vérifier weekend (5=samedi, 6=dimanche)
        if cur.weekday() >= 5:
            cur += timedelta(days=1); continue
        # Vérifier férié
        if date_str in feries_set:
            cur += timedelta(days=1); continue
        # Vérifier capacité du planning (si applicable)
        if planning_id and nb_simultane >= 1:
            # Compter les bons MAINTENANCE déjà planifiés ce jour sur les équipements
            # appartenant à ce planning
            cnt_sql = """SELECT COUNT(*) AS n FROM interventions i
                         JOIN equipements e ON i.equipement_id = e.id
                         WHERE i.type='MAINTENANCE'
                           AND i.date_prevue = ?
                           AND i.societe_id = ?
                           AND e.planning_id = ?"""
            cnt_params = [date_str, sid, planning_id]
            if ignore_intervention_id:
                cnt_sql += " AND i.id != ?"
                cnt_params.append(ignore_intervention_id)
            res = one(db.execute(cnt_sql, cnt_params))
            current_count = (res and res.get("n")) or 0
            if current_count >= nb_simultane:
                cur += timedelta(days=1); continue
        # OK : date dispo
        return (date_str, (cur != original))
    # Pas trouvé dans la fenêtre
    return (None, False)
...
def find_next_slot():
    ...
    Body : { date_souhaitee, equipement_id (pour récupérer le planning), type? }
    ...
    new_date, was_shifted = _find_next_available_date(db, date_souhaitee, type_iv, sid, planning_id)
    if not new_date:
...
def create_intervention():
    ...
    try:
        ...
        if type_iv == "MAINTENANCE" and date_prevue_final and not force_date:
            ...
            new_date, shifted = _find_next_available_date(db, date_prevue_final, type_iv, sid, eq_planning_id)
...
def set_equipement_valeurs_custom(eid):
    ...
    for item in valeurs:
        ...
        if not c: continue
        if c["type"] == "file": continue  # on ne touche pas les fichiers ici
        val = item.get("valeur")
        if val is None: val = ""

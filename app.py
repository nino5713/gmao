...
def init_db():
    ...
    -- ASTREINTES BIS : nouveau système avec groupes et périodes contiguës
    CREATE TABLE IF NOT EXISTS astreinte_bis_groupes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        nom TEXT NOT NULL,
        nb_techs INTEGER NOT NULL DEFAULT 1,
        ordre INTEGER DEFAULT 0,
        created_at TEXT DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS astreinte_bis_membres (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        groupe_id INTEGER NOT NULL REFERENCES astreinte_bis_groupes(id) ON DELETE CASCADE,
        technicien_id INTEGER NOT NULL REFERENCES utilisateurs(id) ON DELETE CASCADE,
        ordre INTEGER DEFAULT 0,
        UNIQUE(groupe_id, technicien_id)
    );
    CREATE INDEX IF NOT EXISTS idx_astreinte_bis_membres_groupe ON astreinte_bis_membres(groupe_id);
    CREATE TABLE IF NOT EXISTS astreinte_bis_periodes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        groupe_id INTEGER NOT NULL REFERENCES astreinte_bis_groupes(id) ON DELETE CASCADE,
        date_debut TEXT NOT NULL,
        heure_debut TEXT NOT NULL DEFAULT '08:00',
        date_fin TEXT NOT NULL,
        heure_fin TEXT NOT NULL DEFAULT '08:00',
        technicien_id INTEGER REFERENCES utilisateurs(id) ON DELETE SET NULL,
        backup_id INTEGER REFERENCES utilisateurs(id) ON DELETE SET NULL,
        notes TEXT DEFAULT '',
        created_at TEXT DEFAULT (datetime('now'))
    );
    CREATE INDEX IF NOT EXISTS idx_astreinte_bis_periodes_groupe ON astreinte_bis_periodes(groupe_id);
    CREATE INDEX IF NOT EXISTS idx_astreinte_bis_periodes_dates ON astreinte_bis_periodes(date_debut, date_fin);
...
# ══════════════════════════════════════════════════════════════════════
# ASTREINTES BIS — Nouveau système avec groupes & rotations
# ══════════════════════════════════════════════════════════════════════
@app.route("/api/astreintes_bis/groupes", methods=["GET"])
@require_auth
def ab_list_groupes():
    db = get_db()
    grps = rows(db.execute("SELECT * FROM astreinte_bis_groupes ORDER BY ordre, id"))
    for g in grps:
        g["membres"] = rows(db.execute("""
            SELECT m.id, m.technicien_id, m.ordre, u.nom AS technicien_nom
            FROM astreinte_bis_membres m
            JOIN utilisateurs u ON m.technicien_id = u.id
            WHERE m.groupe_id = ? ORDER BY m.ordre, m.id
        """, (g["id"],)))
    return jsonify(grps)

@app.route("/api/astreintes_bis/groupes", methods=["POST"])
@require_role("admin","manager")
def ab_create_groupe():
    d = request.json or {}
    nom = (d.get("nom","") or "").strip()
    if not nom:
        return jsonify({"error":"Nom requis"}), 400
    nb = int(d.get("nb_techs") or 1)
    if nb not in (1, 2):
        return jsonify({"error":"nb_techs doit être 1 ou 2"}), 400
    db = get_db()
    db.execute("INSERT INTO astreinte_bis_groupes (nom, nb_techs, ordre) VALUES (?,?,?)",
               (nom, nb, int(d.get("ordre",0) or 0)))
    db.commit()
    gid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    return jsonify({"id": gid}), 201

@app.route("/api/astreintes_bis/groupes/<int:gid>", methods=["PATCH"])
@require_role("admin","manager")
def ab_update_groupe(gid):
    d = request.json or {}
    db = get_db()
    sets, params = [], []
    if "nom" in d:
    ...
    if "nb_techs" in d:
        nb = int(d["nb_techs"])
        if nb not in (1,2): return jsonify({"error":"nb_techs doit être 1 ou 2"}), 400
        sets.append("nb_techs=?"); params.append(nb)
    if "ordre" in d:
        sets.append("ordre=?"); params.append(int(d["ordre"] or 0))
    if not sets: return jsonify({"ok": True})
    params.append(gid)
    db.execute(f"UPDATE astreinte_bis_groupes SET {','.join(sets)} WHERE id=?", params)
    db.commit()
    return jsonify({"ok": True})

@app.route("/api/astreintes_bis/groupes/<int:gid>", methods=["DELETE"])
@require_role("admin","manager")
def ab_delete_groupe(gid):
    db = get_db()
    db.execute("DELETE FROM astreinte_bis_groupes WHERE id=?", (gid,))
    db.commit()
    return jsonify({"ok": True})

# Membres d'un groupe
@app.route("/api/astreintes_bis/groupes/<int:gid>/membres", methods=["POST"])
@require_role("admin","manager")
def ab_add_membre(gid):
    d = request.json or {}
    tid = int(d.get("technicien_id") or 0)
    if not tid: return jsonify({"error":"technicien_id requis"}), 400
    db = get_db()
    # Vérifier que l'utilisateur existe
    if not one(db.execute("SELECT 1 FROM utilisateurs WHERE id=?", (tid,))):
        return jsonify({"error":"Utilisateur introuvable"}), 404
    try:
        db.execute("INSERT INTO astreinte_bis_membres (groupe_id, technicien_id, ordre) VALUES (?,?,?)",
                   (gid, tid, int(d.get("ordre",0) or 0)))
        db.commit()
    except Exception:
        return jsonify({"error":"Membre déjà dans le groupe"}), 400
    mid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    return jsonify({"id": mid}), 201

@app.route("/api/astreintes_bis/membres/<int:mid>", methods=["DELETE"])
@require_role("admin","manager")
def ab_delete_membre(mid):
    db = get_db()
    db.execute("DELETE FROM astreinte_bis_membres WHERE id=?", (mid,))
    db.commit()
    return jsonify({"ok": True})

# Périodes
@app.route("/api/astreintes_bis/periodes", methods=["GET"])
@require_auth
def ab_list_periodes():
    db = get_db()
    sql = """SELECT p.*,
                    g.nom AS groupe_nom, g.nb_techs,
                    u1.nom AS technicien_nom,
                    u2.nom AS backup_nom
             FROM astreinte_bis_periodes p
             JOIN astreinte_bis_groupes g ON p.groupe_id = g.id
             LEFT JOIN utilisateurs u1 ON p.technicien_id = u1.id
             LEFT JOIN utilisateurs u2 ON p.backup_id = u2.id
             WHERE 1=1"""
    params = []
    if request.args.get("groupe_id"):
        sql += " AND p.groupe_id=?"; params.append(request.args["groupe_id"])
    # Filtre par plage de dates (chevauchement)
    df = request.args.get("date_fin"); dd = request.args.get("date_debut")
    if dd: sql += " AND p.date_fin >= ?"; params.append(dd)
    if df: sql += " AND p.date_debut <= ?"; params.append(df)
    sql += " ORDER BY p.date_debut, p.heure_debut"
    return jsonify(rows(db.execute(sql, params)))

@app.route("/api/astreintes_bis/periodes", methods=["POST"])
@require_role("admin","manager")
def ab_create_periode():
    d = request.json or {}
    gid = int(d.get("groupe_id") or 0)
    if not gid: return jsonify({"error":"groupe_id requis"}), 400
    dd = d.get("date_debut"); df = d.get("date_fin")
    if not dd or not df: return jsonify({"error":"date_debut et date_fin requis"}), 400
    db = get_db()
    db.execute("""INSERT INTO astreinte_bis_periodes
        (groupe_id, date_debut, heure_debut, date_fin, heure_fin, technicien_id, backup_id, notes)
        VALUES (?,?,?,?,?,?,?,?)""",
        (gid, dd, d.get("heure_debut","08:00"), df, d.get("heure_fin","08:00"),
         to_int(d.get("technicien_id")), to_int(d.get("backup_id")), d.get("notes","") or ""))
    db.commit()
    pid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    return jsonify({"id": pid}), 201

@app.route("/api/astreintes_bis/periodes/<int:pid>", methods=["PATCH"])
@require_role("admin","manager")
def ab_update_periode(pid):
    d = request.json or {}
    db = get_db()
    sets, params = [], []
    for k in ("date_debut","heure_debut","date_fin","heure_fin","technicien_id","backup_id","notes"):
        if k in d:
            sets.append(f"{k}=?")
            v = d[k]
            if k in ("technicien_id","backup_id"): v = to_int(v)
            params.append(v)
    if not sets: return jsonify({"ok": True})
    params.append(pid)
    db.execute(f"UPDATE astreinte_bis_periodes SET {','.join(sets)} WHERE id=?", params)
    db.commit()
    return jsonify({"ok": True})

@app.route("/api/astreintes_bis/periodes/<int:pid>", methods=["DELETE"])
@require_role("admin","manager")
def ab_delete_periode(pid):
    db = get_db()
    db.execute("DELETE FROM astreinte_bis_periodes WHERE id=?", (pid,))
    db.commit()
    return jsonify({"ok": True})

# Génération automatique de rotation hebdomadaire
@app.route("/api/astreintes_bis/generer_rotation", methods=["POST"])
@require_role("admin","manager")
def ab_generer_rotation():
    """Génère N semaines d'astreinte hebdo (lundi 08:00 → lundi 08:00)
    en répartissant les techniciens du groupe en rotation.
    Body: {groupe_id, date_debut (lundi), nb_semaines, ecraser:bool}"""
    from datetime import datetime, timedelta
    d = request.json or {}
    gid = int(d.get("groupe_id") or 0)
    if not gid: return jsonify({"error":"groupe_id requis"}), 400
    date_debut = d.get("date_debut")
    nb_sem = int(d.get("nb_semaines") or 0)
    if not date_debut or nb_sem <= 0:
        return jsonify({"error":"date_debut et nb_semaines requis"}), 400
    try:
        dt0 = datetime.strptime(date_debut, "%Y-%m-%d").date()
    except Exception:
        return jsonify({"error":"date_debut invalide (format YYYY-MM-DD)"}), 400
    db = get_db()
    grp = one(db.execute("SELECT * FROM astreinte_bis_groupes WHERE id=?", (gid,)))
    if not grp: return jsonify({"error":"Groupe introuvable"}), 404
    membres = rows(db.execute("SELECT technicien_id FROM astreinte_bis_membres WHERE groupe_id=? ORDER BY ordre, id", (gid,)))
    if not membres:
        return jsonify({"error":"Aucun membre dans ce groupe"}), 400
    nb_techs_grp = grp["nb_techs"]  # 1 ou 2
    tech_ids = [m["technicien_id"] for m in membres]
    n = len(tech_ids)
    date_fin_max = (dt0 + timedelta(weeks=nb_sem)).strftime("%Y-%m-%d")
    # Si ecraser : supprimer les périodes existantes du groupe sur cette plage
    if d.get("ecraser"):
        db.execute("""DELETE FROM astreinte_bis_periodes
            WHERE groupe_id=? AND date_debut >= ? AND date_debut < ?""",
            (gid, dt0.strftime("%Y-%m-%d"), date_fin_max))
        db.commit()
    created = 0
    for i in range(nb_sem):
        ds = dt0 + timedelta(weeks=i)
        de = ds + timedelta(days=7)  # lundi suivant
        principal = tech_ids[i % n]
        backup = tech_ids[(i + 1) % n] if nb_techs_grp == 2 and n >= 2 else None
        db.execute("""INSERT INTO astreinte_bis_periodes
            (groupe_id, date_debut, heure_debut, date_fin, heure_fin, technicien_id, backup_id, notes)
            VALUES (?,?,?,?,?,?,?,?)""",
            (gid, ds.strftime("%Y-%m-%d"), "08:00", de.strftime("%Y-%m-%d"), "08:00",
             principal, backup, ""))
        created += 1
    db.commit()

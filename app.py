...
ALLOWED_UPDATE_COLS = {
    ...
    "interventions":         {"equipement_id","technicien_id","equipe_id","type","statut",
                              "date_prevue","date_realisation","description","rapport","heure_prevue","numero_offre","numero_projet_offre"},
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS interventions (
        ...
        type TEXT NOT NULL DEFAULT 'MAINTENANCE' CHECK(type IN ('MAINTENANCE','DEPANNAGE','OFFRE')),
        ...
        numero_offre TEXT DEFAULT '',
        numero_projet_offre TEXT DEFAULT '',
        created_at TEXT DEFAULT (datetime('now'))
    );
    ...
    # Migration : élargir CHECK(type) pour accepter OFFRE en plus de MAINTENANCE et DEPANNAGE
    try:
        schema_row = one(db.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='interventions'"))
        if schema_row and "'OFFRE'" not in (schema_row.get("sql") or ""):
            logger.info("[MIGRATION] recreation table interventions pour ajouter type OFFRE")
            ...
            db.executescript("""
                CREATE TABLE interventions_new (
                    ...
                    type TEXT NOT NULL DEFAULT 'MAINTENANCE'
                        CHECK(type IN ('MAINTENANCE','DEPANNAGE','OFFRE')),
                    statut TEXT NOT NULL DEFAULT 'PLANIFIEE'
                        CHECK(statut IN ('PLANIFIEE','EN_COURS','TERMINEE','ANNULEE')),
                    date_prevue TEXT, date_realisation TEXT,
                    description TEXT DEFAULT '', rapport TEXT DEFAULT '',
            ...
            db.commit()
            logger.info("[MIGRATION] table interventions recreee avec OFFRE")
    except Exception as e:
        logger.error(f"[MIGRATION] echec recreation interventions: {e}")

    # Colonne numero_offre (obligatoire pour les bons OFFRE, format DP + 8 chiffres)
    _add_col("interventions", "numero_offre", "TEXT DEFAULT ''")
    # Colonne numero_projet_offre (N° de projet saisi manuellement pour les OFFRE, utilisé dans le pointage)
    _add_col("interventions", "numero_projet_offre", "TEXT DEFAULT ''")
...
def create_intervention():
    ...
    # Validation numero_offre pour type OFFRE : format DP + 8 chiffres
    numero_offre = (d.get("numero_offre") or "").strip()
    numero_projet_offre = (d.get("numero_projet_offre") or "").strip()
    if type_iv == "OFFRE":
        if not numero_offre:
            return jsonify({"error":"Numéro de l'offre requis pour un bon OFFRE"}),400
        if not re.match(r"^DP\d{8}$", numero_offre):
            return jsonify({"error":"Le numéro de l'offre doit être au format DP suivi de 8 chiffres (ex: DP12345678)"}),400
        if not numero_projet_offre:
            return jsonify({"error":"Numéro de projet requis pour un bon OFFRE"}),400
    else:
        numero_offre = ""  # ignorer si pas OFFRE
        numero_projet_offre = ""
    # Préfixe numéro selon type : BP (maintenance), BC (dépannage), BO (offre)
    prefix_map = {"MAINTENANCE": "BP", "DEPANNAGE": "BC", "OFFRE": "BO"}
    prefix = prefix_map.get(type_iv, "BT")
    num = next_numero(db, prefix, "interventions", "numero")
    db.execute("""INSERT INTO interventions
        (numero,equipement_id,technicien_id,equipe_id,type,statut,date_prevue,description,numero_offre,numero_projet_offre)
        VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (num,d["equipement_id"],to_int(d.get("technicien_id")),to_int(d.get("equipe_id")),
         ...
         numero_offre,numero_projet_offre))
...
def update_intervention(iid):
    d=request.json or {}; db=get_db(); sets,params=[],[]
    # Si le type est changé en OFFRE ou si numero_offre/numero_projet_offre est fourni, valider
    if "numero_offre" in d or "numero_projet_offre" in d or d.get("type") == "OFFRE":
        # Récupérer le type final (celui envoyé ou actuel en DB)
        cur = one(db.execute("SELECT type FROM interventions WHERE id=?",(iid,)))
        final_type = d.get("type") or (cur and cur.get("type"))
        if final_type == "OFFRE":
            numoffre = (d.get("numero_offre") or "").strip()
            if not numoffre:
                # Récupérer celui en DB si pas fourni
                if "numero_offre" not in d:
                    existing = one(db.execute("SELECT numero_offre FROM interventions WHERE id=?",(iid,)))
                    numoffre = (existing and existing.get("numero_offre")) or ""
                if not numoffre:
                    return jsonify({"error":"Numéro de l'offre requis pour un bon OFFRE"}),400
            if numoffre and not re.match(r"^DP\d{8}$", numoffre):
                return jsonify({"error":"Le numéro de l'offre doit être au format DP suivi de 8 chiffres (ex: DP12345678)"}),400
            if "numero_offre" in d:
                d["numero_offre"] = numoffre
            # Vérifier numero_projet_offre
            nproj = (d.get("numero_projet_offre") or "").strip()
            if "numero_projet_offre" in d:
                if not nproj:
                    return jsonify({"error":"Numéro de projet requis pour un bon OFFRE"}),400
                d["numero_projet_offre"] = nproj
            else:
                existing = one(db.execute("SELECT numero_projet_offre FROM interventions WHERE id=?",(iid,)))
                if not (existing and existing.get("numero_projet_offre")):
                    return jsonify({"error":"Numéro de projet requis pour un bon OFFRE"}),400
    for f in ["technicien_id","equipe_id","type","statut","date_prevue","heure_prevue","date_realisation","description","rapport","numero_offre","numero_projet_offre"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
...
def pointage_mensuel(annee, mois):
    ...
    sql = """SELECT ci.date, ci.heure_debut, ci.heure_fin, ci.total_heures,
                    ...
                    i.numero_projet_offre,
                    e.designation AS equipement, p.nom AS projet,
                    p.numero_projet AS num_projet_p
    ...
    # Calculer le numéro de projet effectif : pour OFFRE → numero_projet_offre, sinon → projets.numero_projet
    for l in lignes:
        if l.get("type_bon") == "OFFRE":
            l["num_projet"] = l.get("numero_projet_offre") or ""
        else:
            l["num_projet"] = l.get("num_projet_p") or ""
    ...
    for l in lignes:
        ...
        par_tech[key]["total_heures"] += float(l.get("total_heures") or 0)
...
def pointage_export(annee, mois):
    ...
    lignes = rows(db.execute("""
        SELECT ci.date, ci.heure_debut, ci.heure_fin, ci.total_heures,
               ...
               i.numero AS num_bon, i.type AS type_bon, i.numero_projet_offre,
               e.designation AS equipement, p.nom AS projet,
               p.numero_projet AS num_projet_p
        FROM cr_intervenants ci
        JOIN comptes_rendus cr ON ci.cr_id = cr.id
    ...
    ws = wb.active
    ws.title = "Détail"
    headers = ["Technicien", "Date", "Début", "Fin", "Total h", "N° Bon", "Type", "N° Projet", "Équipement", "Projet"]
    ...
    for i, h in enumerate(headers, 1):
        ...
        c.border = border
    for idx, l in enumerate(lignes, 2):
        row_data = [
        ...
        for ci, v in enumerate(row_data, 1):
            ...
            c.border = border
    # Largeurs
    widths = [22, 12, 8, 8, 10, 14, 14, 14, 30, 25]
    for i, w in enumerate(widths, 1):
        ws.column_dimensions[chr(64+i)].width = w
    ws.freeze_panes = "A2"
    # ── Feuille 2 : Totaux ──
    ws2 = wb.create_sheet("Totaux")
    headers2 = ["Technicien", "Total heures", "Nb jours travaillés", "Nb interventions"]
    ...
    for l in lignes:
        key = l["nom_tech"]
        if key not in par_tech:
            par_tech[key] = {"nom": key, "total": 0, "jours": set(), "bons": set()}
        par_tech[key]["total"] += float(l.get("total_heures") or 0)
        par_tech[key]["jours"].add(l["date"])
        par_tech[key]["bons"].add(l.get("num_bon",""))
    for idx, (nom, v) in enumerate(sorted(par_tech.items()), 2):
        row_data = [v["nom"], round(v["total"],2), len(v["jours"]), len(v["bons"])]
        for ci, val in enumerate(row_data, 1):
            c = ws2.cell(row=idx, column=ci, value=val)
            c.border = border
            if ci == 2: c.font = Font(bold=True)
    for i, w in enumerate([22, 14, 20, 18], 1):

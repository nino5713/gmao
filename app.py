...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS types_pieces (
        ...
        has_sous_pieces INTEGER NOT NULL DEFAULT 0,
        ordre INTEGER DEFAULT 0,
        actif INTEGER NOT NULL DEFAULT 1,
        created_at TEXT DEFAULT (datetime('now')),
    ...
    -- v218.171 : sous-pièces d'une pièce critique (typiquement strings pour les PANNEAUX)
    CREATE TABLE IF NOT EXISTS piece_sous_pieces (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        piece_id INTEGER NOT NULL REFERENCES pieces(id) ON DELETE CASCADE,
        designation TEXT NOT NULL,
        ordre INTEGER DEFAULT 0,
    ...
    CREATE INDEX IF NOT EXISTS idx_piece_sous_pieces_p ON piece_sous_pieces(piece_id);
    -- v218.171 : mesures STRING enregistrées sur les CRs (1 ligne par sous-pièce × CR)
    CREATE TABLE IF NOT EXISTS cr_string_mesures (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        cr_id INTEGER NOT NULL REFERENCES comptes_rendus(id) ON DELETE CASCADE,
        sous_piece_id INTEGER NOT NULL REFERENCES piece_sous_pieces(id) ON DELETE CASCADE,
        tension TEXT DEFAULT '',
        intensite TEXT DEFAULT '',
        resistance TEXT DEFAULT '',
        updated_at TEXT DEFAULT (datetime('now')),
        UNIQUE(cr_id, sous_piece_id)
    );
    CREATE INDEX IF NOT EXISTS idx_cr_string_cr ON cr_string_mesures(cr_id);
    ...
    # v218.171 : flag has_sous_pieces sur types_pieces
    _add_col("types_pieces", "has_sous_pieces", "INTEGER NOT NULL DEFAULT 0")
...
def list_types_pieces():
    ...
    if incl_inactifs:
        types = rows(db.execute(
            "SELECT id, nom, is_batterie, has_sous_pieces, ordre, actif FROM types_pieces WHERE societe_id=? ORDER BY ordre, nom",
            (sid,)
        ))
    else:
        types = rows(db.execute(
            "SELECT id, nom, is_batterie, has_sous_pieces, ordre, actif FROM types_pieces WHERE societe_id=? AND actif=1 ORDER BY ordre, nom",
            (sid,)
        ))
...
def create_type_piece():
    ...
    has_sp = 1 if d.get("has_sous_pieces") else 0
    # Ordre = max+1
    ...
    cur = db.execute(
        "INSERT INTO types_pieces (societe_id, nom, is_batterie, has_sous_pieces, ordre) VALUES (?,?,?,?,?)",
        (sid, nom, is_bat, has_sp, ordre)
    )
    db.commit()
    return jsonify({"id": cur.lastrowid, "nom": nom, "is_batterie": is_bat, "has_sous_pieces": has_sp, "ordre": ordre, "actif": 1}), 201
...
def update_type_piece(tid):
    ...
    if "has_sous_pieces" in d:
        sets.append("has_sous_pieces=?"); params.append(1 if d["has_sous_pieces"] else 0)
    if "ordre" in d:
...
# ══ SOUS-PIÈCES (v218.171) ══
# Une pièce de type "has_sous_pieces" peut avoir N sous-pièces (designation seulement).
# Typiquement pour les PANNEAUX : chaque sous-pièce = un string.
@app.route("/api/pieces/<int:pid>/sous-pieces", methods=["GET"])
@require_auth
def list_sous_pieces(pid):
    db = get_db()
    return jsonify(rows(db.execute(
        "SELECT id, designation, ordre FROM piece_sous_pieces WHERE piece_id=? ORDER BY ordre, id",
        (pid,)
    )))

@app.route("/api/pieces/<int:pid>/sous-pieces", methods=["PUT"])
@require_role("admin", "manager")
def set_sous_pieces(pid):
    """Remplace toute la liste des sous-pièces d'une pièce.
    Body : { items: [{designation}, ...] }
    Préserve les IDs existants si la designation match (pour ne pas casser les liens cr_string_mesures)."""
    d = request.json or {}
    items = d.get("items") or []
    if not isinstance(items, list):
        return jsonify({"error": "items doit être une liste"}), 400
    db = get_db()
    if not one(db.execute("SELECT 1 FROM pieces WHERE id=?", (pid,))):
        return jsonify({"error": "Pièce introuvable"}), 404
    # Stratégie : pour préserver les mesures CR existantes, on essaie de matcher par designation
    existing = rows(db.execute("SELECT id, designation FROM piece_sous_pieces WHERE piece_id=?", (pid,)))
    existing_by_des = {(r["designation"] or "").strip().lower(): r["id"] for r in existing}
    kept_ids = set()
    for idx, item in enumerate(items):
        designation = (item.get("designation") or "").strip()
        if not designation:
        ...
        if key in existing_by_des:
            # Update ordre
            sid_existing = existing_by_des[key]
            db.execute("UPDATE piece_sous_pieces SET designation=?, ordre=? WHERE id=?",
                       (designation, idx, sid_existing))
            kept_ids.add(sid_existing)
        else:
            db.execute("INSERT INTO piece_sous_pieces (piece_id, designation, ordre) VALUES (?,?,?)",
                       (pid, designation, idx))
    # Supprimer ceux qui ne sont plus présents
    for ex in existing:
        if ex["id"] not in kept_ids and (ex["designation"] or "").strip().lower() not in [
            (i.get("designation") or "").strip().lower() for i in items
        ]:
            db.execute("DELETE FROM piece_sous_pieces WHERE id=?", (ex["id"],))
    db.commit()
    return jsonify({"ok": True})


# ══ MESURES STRING (sur un CR) ══
# ══ MESURES STRING (sur un CR) ══
def _resolve_cr_id_from_iv(db, iv_id):
    """Trouve le CR associé à une intervention (le plus récent)."""
    cr = one(db.execute("SELECT id FROM comptes_rendus WHERE intervention_id=? ORDER BY id DESC LIMIT 1", (iv_id,)))
    return cr["id"] if cr else None

@app.route("/api/interventions/<int:iv_id>/string-mesures", methods=["GET"])
@require_auth
def get_string_mesures_by_iv(iv_id):
    """Version intervention-based (résout cr_id automatiquement, ou retourne données vides si pas de CR)."""
    db = get_db()
    cr_id = _resolve_cr_id_from_iv(db, iv_id)
    iv = one(db.execute("SELECT equipement_id FROM interventions WHERE id=?", (iv_id,)))
    if not iv: return jsonify([])
    eid = iv["equipement_id"]
    sid = current_societe_id()
    pieces = rows(db.execute("""
        SELECT p.id, p.type_piece FROM pieces p
        JOIN types_pieces tp ON tp.nom=p.type_piece AND tp.societe_id=?
        WHERE p.equipement_id=? AND tp.has_sous_pieces=1
    """, (sid, eid)))
    out = []
    for p in pieces:
        sps = rows(db.execute(
            "SELECT id, designation FROM piece_sous_pieces WHERE piece_id=? ORDER BY ordre, id",
            (p["id"],)
        ))
        for sp in sps:
            m = None
            if cr_id:
                m = one(db.execute(
                    "SELECT tension, intensite, resistance FROM cr_string_mesures WHERE cr_id=? AND sous_piece_id=?",
                    (cr_id, sp["id"])
                ))
            out.append({
                "sous_piece_id": sp["id"],
                ...
                "tension": (m and m.get("tension")) or "",
                "intensite": (m and m.get("intensite")) or "",
                "resistance": (m and m.get("resistance")) or "",
            })
    return jsonify(out)

@app.route("/api/interventions/<int:iv_id>/string-mesures", methods=["PUT"])
@require_role("admin", "manager", "tech")
def set_string_mesures_by_iv(iv_id):
    """Body : { items: [{sous_piece_id, tension, intensite, resistance}, ...] }
    Crée le CR si nécessaire."""
    d = request.json or {}
    items = d.get("items") or []
    if not isinstance(items, list):
        return jsonify({"error": "items doit être une liste"}), 400
    ...
    if not cr_id:
        # Créer un CR minimum
        cur = db.execute("INSERT INTO comptes_rendus (intervention_id, created_at) VALUES (?, datetime('now'))", (iv_id,))
        cr_id = cur.lastrowid
    for it in items:
        spid = to_int(it.get("sous_piece_id"))
        if not spid: continue
        db.execute("""INSERT INTO cr_string_mesures (cr_id, sous_piece_id, tension, intensite, resistance, updated_at)
                      VALUES (?, ?, ?, ?, ?, datetime('now'))
                      ON CONFLICT(cr_id, sous_piece_id) DO UPDATE SET
                        tension=excluded.tension, intensite=excluded.intensite,
                        resistance=excluded.resistance, updated_at=excluded.updated_at""",
                   (cr_id, spid, str(it.get("tension","") or ""), str(it.get("intensite","") or ""),
                    str(it.get("resistance","") or "")))
    db.commit()
    return jsonify({"ok": True})


@app.route("/api/cr/<int:cr_id>/string-mesures", methods=["GET"])
@require_auth
def get_string_mesures(cr_id):
    """Retourne toutes les sous-pièces concernées par ce CR (via la pièce critique de l'équipement)
    + les valeurs déjà saisies pour chacune.
    Retour : [{sous_piece_id, designation, tension, intensite, resistance}, ...]"""
    db = get_db()
    cr = one(db.execute("""SELECT cr.id, cr.intervention_id, i.equipement_id
                           FROM comptes_rendus cr JOIN interventions i ON cr.intervention_id=i.id
                           WHERE cr.id=?""", (cr_id,)))
    if not cr:
        return jsonify({"error": "CR introuvable"}), 404
    eid = cr.get("equipement_id")
    # Trouver les pièces de l'équipement dont le type a has_sous_pieces=1
    sid = current_societe_id()
    pieces = rows(db.execute("""
        SELECT p.id, p.type_piece FROM pieces p
        JOIN types_pieces tp ON tp.nom=p.type_piece AND tp.societe_id=?
...
@app.route("/api/cr/<int:cr_id>/string-mesures", methods=["PUT"])
@require_role("admin", "manager", "tech")
def set_string_mesures(cr_id):
    """Body : { items: [{sous_piece_id, tension, intensite, resistance}, ...] }"""
    d = request.json or {}
    items = d.get("items") or []
    if not isinstance(items, list):
...
def get_piece_single(pid):
    ...
    if not p:
        return jsonify({"error": "Pièce introuvable"}), 404
    # v218.171 : inclure les sous-pièces
    try:
        p["sous_pieces"] = rows(db.execute(
            "SELECT id, designation, ordre FROM piece_sous_pieces WHERE piece_id=? ORDER BY ordre, id",
            (pid,)
        ))
    except Exception:
...
def create_mesure_bloc():
    ...
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations", "mesures_batteries", "image", "equipement", "image_checklist", "string"):
        return jsonify({"error": "type invalide"}), 400
    ordre = int(d.get("ordre") or 0)
...
def get_intervention_mesures(iid):
    ...
    for b in blocs:
        ...
        # v218.171 : Pour les modules 'string', charger les sous-pièces des pièces du type "has_sous_pieces"
        if (b.get("type") or "") == "string":
            sid_ = current_societe_id()
            cr_id_ = _resolve_cr_id_from_iv(db, iid)
            pieces_sp = rows(db.execute("""
                SELECT p.id, p.type_piece FROM pieces p
                JOIN types_pieces tp ON tp.nom=p.type_piece AND tp.societe_id=?
                WHERE p.equipement_id=? AND tp.has_sous_pieces=1
            """, (sid_, iv["equipement_id"])))
            string_data = []
            for p in pieces_sp:
                sps_ = rows(db.execute(
                    "SELECT id, designation FROM piece_sous_pieces WHERE piece_id=? ORDER BY ordre, id",
                    (p["id"],)
                ))
                for sp in sps_:
                    ...
                    string_data.append({
                        ...
                        "designation": sp["designation"],
                        "type_piece": p["type_piece"],
                        "tension": (m_ and m_.get("tension")) or "",
                        "intensite": (m_ and m_.get("intensite")) or "",
                        "resistance": (m_ and m_.get("resistance")) or "",
                    })
            b["string_data"] = string_data
    # Valeurs saisies

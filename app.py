...
def get_equipements():
    ...
    for e in equips:
        ...
        e["gammes"] = rows(db.execute(
            "SELECT g.* FROM gammes g JOIN equipement_gammes eg ON g.id=eg.gamme_id WHERE eg.equipement_id=? ORDER BY g.nom",(e["id"],)))
...
def get_equipement(eid):
    db = get_db()
    e  = one(db.execute("SELECT e.*,p.nom AS projet_nom,p.numero_projet,g.nom AS gamme_nom,t.nom AS technique_nom FROM equipements e JOIN projets p ON e.projet_id=p.id LEFT JOIN gammes g ON e.gamme_id=g.id LEFT JOIN techniques t ON e.technique_id=t.id WHERE e.id=?",(eid,)))
    ...
    e["gammes"]    = rows(db.execute(
        "SELECT g.* FROM gammes g JOIN equipement_gammes eg ON g.id=eg.gamme_id WHERE eg.equipement_id=? ORDER BY g.nom",(eid,)))
    return jsonify(e)
...
def create_equipement():
    ...
    db = get_db()
    db.execute("INSERT INTO equipements (projet_id,designation,type_technique,localisation,marque,modele,puissance,numero_serie,in_out,date_mise_en_service,statut,technique,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
               (d["projet_id"],d["designation"],d["type_technique"],d.get("localisation",""),d.get("marque",""),
                ...
                d.get("date_mise_en_service") or None,d.get("statut","EN_SERVICE"),d.get("technique",""),d.get("notes","")))
...
def update_equipement(eid):
    ...
    db = get_db()
    sets,params=[],[]
    for f in ["designation","type_technique","localisation","marque","modele","puissance","numero_serie","in_out","date_mise_en_service","statut","gamme_id","technique_id","notes"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
    if not sets: return jsonify({"error":"Rien"}),400
    params.append(eid)
...
def create_technique():
    ...
    db = get_db()
    try:
        db.execute("INSERT INTO techniques (nom,description) VALUES (?,?)",
                   (d["nom"], d.get("description","")))
        db.commit()
        return jsonify({"id":db.execute("SELECT last_insert_rowid()").fetchone()[0]}),201
    except Exception as e: return jsonify({"error":str(e)}),400
...
def update_technique(tid):
    ...
    if not sets: return jsonify({"error":"Rien"}),400
    params.append(tid)
    db.execute(f"UPDATE techniques SET {','.join(sets)} WHERE id=?",params)
    db.commit(); return jsonify({"ok":True})
...
def delete_technique(tid):
    db = get_db()
    db.execute("DELETE FROM techniques WHERE id=?",(tid,))
    db.commit(); return jsonify({"ok":True})
...
def get_gammes():
    db = get_db()
    gammes = rows(db.execute("SELECT * FROM gammes ORDER BY nom"))
    for g in gammes:
        g["operations"] = rows(db.execute(
            "SELECT * FROM gamme_operations WHERE gamme_id=? ORDER BY ordre", (g["id"],)))
...
def get_gamme(gid):
    db = get_db()
    g = one(db.execute("SELECT * FROM gammes WHERE id=?", (gid,)))
    if not g: return jsonify({"error":"Non trouvé"}),404
    g["operations"] = rows(db.execute(
        "SELECT * FROM gamme_operations WHERE gamme_id=? ORDER BY ordre", (gid,)))
    return jsonify(g)
...
def create_gamme():
    d = request.json or {}
    if not d.get("nom") or not d.get("periodicite"):
        return jsonify({"error":"nom et periodicite requis"}),400
    db = get_db()
    db.execute("INSERT INTO gammes (nom,periodicite,temps) VALUES (?,?,?)",
               (d["nom"], d["periodicite"], d.get("temps","00h00")))
    db.commit()
    gid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
    for i, op in enumerate(d.get("operations",[])):
        if op.get("description"):
            db.execute("INSERT INTO gamme_operations (gamme_id,ordre,description) VALUES (?,?,?)",
                       (gid, i, op["description"]))
    db.commit()
...
def delete_gamme(gid):
    db = get_db()
    db.execute("DELETE FROM gammes WHERE id=?", (gid,))
    db.commit()
    return jsonify({"ok":True})
...
def generate_pdf(iid):
    try:
        ...
        return jsonify({"error":"Module rapport_pdf non disponible"}),500

    db  = get_db()
    i   = one(db.execute("""
        SELECT i.*,e.designation AS equip_nom,e.type_technique,e.localisation,e.marque,e.modele,
    ...
    def merge_field(field):
        vals = [cr.get(field,'') for cr in crs if cr.get(field,'')]
        return '\n'.join(vals) if vals else ''

    all_ivs,seen = [],set()
    for cr in crs:
        for iv in cr.get('intervenants',[]):
            key = (iv.get('nom',''),iv.get('date',''))
            if key not in seen:
                seen.add(key); all_ivs.append(iv)

    iv_names = ', '.join(set(iv.get('nom','') for iv in all_ivs if iv.get('nom','')))
    if not iv_names and i.get('technicien_nom'):
        iv_names = i['technicien_nom']

    date_iv = merge_field('date_intervention') or (i.get('date_realisation') or i.get('date_prevue') or now()[:10])
    date_iv = date_iv.split('\n')[0][:10]

    if i['type'] == 'DEPANNAGE':
        titre_rapport = "RAPPORT D'INTERVENTION - DEPANNAGE"
        type_label    = "DEPANNAGE"
    else:
        titre_rapport = "RAPPORT D'INTERVENTION"
        type_label    = "MAINTENANCE PREVENTIVE"

    equip_label = i.get('equip_nom','--')
    if i.get('marque') and i.get('modele'):
        equip_label += f" - {i['marque']} {i['modele']}"

    data = {
        'titre_rapport':     titre_rapport,
        'titre':             equip_label,
        'client':            i.get('client_nom','--'),
        'numero_projet':     i.get('numero_projet','--'),
        'numero_iv':         i.get('numero','--'),
        'type_label':        type_label,
        'date':              date_iv,
        'equipement':        equip_label,
        'localisation':      i.get('localisation','--'),
        'intervenants':      iv_names or '--',
        'intervenants_list': all_ivs,
        'observations':      merge_field('observations'),
        'actions_realisees': merge_field('actions_realisees'),
        'mesures':           merge_field('mesures'),
        'recommandations':   merge_field('recommandations'),
        'conclusion':        merge_field('conclusion'),
    }

    try:
        pdf_bytes = generate_rapport(data)
    except Exception as e:
        return jsonify({"error":f"Erreur PDF : {str(e)}"}),500

    filename = f"rapport_{i.get('numero','IV')}_{i.get('client_nom','SOCOM')}.pdf"

        ...
        if r["statut"]=="TERMINEE":
            try:
                ...
                pdf_bytes = generate_rapport({
                    ...
                    "sous_equipements": (rows(db.execute("SELECT designation, marque, type, numero_serie, nombre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id", (iv_full.get("equipement_id"),))) if iv_full.get("equipement_id") else []),
...
_COVER_DYNAMIC_FIELDS = [
    ...
    # v218.198 : heures réelles de début/fin
...
def _migrate_cover_pages(db):
    ...
    try:
        ...
        # v218.182 : Table cover_pages_mes pour les bons de mise en service
        db.execute("""
            CREATE TABLE IF NOT EXISTS cover_pages_mes (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                societe_id INTEGER NOT NULL UNIQUE,
                blocks_json TEXT NOT NULL DEFAULT '[]',
                header_blocks_json TEXT DEFAULT '[]',
        ...
        """)
        # Initialiser SOCOM avec un layout MES par défaut (clone du BP comme base)
        existing_mes = one(db.execute("SELECT id FROM cover_pages_mes WHERE societe_id=1"))
        if not existing_mes:
            db.execute("""INSERT INTO cover_pages_mes
                (societe_id, blocks_json, header_blocks_json, footer_blocks_json)
                VALUES (1, ?, ?, ?)""",
                (json.dumps(_COVER_DEFAULT_BLOCKS),
                 json.dumps(_HEADER_DEFAULT_BLOCKS),
                 json.dumps(_FOOTER_DEFAULT_BLOCKS)))
            logger.info("[cover-pages-mes] SOCOM (id=1) initialisée avec layouts MES par défaut")
...
def init_db():
    ...
    -- v218.176 : table equipement_cellules RESTAURÉE
    -- (avait été DROP par migration v218.160 qui tournait à chaque démarrage)
    ...
    CREATE INDEX IF NOT EXISTS idx_equipement_cellules ON equipement_cellules(equipement_id, ordre);
    ...
    -- v218.174 : valeurs du module "Mesures String" par intervention × bloc
    -- Chaque entrée stocke les lignes du tableau au format JSON dans data.
    CREATE TABLE IF NOT EXISTS iv_mesures_string (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        intervention_id INTEGER NOT NULL REFERENCES interventions(id) ON DELETE CASCADE,
        bloc_id INTEGER NOT NULL REFERENCES mesure_blocs(id) ON DELETE CASCADE,
        data TEXT NOT NULL DEFAULT '[]',
        updated_at TEXT DEFAULT (datetime('now')),
        UNIQUE(intervention_id, bloc_id)
    );
    CREATE INDEX IF NOT EXISTS idx_iv_mesures_string ON iv_mesures_string(intervention_id);
    -- v218.179 : valeurs du module "Borne de charge" : 1 entrée par (intervention, bloc, sous_eq)
    -- conforme et non_conforme sont 0 ou 1 ; les sous-équipements eux-mêmes sont lus depuis equipement_sous_equipements.
    CREATE TABLE IF NOT EXISTS iv_borne_charge (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        intervention_id INTEGER NOT NULL REFERENCES interventions(id) ON DELETE CASCADE,
        bloc_id INTEGER NOT NULL REFERENCES mesure_blocs(id) ON DELETE CASCADE,
        sous_eq_id INTEGER NOT NULL REFERENCES equipement_sous_equipements(id) ON DELETE CASCADE,
        conforme INTEGER NOT NULL DEFAULT 0,
        non_conforme INTEGER NOT NULL DEFAULT 0,
        updated_at TEXT DEFAULT (datetime('now')),
        UNIQUE(intervention_id, bloc_id, sous_eq_id)
    );
    CREATE INDEX IF NOT EXISTS idx_iv_borne_charge ON iv_borne_charge(intervention_id, bloc_id);
    ...
    # v218.198 : heures réelles de début/fin (remplies au démarrage/terminaison)
    _add_col("interventions", "heure_debut_reel", "TEXT DEFAULT ''")
    _add_col("interventions", "heure_fin_reel", "TEXT DEFAULT ''")
    ...
    # v218.225 : migration réparatrice idempotente — toute occupation ayant un technicien_id
    # mais AUCUNE liaison dans occupation_techniciens (cas des congés validés avant v218.225,
    # qui n'inséraient pas la liaison → invisibles dans le pointage). Crée la liaison manquante.
    try:
        orphan_occs = rows(db.execute("""
            SELECT o.id, o.technicien_id
            FROM occupations o
            WHERE o.technicien_id IS NOT NULL
              AND NOT EXISTS (
                  SELECT 1 FROM occupation_techniciens ot WHERE ot.occupation_id = o.id
              )
        """))
        for o in orphan_occs:
            db.execute(
                "INSERT OR IGNORE INTO occupation_techniciens (occupation_id, technicien_id) VALUES (?,?)",
                (o["id"], o["technicien_id"]))
        if orphan_occs:
            db.commit()
            logger.info(f"[MIG_OCC] v218.225 : {len(orphan_occs)} occupation(s) orpheline(s) reliée(s) (congés)")
    except Exception as e:
        logger.warning(f"[MIG_OCC] v218.225 réparation orphelines échec : {e}")
        ...
        if r and r.get("sql") and "ASTREINTE" not in r["sql"]:
            ...
            db.executescript("""
                CREATE TABLE interventions_new3 (
                    ...
                    equipe_id INTEGER REFERENCES equipes(id),
                    type TEXT NOT NULL DEFAULT 'MAINTENANCE'
                        CHECK(type IN ('MAINTENANCE','DEPANNAGE','ASTREINTE','MISE_EN_SERVICE')),
                    statut TEXT NOT NULL DEFAULT 'PLANIFIEE'
                        CHECK(statut IN ('A_PLANIFIER','PLANIFIEE','EN_COURS','TERMINEE','ANNULEE')),
                    date_prevue TEXT, date_realisation TEXT,
                    description TEXT DEFAULT '', rapport TEXT DEFAULT '',
    ...
    # v218.180 : type de rapport (extension de is_bc)
    # Valeurs : 'maintenance' (par défaut, équivalent à is_bc=0), 'depannage' (= is_bc=1), 'mise_en_service'
    # Note : is_bc reste pour rétrocompat, mais rapport_kind est plus extensible.
    _add_col("mesure_blocs", "rapport_kind", "TEXT DEFAULT 'maintenance'")
    db.execute("CREATE INDEX IF NOT EXISTS idx_mb_kind ON mesure_blocs(rapport_kind, societe_id)")
    # Migration : aligner rapport_kind avec is_bc pour les modules existants
    try:
        db.execute("UPDATE mesure_blocs SET rapport_kind='depannage' WHERE is_bc=1 AND (rapport_kind IS NULL OR rapport_kind='maintenance')")
        db.commit()
    except Exception: pass
    ...
    # v218.179 : colonne numero_serie sur sous-équipements
    _add_col("equipement_sous_equipements", "numero_serie", "TEXT DEFAULT ''")
    ...
    # v218.160 : Suppression des données obsolètes (DÉSACTIVÉE en v218.176)
    # Cette migration purgeait cr_tableaux + equipement_tableaux + equipement_cellules à CHAQUE démarrage,
    # ce qui empêchait toute persistance des sous-équipements. La purge a déjà eu lieu depuis longtemps,
    # cette migration est donc désactivée définitivement.
    # try:
    ...
    # v218.181 : Élargir CHECK interventions.type pour inclure MISE_EN_SERVICE
    try:
        schema_row = one(db.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='interventions'"
        ))
        sql_text = (schema_row.get("sql") or "") if schema_row else ""
        if sql_text and "CHECK" in sql_text and "type IN" in sql_text and "MISE_EN_SERVICE" not in sql_text:
            logger.info("[MIG v218.181] CHECK constraint sur interventions.type doit être élargi…")
            db.execute("PRAGMA foreign_keys=OFF")
            db.execute("BEGIN TRANSACTION")
            try:
                # Récupérer colonnes existantes pour copie
                existing_cols = [c["name"] for c in rows(db.execute("PRAGMA table_info(interventions)"))]
                # Recréer la table avec le bon CHECK
                db.execute("""
                    CREATE TABLE interventions_new (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        numero TEXT UNIQUE,
                ...
                """)
                new_cols = ["id","numero","equipement_id","technicien_id","equipe_id","type","statut","date_prevue",
                            "date_realisation","description","rapport","created_at","heure_prevue","tableau_id"]
                common = [c for c in new_cols if c in existing_cols]
                cols_str = ",".join(common)
                db.execute(f"INSERT INTO interventions_new ({cols_str}) SELECT {cols_str} FROM interventions")
                # Copier aussi les colonnes ajoutées dynamiquement par _add_col
                added_cols = [c for c in existing_cols if c not in new_cols]
                for col in added_cols:
                    # Récupérer le type/default de la colonne
                    col_info = one(db.execute(
                        f"SELECT type, dflt_value, \"notnull\" FROM pragma_table_info('interventions') WHERE name=?",
                        (col,)
                    ))
                    if col_info:
                        coltype = col_info.get("type") or "TEXT"
                        default = col_info.get("dflt_value")
                        notnull = col_info.get("notnull") or 0
                        ddl = f"ALTER TABLE interventions_new ADD COLUMN {col} {coltype}"
                        if notnull: ddl += " NOT NULL"
                        if default is not None: ddl += f" DEFAULT {default}"
                        try:
                            db.execute(ddl)
                            db.execute(f"UPDATE interventions_new SET {col}=(SELECT {col} FROM interventions WHERE interventions.id=interventions_new.id)")
                        except Exception as e_col:
                            logger.error(f"[MIG v218.181] copie col {col} échec: {e_col}")
                db.execute("DROP TABLE interventions")
                db.execute("ALTER TABLE interventions_new RENAME TO interventions")
                db.execute("COMMIT")
                logger.info("[MIG v218.181] CHECK élargi avec MISE_EN_SERVICE")
            except Exception as e_inner:
                db.execute("ROLLBACK")
                logger.error(f"[MIG v218.181] échec: {e_inner}")
            db.execute("PRAGMA foreign_keys=ON")
    except Exception as e:
        logger.error(f"[MIG v218.181] détection échec: {e}")
...
def set_cover_page():
    ...
    d = request.json or {}
    # v218.185 : ajout equipment_image
    valid_cover_types = {"title", "subtitle", "image", "equipment_image", "info_box", "text", "spacer", "separator",
...
def _cover_bc_validators():
    ...
    # v218.185 : ajout equipment_image (insère automatiquement la photo de l'équipement du bon)
    valid_cover_types = {"title", "subtitle", "image", "equipment_image", "info_box", "text", "spacer", "separator",
                         "gamme_maintenance", "observations"}
    valid_hf_types = {"row", "text", "image", "spacer", "separator"}
    valid_cols = {"left", "center", "right"}
...
# v218.182 : Page de garde pour les bons MISE EN SERVICE
@app.route("/api/cover-page-mes", methods=["GET"])
@require_auth
def get_cover_page_mes():
    """Page de garde des bons MISE EN SERVICE (1 layout par société)."""
    sid = current_societe_id()
    db = get_db()
    row = one(db.execute("""SELECT blocks_json, header_blocks_json, footer_blocks_json
                            FROM cover_pages_mes WHERE societe_id=?""", (sid,)))
    if not row:
        return jsonify({
            "societe_id": sid,
            "blocks": list(_COVER_DEFAULT_BLOCKS),
...
@app.route("/api/cover-page-mes", methods=["PUT"])
@require_role("admin")
def set_cover_page_mes():
    """Met à jour la page de garde MES. Body: { blocks?, header_blocks?, footer_blocks? }."""
    d = request.json or {}
    valid_cover_types, valid_hf_types, valid_cols = _cover_bc_validators()
    ...
    if existing:
        db.execute("""UPDATE cover_pages_mes SET
                        blocks_json=?, header_blocks_json=?, footer_blocks_json=?,
                        updated_at=datetime('now')
                      WHERE societe_id=?""",
                   (json.dumps(cur_cover), json.dumps(cur_header), json.dumps(cur_footer), sid))
        ...
        db.execute("""INSERT INTO cover_pages_mes
                   ...
                   (sid, json.dumps(cur_cover), json.dumps(cur_header), json.dumps(cur_footer)))
    db.commit()
    log_action(request.user, "UPDATE", "cover-page-mes", sid, f"sid={sid}",
               f"cover={len(cur_cover)} header={len(cur_header)} footer={len(cur_footer)}")
    return jsonify({"ok": True, "blocks": cur_cover, "header_blocks": cur_header, "footer_blocks": cur_footer})
...
def get_equipement(eid):
    ...
    # v218.157 : Sous-équipements universels (toutes techniques)
    try:
        e["sous_equipements"]=rows(db.execute("SELECT id,designation,marque,type,numero_serie,nombre,ordre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre,id",(eid,)))
    except Exception:
        e["sous_equipements"]=[]
...
# Sous-équipements universels (toutes techniques) : désignation, marque, type, n°série, nombre
@app.route("/api/equipements/<int:eid>/sous-equipements", methods=["GET"])
@require_auth
def list_sous_equipements(eid):
    db = get_db()
    return jsonify(rows(db.execute(
        "SELECT id, designation, marque, type, numero_serie, nombre, ordre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id",
        (eid,)
    )))
...
def set_sous_equipements(eid):
    ...
    Body : { items: [{designation, marque, type, numero_serie, nombre}, ...] }"""
    d = request.json or {}
    ...
    for idx, item in enumerate(items):
        ...
        numero_serie = (item.get("numero_serie") or "").strip()
        try:
            ...
            nombre = 1
        db.execute(
            "INSERT INTO equipement_sous_equipements (equipement_id, designation, marque, type, numero_serie, nombre, ordre) VALUES (?,?,?,?,?,?,?)",
            (eid, designation, marque, type_, numero_serie, nombre, idx)
        )
        inserted += 1
    db.commit()
...
# ══ MESURES STRING (v218.174) ══
# Module "Mesures String" : tableau libre 5 colonnes, lignes ajoutables par le tech.
# Stockage : 1 entrée par (intervention, bloc) avec data = JSON [{designation, tension, courant, resistance, nbr_panneaux}, ...]
@app.route("/api/interventions/<int:iv_id>/mesures-string/<int:bloc_id>", methods=["GET"])
@require_auth
def get_iv_mesures_string(iv_id, bloc_id):
    db = get_db()
    r = one(db.execute(
        "SELECT data FROM iv_mesures_string WHERE intervention_id=? AND bloc_id=?",
        (iv_id, bloc_id)
    ))
    try:
        data = json.loads(r["data"]) if r and r.get("data") else []
    except Exception:
        data = []
    return jsonify({"lignes": data})

@app.route("/api/interventions/<int:iv_id>/mesures-string/<int:bloc_id>", methods=["PUT"])
@require_auth
def set_iv_mesures_string(iv_id, bloc_id):
    """Body : { lignes: [{designation, tension, courant, resistance, nbr_panneaux}, ...] }"""
    d = request.json or {}
    lignes = d.get("lignes") or []
    if not isinstance(lignes, list):
        return jsonify({"error": "lignes doit être une liste"}), 400
    # Normalisation : on garde les 5 champs et on caste en string
    clean = []
    for l in lignes:
        if not isinstance(l, dict): continue
        clean.append({
            "designation": str(l.get("designation","") or ""),
            "tension": str(l.get("tension","") or ""),
            "courant": str(l.get("courant","") or ""),
            "resistance": str(l.get("resistance","") or ""),
            "nbr_panneaux": str(l.get("nbr_panneaux","") or ""),
        })
    data_json = json.dumps(clean, ensure_ascii=False)
    db = get_db()
    db.execute("""INSERT INTO iv_mesures_string (intervention_id, bloc_id, data, updated_at)
                  VALUES (?, ?, ?, datetime('now'))
                  ON CONFLICT(intervention_id, bloc_id) DO UPDATE SET
                    data=excluded.data, updated_at=excluded.updated_at""",
               (iv_id, bloc_id, data_json))
    db.commit()
    return jsonify({"ok": True, "nb_lignes": len(clean)})


# ══ BORNE DE CHARGE (v218.179) ══
# Module qui liste les sous-équipements d'un équipement et permet au tech
# de cocher Conforme / Non conforme pour chacun.
@app.route("/api/interventions/<int:iv_id>/borne-charge/<int:bloc_id>", methods=["GET"])
@require_auth
def get_iv_borne_charge(iv_id, bloc_id):
    """Retourne la liste des sous-équipements de l'équipement de l'intervention,
    avec les valeurs cochées (si déjà saisies)."""
    db = get_db()
    iv = one(db.execute("SELECT equipement_id FROM interventions WHERE id=?", (iv_id,)))
    if not iv:
        return jsonify({"error": "Intervention introuvable"}), 404
    eid = iv["equipement_id"]
    sous_eqs = rows(db.execute(
        "SELECT id, designation, marque, type, numero_serie FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id",
        (eid,)
    ))
    out = []
    for se in sous_eqs:
        val = one(db.execute(
            "SELECT conforme, non_conforme FROM iv_borne_charge WHERE intervention_id=? AND bloc_id=? AND sous_eq_id=?",
            (iv_id, bloc_id, se["id"])
        ))
        out.append({
            "sous_eq_id": se["id"],
            "designation": se.get("designation") or "",
            "marque": se.get("marque") or "",
            "type": se.get("type") or "",
            "numero_serie": se.get("numero_serie") or "",
            "conforme": bool(val and val.get("conforme")),
            "non_conforme": bool(val and val.get("non_conforme")),
        })
    return jsonify({"lignes": out})

@app.route("/api/interventions/<int:iv_id>/borne-charge/<int:bloc_id>", methods=["PUT"])
@require_auth
def set_iv_borne_charge(iv_id, bloc_id):
    """Body : { lignes: [{sous_eq_id, conforme, non_conforme}, ...] }"""
    d = request.json or {}
    lignes = d.get("lignes") or []
    if not isinstance(lignes, list):
        return jsonify({"error": "lignes doit être une liste"}), 400
    db = get_db()
    for l in lignes:
        sid_ = to_int(l.get("sous_eq_id"))
        if not sid_: continue
        conf = 1 if l.get("conforme") else 0
        nconf = 1 if l.get("non_conforme") else 0
        db.execute("""INSERT INTO iv_borne_charge (intervention_id, bloc_id, sous_eq_id, conforme, non_conforme, updated_at)
                      VALUES (?, ?, ?, ?, ?, datetime('now'))
                      ON CONFLICT(intervention_id, bloc_id, sous_eq_id) DO UPDATE SET
                        conforme=excluded.conforme, non_conforme=excluded.non_conforme, updated_at=excluded.updated_at""",
                   (iv_id, bloc_id, sid_, conf, nconf))
    db.commit()
    return jsonify({"ok": True, "nb_lignes": len(lignes)})
...
def get_interventions():
    ...
    # v218.202 : ajouter aussi la liste des NOMS des techniciens pour affichage planning
    for r in result:
        ...
        # v218.202 : récupérer les noms pour affichage tuile planning
        # v218.203 : la table utilisateurs n'a que la colonne 'nom' (pas 'prenom')
        if r["technicien_ids"]:
            try:
                ph = ",".join("?" * len(r["technicien_ids"]))
                tech_noms_rows = rows(db.execute(
                    f"SELECT id, nom FROM utilisateurs WHERE id IN ({ph}) ORDER BY nom",
                    tuple(r["technicien_ids"])
                ))
                r["technicien_noms"] = [
                    (tn.get("nom") or "").strip()
                    for tn in tech_noms_rows
                    if tn.get("nom")
                ]
            except Exception:
                r["technicien_noms"] = []
...
def create_intervention():
    ...
    # v218.18 : ajout du type ASTREINTE
    # v218.181 : ajout du type MISE_EN_SERVICE
    if type_iv not in ("MAINTENANCE","DEPANNAGE","ASTREINTE","MISE_EN_SERVICE"):
        return jsonify({"error":"Type invalide"}),400
    try:
        # Préfixe numéro : BP (maintenance), BC (dépannage), BA (astreinte), BM (mise en service)
        prefix_map = {"MAINTENANCE": "BP", "DEPANNAGE": "BC", "ASTREINTE": "BA", "MISE_EN_SERVICE": "BM"}
...
def update_intervention(iid):
    d=request.json or {}; db=get_db(); sets,params=[],[]
    # Valider le type si fourni (v218.18 : ASTREINTE accepté ; v218.181 : MISE_EN_SERVICE accepté)
    if "type" in d and d["type"] not in ("MAINTENANCE","DEPANNAGE","ASTREINTE","MISE_EN_SERVICE"):
        return jsonify({"error":"Type invalide"}),400
    ...
    for f in ["technicien_id","equipement_id","equipe_id","type","statut","date_prevue","heure_prevue","date_realisation","description","rapport","tableau_id","gamme_id","heure_debut_reel","heure_fin_reel"]:
        if f in d: sets.append(f"{f}=?"); params.append(d[f])
    # v218.198 : auto-remplir heure_debut_reel / heure_fin_reel selon transition de statut
    if d.get("statut") == "EN_COURS":
        # Si heure_debut_reel vide, la remplir avec l'heure actuelle
        iv_curr2 = one(db.execute("SELECT heure_debut_reel FROM interventions WHERE id=?", (iid,))) or {}
        if not (iv_curr2.get("heure_debut_reel") or "").strip() and "heure_debut_reel" not in d:
            from datetime import datetime as _dt
            _now_hm = _dt.now().strftime("%H:%M")
            sets.append("heure_debut_reel=?"); params.append(_now_hm)
    if d.get("statut") == "TERMINEE":
        # Remplir heure_fin_reel
        iv_curr3 = one(db.execute("SELECT heure_fin_reel FROM interventions WHERE id=?", (iid,))) or {}
        if not (iv_curr3.get("heure_fin_reel") or "").strip() and "heure_fin_reel" not in d:
            from datetime import datetime as _dt
            _now_hm = _dt.now().strftime("%H:%M")
            sets.append("heure_fin_reel=?"); params.append(_now_hm)
    # Vérification : pour terminer un bon, il faut au minimum un compte rendu
    # v218.198 : les bons MISE_EN_SERVICE n'ont pas besoin de CR
    if "statut" in d and d["statut"]=="TERMINEE":
        iv_row = one(db.execute("SELECT type FROM interventions WHERE id=?", (iid,)))
        _iv_type_chk = (iv_row.get("type") if iv_row else "") or ""
        if _iv_type_chk != "MISE_EN_SERVICE":
            nb_cr = (one(db.execute("SELECT COUNT(*) AS n FROM comptes_rendus WHERE intervention_id=?", (iid,))) or {}).get("n", 0)
            if nb_cr == 0:
                type_label = "maintenance" if _iv_type_chk == "MAINTENANCE" else "dépannage"
...
def valider_demande_conge(dcid):
    ...
    while cur <= d1:
        if cur.weekday() < 5:  # lun-ven
            ...
            new_oid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
            # v218.225 : créer la liaison multi-tech (sinon l'occupation n'apparaît pas
            # dans le pointage qui lit via occupation_techniciens en JOIN strict).
            db.execute(
                "INSERT OR IGNORE INTO occupation_techniciens (occupation_id, technicien_id) VALUES (?,?)",
                (new_oid, dc["technicien_id"]))
            created_ids.append(new_oid)
...
def _hm_to_minutes(s):
    """Convertit 'HH:MM' en minutes, ou None si invalide."""
    if not s:
        return None
    m = re.match(r'^\s*(\d{1,2})[:hH](\d{2})', str(s))
    if not m:
        return None
    return int(m.group(1)) * 60 + int(m.group(2))


def _bm_pointage_lignes(db, sid, date_debut, date_fin, tech_filter, allowed_ids):
    """v218.224 : Lignes de pointage pour les bons MISE_EN_SERVICE (BM).
    Les BM n'ont pas forcément de CR (ils peuvent être terminés sans), donc leurs heures
    ne remontent pas via cr_intervenants. On les comptabilise ici via :
      - durée = heure_fin_reel - heure_debut_reel (fallback heure_prevue si besoin)
      - techniciens via intervention_techniciens (fallback technicien_id principal)
    On exclut les BM qui ont déjà des intervenants dans un CR (sinon double comptage).
    Retourne une liste de lignes au même format que les lignes d'intervention.
    """
    # Récupérer les BM terminés OU avec des heures réelles, dans la période.
    # La date de référence = date_realisation si dispo, sinon date_prevue.
    sql_bm = """
        SELECT i.id, i.numero AS num_bon, i.type AS type_bon,
               i.heure_debut_reel, i.heure_fin_reel, i.heure_prevue,
               COALESCE(NULLIF(i.date_realisation,''), i.date_prevue) AS date_pointage,
               i.technicien_id,
               e.designation AS equipement, p.nom AS projet, p.numero_projet AS num_projet
        FROM interventions i
        JOIN equipements e ON i.equipement_id = e.id
        JOIN projets p ON e.projet_id = p.id
        WHERE i.societe_id = ?
          AND i.type = 'MISE_EN_SERVICE'
          AND COALESCE(NULLIF(i.date_realisation,''), i.date_prevue) BETWEEN ? AND ?
    """
    bms = rows(db.execute(sql_bm, (sid, date_debut, date_fin)))
    if not bms:
        return []
    ...
    for bm in bms:
        iid = bm["id"]
        # Exclure si le BM a déjà des intervenants dans un CR (évite double comptage)
        has_cr_int = one(db.execute("""
            SELECT 1 AS x FROM cr_intervenants ci
            JOIN comptes_rendus cr ON ci.cr_id = cr.id
            WHERE cr.intervention_id = ? LIMIT 1
        """, (iid,)))
        if has_cr_int:
            continue

        # Calcul de la durée en heures
        deb = bm.get("heure_debut_reel") or bm.get("heure_prevue") or ""
        fin = bm.get("heure_fin_reel") or ""
        mn_deb = _hm_to_minutes(deb)
        mn_fin = _hm_to_minutes(fin)
        if mn_deb is not None and mn_fin is not None and mn_fin > mn_deb:
            total_h = round((mn_fin - mn_deb) / 60.0, 2)
        else:
            # Pas d'heures exploitables → on ne comptabilise pas de durée (0h),
            # mais on liste quand même le bon pour qu'il apparaisse.
            total_h = 0.0

        # Techniciens : table de liaison, fallback technicien principal
        tech_rows = rows(db.execute("""
            SELECT it.utilisateur_id, u.nom
            FROM intervention_techniciens it
            JOIN utilisateurs u ON it.utilisateur_id = u.id
            WHERE it.intervention_id = ?
        """, (iid,)))
        if not tech_rows and bm.get("technicien_id"):
            ur = one(db.execute("SELECT id AS utilisateur_id, nom FROM utilisateurs WHERE id=?",
                                (bm["technicien_id"],)))
            if ur:
                tech_rows = [ur]
        if not tech_rows:
        ...
        for tr in tech_rows:
            uid = tr["utilisateur_id"]
            # Filtres d'autorisation
            if tech_filter and uid != to_int(tech_filter):
                continue
            if (not tech_filter) and (allowed_ids is not None) and (uid not in allowed_ids):
                continue
            out.append({
                "date": bm["date_pointage"],
                "heure_debut": deb,
                "heure_fin": fin,
                "total_heures": total_h,
                "utilisateur_id": uid,
                "nom_tech": tr["nom"] or "Inconnu",
                "cr_id": None,
                "intervention_id": iid,
                "num_bon": bm["num_bon"],
                "type_bon": bm["type_bon"],
                "equipement": bm["equipement"],
                "projet": bm["projet"],
                "num_projet": bm.get("num_projet") or "",
                "source": "intervention",
            })
    return out
...
def pointage_mensuel(annee, mois):
    ...
    # v218.224 : ajouter les bons MISE_EN_SERVICE (BM) qui n'ont pas de CR
    bm_lignes = _bm_pointage_lignes(db, sid, date_debut, date_fin, tech_filter, allowed_ids)
    lignes.extend(bm_lignes)
...
def pointage_export(annee, mois):
    ...
    # v218.224 : ajouter les bons MISE_EN_SERVICE (BM) sans CR (tous techs, pas de restriction d'équipe à l'export admin/manager)
    bm_lignes = _bm_pointage_lignes(db, sid, date_debut, date_fin, None, None)
    all_lignes = all_lignes + bm_lignes
...
def pointage_hebdo(annee, semaine):
    ...
    # v218.224 : ajouter les bons MISE_EN_SERVICE (BM) sans CR
    bm_lignes = _bm_pointage_lignes(db, sid, date_debut, date_fin, tech_filter, allowed_ids)
    lignes.extend(bm_lignes)
...
def pointage_fiche_hebdo(annee, semaine, technicien_id):
    ...
    for r in cr_rows:
        try:
            ...
            continue
    # v218.224 : Bons MISE_EN_SERVICE (BM) sans CR pour ce technicien
    _sid_bm = current_societe_id()
    bm_rows = _bm_pointage_lignes(db, _sid_bm, date_debut, date_fin, technicien_id, None)
    for r in bm_rows:
        try:
            d = _dt.strptime(r["date"][:10], "%Y-%m-%d").date()
            jidx = (d - mon).days
...
def planning_me():
    ...
    # v218.219 : enrichir avec la liste des techniciens (pour affichage sur tuiles mobile)
    #   - Interventions : `technicien_noms` (liste) via intervention_techniciens
    #   - Occupations : `occupation_tech_noms` (liste) via occupation_techniciens
    try:
        iv_ids_set = set(it.get("intervention_id") for it in items if it.get("intervention_id"))
        occ_ids_set = set(it.get("occupation_id") for it in items if it.get("source") == "occupation" and it.get("occupation_id"))
        # Map intervention_id → liste noms (table de liaison)
        iv_tech_map = {}
        if iv_ids_set:
            placeholders = ",".join(["?"] * len(iv_ids_set))
            rows_iv = rows(db.execute(f"""
                SELECT it.intervention_id, u.nom
                FROM intervention_techniciens it
                JOIN utilisateurs u ON it.utilisateur_id = u.id
                WHERE it.intervention_id IN ({placeholders}) AND u.actif=1
                ORDER BY u.nom
            """, list(iv_ids_set)))
            for r in rows_iv:
                iv_tech_map.setdefault(r["intervention_id"], []).append(r["nom"])
            # Fallback : si pas de liaison, prendre le technicien principal
            missing_ivs = [iid for iid in iv_ids_set if iid not in iv_tech_map]
            if missing_ivs:
                ph = ",".join(["?"] * len(missing_ivs))
                rows_main = rows(db.execute(f"""
                    SELECT i.id, u.nom
                    FROM interventions i
                    LEFT JOIN utilisateurs u ON i.technicien_id = u.id
                    WHERE i.id IN ({ph})
                ...
                for r in rows_main:
                    if r.get("nom"):
                        iv_tech_map[r["id"]] = [r["nom"]]
        # Map occupation_id → liste noms (table de liaison)
        occ_tech_map = {}
        if occ_ids_set:
            ph = ",".join(["?"] * len(occ_ids_set))
            rows_occ = rows(db.execute(f"""
                SELECT ot.occupation_id, u.nom
                FROM occupation_techniciens ot
                JOIN utilisateurs u ON ot.technicien_id = u.id
                WHERE ot.occupation_id IN ({ph}) AND u.actif=1
                ORDER BY u.nom
            """, list(occ_ids_set)))
            for r in rows_occ:
                occ_tech_map.setdefault(r["occupation_id"], []).append(r["nom"])
        # Injecter sur chaque item
        for it in items:
            iid = it.get("intervention_id")
            if iid and iv_tech_map.get(iid):
                it["technicien_noms"] = iv_tech_map[iid]
            if it.get("source") == "occupation":
                oid = it.get("occupation_id")
                if oid and occ_tech_map.get(oid):
                    it["occupation_tech_noms"] = occ_tech_map[oid]
    except Exception as ex:
        logger.exception(f"[planning_me] Enrichissement technicien_noms échec : {ex}")
...
def get_mesure_blocs():
    ...
    db = get_db()
    where = []; params = []
    # v218.180 : rapport_kind = 'maintenance' (BP), 'depannage' (BC), ou 'mise_en_service' (MES)
    rapport_kind = request.args.get("rapport_kind", "")
    is_bc_param = request.args.get("is_bc", "0")
    is_bc = is_bc_param in ("1", "true", "True", "yes")
    sid = current_societe_id()
    if rapport_kind == "mise_en_service":
        # MES : globaux par société + filtrage par technique/sous_type comme BP
        where = ["mb.rapport_kind='mise_en_service'", "mb.societe_id=?"]
        params = [sid]
        technique_id = request.args.get("technique_id")
    ...
    elif rapport_kind == "depannage" or is_bc:
        # Modules BC : globaux pour la société
        where = ["mb.is_bc=1", "mb.societe_id=?"]
        params = [sid]
    else:
        # Modules BP (maintenance) : exclure les BC + MES + filtrer par technique/sous_type si fourni
        where.append("(mb.is_bc=0 OR mb.is_bc IS NULL)")
        where.append("(mb.rapport_kind='maintenance' OR mb.rapport_kind IS NULL)")
        technique_id = request.args.get("technique_id")
        sous_type_id = request.args.get("sous_type_id")
        if technique_id:
            where.append("mb.technique_id=?"); params.append(int(technique_id))
...
def create_mesure_bloc():
    ...
    d = request.json or {}
    # v218.180 : rapport_kind explicite remplace progressivement is_bc
    rapport_kind = (d.get("rapport_kind") or "").strip().lower()
    is_bc = bool(d.get("is_bc"))
    # Compat ascendante : is_bc=true → rapport_kind='depannage'
    if not rapport_kind:
        rapport_kind = "depannage" if is_bc else "maintenance"
    if rapport_kind not in ("maintenance", "depannage", "mise_en_service"):
        rapport_kind = "maintenance"
    is_bc = (rapport_kind == "depannage")
    nom = (d.get("nom") or "").strip()
    if not nom:
        return jsonify({"error": "nom requis"}), 400
    if rapport_kind == "depannage":
        # Mode BC : pas de technique/sous_type, on stocke avec societe_id
        technique_id = None
        sous_type_id = None
    else:
        # BP (maintenance) ou MES (mise en service) : besoin de technique + sous_type
        technique_id = d.get("technique_id")
        sous_type_id = d.get("sous_type_id")
        if not technique_id or not sous_type_id:
            return jsonify({"error": "technique_id et sous_type_id requis (ou rapport_kind='depannage')"}), 400
    type_module = (d.get("type") or "tableau").strip().lower()
    if type_module not in ("tableau", "texte", "checklist", "graphique", "preconisations", "mesures_batteries", "image", "equipement", "image_checklist", "mesures_string", "borne_charge"):
        return jsonify({"error": "type invalide"}), 400
    ordre = int(d.get("ordre") or 0)
    ...
    db.execute("""INSERT INTO mesure_blocs
                  (technique_id, sous_type_id, nom, type, ordre, field_options, largeur, page_num, icon, is_bc, rapport_kind, societe_id)
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
               (technique_id, sous_type_id, nom, type_module, ordre, bloc_fopts, largeur, page_num, icon,
                1 if is_bc else 0, rapport_kind, sid))
    bid = db.execute("SELECT last_insert_rowid()").fetchone()[0]
...
def get_intervention_mesures(iid):
    ...
    v218.190 : gère explicitement les 3 types : MAINTENANCE / DEPANNAGE / MISE_EN_SERVICE."""
    db = get_db()
    iv = one(db.execute(
        "SELECT i.id, i.type, i.equipement_id, i.societe_id FROM interventions i WHERE i.id = ?",
        (iid,)
    ))
    ...
    technique_id = None
    sous_type_id = None
    sous_type_label = None
    blocs = []

    if iv_type == "DEPANNAGE":
        # Modules globaux par société (is_bc=1)
        blocs = rows(db.execute("""
            SELECT mb.id, mb.nom, mb.ordre, mb.type, mb.field_options, mb.largeur, mb.page_num, mb.icon FROM mesure_blocs mb
            WHERE mb.is_bc=1 AND mb.societe_id=?
            ORDER BY mb.page_num, mb.ordre, mb.nom
        """, (sid,)))
        logger.info(f"[mesures GET DEPANNAGE] iid={iid} sid={sid} blocs={len(blocs)}")
    elif iv_type == "MISE_EN_SERVICE":
        # MES : technique de l'équipement × sous-type "Mise en service"
        technique_id = _resolve_technique_id(iv["equipement_id"])
        st = one(db.execute("SELECT id, nom FROM maintenance_sous_types WHERE LOWER(nom)='mise en service'"))
        sous_type_id = st["id"] if st else None
        sous_type_label = st["nom"] if st else None
        logger.info(f"[mesures GET MES] iid={iid} eq={iv['equipement_id']} tech={technique_id} st={sous_type_id} sid={sid}")
        if technique_id and sous_type_id:
            blocs = rows(db.execute("""
                SELECT mb.id, mb.nom, mb.ordre, mb.type, mb.field_options, mb.largeur, mb.page_num, mb.icon FROM mesure_blocs mb
                WHERE mb.technique_id=? AND mb.sous_type_id=? AND mb.rapport_kind='mise_en_service' AND mb.societe_id=?
                ORDER BY mb.page_num, mb.ordre, mb.nom
            """, (technique_id, sous_type_id, sid)))
        logger.info(f"[mesures GET MES] blocs trouvés={len(blocs)}")
    else:
        # MAINTENANCE : technique + sous-type déduit des gammes
        technique_id = _resolve_technique_id(iv["equipement_id"])
        ...
        if technique_id and sous_type_id:
            blocs = rows(db.execute("""
                SELECT mb.id, mb.nom, mb.ordre, mb.type, mb.field_options, mb.largeur, mb.page_num, mb.icon FROM mesure_blocs mb
                WHERE mb.technique_id=? AND mb.sous_type_id=? AND (mb.is_bc=0 OR mb.is_bc IS NULL) AND (mb.rapport_kind='maintenance' OR mb.rapport_kind IS NULL)
                ORDER BY mb.page_num, mb.ordre, mb.nom
            """, (technique_id, sous_type_id)))

    # Si aucun bloc applicable
    if not blocs:
    ...
    for b in blocs:
        ...
        # v218.174 : Pour les modules 'mesures_string', charger les lignes saisies
        if (b.get("type") or "") == "mesures_string":
            r_ms = one(db.execute(
                "SELECT data FROM iv_mesures_string WHERE intervention_id=? AND bloc_id=?",
                (iid, b["id"])
            ))
            try:
                b["mesures_string_lignes"] = json.loads(r_ms["data"]) if r_ms and r_ms.get("data") else []
            except Exception:
                b["mesures_string_lignes"] = []
        # v218.179 : Pour les modules 'borne_charge', charger les sous-équipements + valeurs cochées
        if (b.get("type") or "") == "borne_charge":
            sous_eqs = rows(db.execute(
                "SELECT id, designation, marque, type, numero_serie FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id",
                (iv["equipement_id"],)
            ))
            ...
            b["borne_charge_lignes"] = bc_lignes
    # Valeurs saisies
...
def _bloc_equipement_data(equipement_id, intervention_id, db):
    ...
    # v218.177 : Sous-équipements universels (table equipement_sous_equipements, créée en v218.157)
    sous_equipements_list = []
    try:
        sous_equipements_list = [dict(r) for r in rows(db.execute(
            "SELECT designation, marque, type, numero_serie, nombre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id",
            (equipement_id,)
        ))]
    except Exception:
        sous_equipements_list = []
    ...
    # v218.175 : charger les valeurs des champs personnalisés associés à l'équipement
    # Format dans le dict retourné : {"custom:<id>": {"label": "...", "value": "..."}}
    # v218.178 : fallback par type_technique (texte) si technique_id est NULL
    custom_fields = {}
    try:
        # Récupérer la société + technique de l'équipement
        eq_meta = one(db.execute("SELECT societe_id, technique_id, type_technique FROM equipements WHERE id=?", (equipement_id,)))
        sid_eq = eq_meta.get("societe_id") if eq_meta else current_societe_id()
        tech_id = eq_meta.get("technique_id") if eq_meta else None
        # Si technique_id manquant, essayer de le résoudre via type_technique (le nom texte)
        if not tech_id and eq_meta and eq_meta.get("type_technique"):
            tn = (eq_meta.get("type_technique") or "").strip()
            if tn:
                t_row = one(db.execute("SELECT id FROM techniques WHERE LOWER(nom)=LOWER(?)", (tn,)))
                if t_row:
                    tech_id = t_row.get("id")
        # Champs globaux + champs spécifiques à la technique de l'équipement
        if tech_id:
            champ_defs = rows(db.execute("""
                SELECT id, label, slug, type FROM equipement_champs
                WHERE societe_id=? AND (technique_id IS NULL OR technique_id=?)
                ORDER BY ordre, label
            """, (sid_eq, tech_id)))
        else:
            champ_defs = rows(db.execute("""
                SELECT id, label, slug, type FROM equipement_champs
                WHERE societe_id=? AND technique_id IS NULL
                ORDER BY ordre, label
            """, (sid_eq,)))
        for c in champ_defs:
            val_row = one(db.execute(
                "SELECT valeur FROM equipement_valeurs_custom WHERE equipement_id=? AND champ_id=?",
                (equipement_id, c["id"])
            ))
            val = (val_row.get("valeur") if val_row else "") or ""
            custom_fields[f"custom:{c['id']}"] = {
                "label": c.get("label") or c.get("slug") or f"Champ {c['id']}",
                "value": str(val),
                "type": c.get("type") or "text",
            }
    except Exception as e:
        logger.error(f"[_bloc_equipement_data] champs perso échec eid={equipement_id}: {e}")
    ...
    result = {
        ...
        # v218.175 : champs personnalisés
        "custom_fields": custom_fields,
        # v218.177 : sous-équipements universels (toutes techniques)
        "sous_equipements_list": sous_equipements_list,
    }
    return result
...
def _intervention_mesures_data(intervention_id):
    ...
    # v218.181 : Pour un bon de MISE EN SERVICE, on charge les modules rapport_kind='mise_en_service'
    iv_type = (iv.get("type") or "").upper()
    if iv_type == "DEPANNAGE":
        sid = iv.get("societe_id") or current_societe_id()
        blocs = rows(db.execute("""
        ...
        # MES : modules par technique × sous-type avec rapport_kind='mise_en_service'
        # v218.188 : sous-type forcé à "Mise en service" (pas via les gammes, qui sont pour la maintenance)
        # v218.189 : logs détaillés pour diagnostic
        sid = iv.get("societe_id") or current_societe_id()
        technique_id = _resolve_technique_id(iv["equipement_id"])
        logger.info(f"[MES rapport] iid={intervention_id} eq={iv['equipement_id']} resolved_tech_id={technique_id} sid={sid}")
        if not technique_id:
            logger.warning(f"[MES rapport] technique_id introuvable pour eq={iv['equipement_id']}")
            return []
        # Trouver le sous-type "Mise en service" (case-insensitive)
        st = one(db.execute("SELECT id FROM maintenance_sous_types WHERE LOWER(nom)='mise en service'"))
        logger.info(f"[MES rapport] sous_type='mise en service' resolved={st}")
        if not st:
            logger.warning("[MES rapport] sous-type 'Mise en service' introuvable")
            return []
        blocs = rows(db.execute("""
            SELECT id, nom, type, field_options, largeur, page_num, icon FROM mesure_blocs
            WHERE technique_id=? AND sous_type_id=? AND rapport_kind='mise_en_service' AND societe_id=?
            ORDER BY page_num, ordre, nom
        """, (technique_id, st["id"], sid)))
        logger.info(f"[MES rapport] blocs trouvés={len(blocs)} pour tech={technique_id} st={st['id']} sid={sid}")
    else:
        technique_id = _resolve_technique_id(iv["equipement_id"])
        if not technique_id:
            return []
        ...
        blocs = rows(db.execute("""
            SELECT id, nom, type, field_options, largeur, page_num, icon FROM mesure_blocs
            WHERE technique_id=? AND sous_type_id=? AND (is_bc=0 OR is_bc IS NULL) AND (rapport_kind='maintenance' OR rapport_kind IS NULL)
            ORDER BY page_num, ordre, nom
        """, (technique_id, st["id"])))
    if not blocs:
        return []
    ...
    for b in blocs:
        ...
        # v218.209 : on stocke aussi le path absolu pour rendu PDF + on charge equipement_data pour les modules 'equipement'
        if bloc_data["type"] == "image":
            ...
            if iv2 and iv2.get("filename"):
                ...
                # v218.209 : path absolu pour le PDF
                try:
                    bloc_data["equipement_image_path"] = str(IMAGES_DIR / iv2["filename"])
                except Exception:
                    pass
        # v218.209 : Pour les modules 'equipement' : charger les données équipement (comme l'endpoint frontend)
        if bloc_data["type"] == "equipement":
            try:
                iv2 = one(db.execute(
                    "SELECT equipement_id FROM interventions WHERE id=?",
                    (intervention_id,)
                ))
                if iv2 and iv2.get("equipement_id"):
                    eq_data = _bloc_equipement_data(iv2["equipement_id"], intervention_id, db)
                    bloc_data["equipement_data"] = eq_data
            except Exception as e:
                logger.warning(f"[_intervention_mesures_data] equipement_data échec iv={intervention_id}: {e}")
...
def generate_pdf(iid):
    ...
    # v218.199 : prendre TOUS les techniciens du bon via intervention_techniciens,
    # complété par ceux des CRs et le tech principal
    noms_uniques = []
    seen_n = set()
    # 1) Techniciens affectés au bon (intervention_techniciens)
    try:
        tech_rows = rows(db.execute("""
            SELECT u.nom FROM intervention_techniciens it
            JOIN utilisateurs u ON u.id = it.utilisateur_id
            WHERE it.intervention_id = ?
            ORDER BY u.nom
        """, (iid,)))
        for tr in tech_rows:
            n = (tr.get("nom") or "").strip()
            if n and n not in seen_n:
                seen_n.add(n); noms_uniques.append(n)
    except Exception: pass
    # 2) Intervenants des CRs (si présents)
    for cr in comptes_rendus_pdf:
        for iv_int in cr.get("intervenants", []):
            n = iv_int.get("nom", "")
            if n and n != "—" and n not in seen_n:
                seen_n.add(n); noms_uniques.append(n)
    # 3) Fallback technicien principal
    ...
    try:
        ...
        _societe_nom = ""
        _societe_logo_path = ""
        _equipment_image_path = ""
        try:
            _iv_sid = i.get("societe_id") or current_societe_id()
            _iv_type = (i.get("type") or "").upper()
            # v218.185 : router selon type d'intervention
            if _iv_type == "DEPANNAGE":
                _cover_table = "cover_pages_bc"
            elif _iv_type == "MISE_EN_SERVICE":
                _cover_table = "cover_pages_mes"
            else:
                _cover_table = "cover_pages"
            ...
            # v218.185 : photo de l'équipement (depuis image_library via equipements.image_id)
            try:
                _eq_img = one(db.execute("""
                    SELECT il.filename FROM equipements e
                    LEFT JOIN image_library il ON e.image_id = il.id
                    WHERE e.id = ?
                """, (i.get("equipement_id"),)))
                if _eq_img and _eq_img.get("filename"):
                    _img_full = str(IMAGES_DIR / _eq_img["filename"])
                    if os.path.exists(_img_full):
                        _equipment_image_path = _img_full
            except Exception: pass
        except Exception as _e:
        ...
        pdf = generate_rapport({
            ...
            "equipment_image_path": _equipment_image_path,  # v218.185
            "sous_type": _sous_type_label,
            "mesures_techniques": _mesures_techniques,
            # v218.198 : heures réelles de début/fin
            "heure_debut_reel": i.get("heure_debut_reel", "") or "",
            "heure_fin_reel": i.get("heure_fin_reel", "") or "",
            # v218.200 : heure planifiée (fallback pour heure_debut sur PDF)
            "heure_prevue": i.get("heure_prevue", "") or "",
            ...
            # v218.158 : sous-équipements universels (toutes techniques)
            "sous_equipements": (rows(db.execute("SELECT designation, marque, type, numero_serie, nombre FROM equipement_sous_equipements WHERE equipement_id=? ORDER BY ordre, id", (i.get("equipement_id"),))) if i.get("equipement_id") else []),
            "securite_items": (rows(db.execute("""
                SELECT se.id, se.libelle, se.photo_data, se.photo_mime,
                       isec.present AS present,
                       isec.conforme AS conforme

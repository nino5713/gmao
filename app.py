    ...
    try:
        ...
        db.execute("""INSERT INTO interventions
            ...
            (num,d["equipement_id"],to_int(d.get("tableau_id")),main_tech,to_int(d.get("equipe_id")),
             type_iv,d.get("statut",default_statut),d.get("date_prevue") or None,heure_prevue_v,d.get("description",""),gamme_id_v,sid))
...
def list_equipement_champs():
    ...
    for c in champs:
        ...
        c["is_system"] = False
    # v218.121 : ajouter les champs système si une technique est ciblée
    if tid:
        sys_champs = _system_champs_for_technique(tid, db)
        # Insérer après les globaux (technique_id IS NULL) et avant les customs spécifiques
        # = tri final par (is_system, ordre)
        champs = champs + sys_champs
        # Re-tri : globaux d'abord, puis système, puis customs spécifiques
        champs.sort(key=lambda c: (
            0 if (c.get("technique_id") is None and not c.get("is_system")) else (1 if c.get("is_system") else 2),
            c.get("ordre", 0) or 0,
            c.get("label", "") or ""
        ))
...
def get_equipement_valeurs_custom(eid):
    ...
    for c in champs:
        ...
        out.append(item)
    # v218.121 : ajouter les champs système avec leur valeur native
    if tid:
        sys_champs = _system_champs_for_technique(tid, db)
        for sc in sys_champs:
            col = sc["system_col"]
            raw_val = eq.get(col)
            val_str = "" if raw_val is None else str(raw_val)
            out.append({
                "champ_id": sc["id"], "slug": sc["slug"], "label": sc["label"],
                "type": sc["type"], "options": sc["options"],
                "technique_id": sc["technique_id"],
                "technique_nom": sc["technique_nom"],
                "ordre": sc["ordre"],
                "is_system": True,
                "system_col": col,
                "valeur": val_str,
            })
    # Trier : globaux non-système d'abord, puis système, puis customs spécifiques

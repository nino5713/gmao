...
def planning_me():
    ...
    try:
        ...
        # 2) Interventions dont un CR tombe dans la fenêtre debut/fin (même si le bon parent est hors période)
        crs_in_range = rows(db.execute(
            """SELECT DISTINCT cr.intervention_id
               ...
               WHERE cr.date_intervention BETWEEN ? AND ?
                 AND ci.utilisateur_id = ?""",
            (debut, fin, target_id)
        ))
        ...
        if intervention_ids:
            ...
            for iid, crs in crs_by_iv.items():
                ...
                for cr in crs:
                    cr_date = (cr.get("date_intervention") or "")[:10]

...
ALLOWED_UPDATE_COLS = {
    ...
    "utilisateurs":          {"nom","email","role","actif","password","token_version","matricule","manager_id"},
...
def get_utilisateurs():
    return jsonify(rows(get_db().execute("""
        SELECT u.id, u.nom, u.email, u.role, u.actif, u.matricule, u.manager_id,
...
def create_utilisateur():
    ...
    try:
        db.execute("INSERT INTO utilisateurs (nom,email,password,role,matricule,manager_id) VALUES (?,?,?,?,?,?)",
                   (d["nom"],d["email"].strip().lower(),hash_password(d["password"]),d.get("role","technicien"),matricule,manager_id))

...
def get_utilisateurs():
    return jsonify(rows(get_db().execute("SELECT id,nom,email,role,actif,matricule FROM utilisateurs ORDER BY nom")))
...
def create_utilisateur():
    ...
    try:
        db.execute("INSERT INTO utilisateurs (nom,email,password,role,matricule) VALUES (?,?,?,?,?)",
                   (d["nom"],d["email"].strip().lower(),hash_password(d["password"]),d.get("role","technicien"),matricule))
...
def create_occupation():
    ...
    for tid in tech_ids:
        db.execute("""INSERT INTO occupations
            (technicien_id, type_id, date, heure_debut, heure_fin, total_heures, notes, numero_projet, nom_chantier)
            VALUES (?,?,?,?,?,?,?,?,?)""",
            (tid, type_id, date, heure_debut, heure_fin,
             total_heures, d.get("notes",""), numero_projet, nom_chantier))
...
def update_occupation(oid):
    ...
    if is_offre:
        ...
        d["nom_chantier"] = ""
    sets, params = [], []
    for f in ["type_id", "date", "heure_debut", "heure_fin", "notes", "numero_projet", "nom_chantier"]:
        if f in d:
            sets.append(f"{f}=?")
...
def pointage_fiche_hebdo(annee, semaine, technicien_id):
    ...
    ws["B7"] = tech.get("nom","")
    # Matricule : on n'en a pas dans le modèle utilisateurs, on met l'id
    ws["I7"] = tech.get("matricule") or ""
    # Manager : on cherche si le projet le plus fréquent du tech est géré par un manager, sinon vide
    mgr = one(db.execute("""
        SELECT u2.nom AS nom FROM utilisateurs u2 WHERE u2.role='manager' LIMIT 1
    """))
    ws["V7"] = (mgr or {}).get("nom","") or ""

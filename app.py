...
def get_interventions():
    ...
    for r in result:
        ...
        # v218.202 : récupérer les noms (prénom + nom) pour affichage tuile planning
        if r["technicien_ids"]:
            ph = ",".join("?" * len(r["technicien_ids"]))
            tech_noms_rows = rows(db.execute(
                f"SELECT id, prenom, nom FROM utilisateurs WHERE id IN ({ph}) ORDER BY prenom, nom",
                tuple(r["technicien_ids"])
            ))
            r["technicien_noms"] = [
                ((tn.get("prenom") or "") + " " + (tn.get("nom") or "")).strip()
                for tn in tech_noms_rows
                if (tn.get("prenom") or tn.get("nom"))
            ]
        else:
            r["technicien_noms"] = []
...
def generate_pdf(iid):
    ...
    try:
        tech_rows = rows(db.execute("""
            SELECT u.prenom, u.nom FROM intervention_techniciens it
            JOIN utilisateurs u ON u.id = it.utilisateur_id
            WHERE it.intervention_id = ?
            ORDER BY u.prenom, u.nom
        """, (iid,)))
        for tr in tech_rows:
            n = ((tr.get("prenom") or "") + " " + (tr.get("nom") or "")).strip()
